...
def collect_imported_module_context(imports):
    ...
    while True:
        ...
        if fs.exists(manifest_path):
            ...
            diagnostics = (diagnostics) + (["layout manifest: missing artifact for import `" +
                                            module_path + "` (expected " + manifest_path + ")"])
        native_text = ""
        if fs.exists(native_text_path):
            native_text = fs.readFile(native_text_path)
        else:
            diagnostics = (diagnostics) + (["native lowering: missing native text artifact for import `" +
                                            module_path + "` (expected " + native_text_path + ")"])
        manifests = (
            manifests) + ([LayoutManifest(structs=manifest_structs, enums=manifest_enums, diagnostics=[])])
    ...
    while True:
        ...
        if target_index < 0:
            diagnostics = append_string(
                diagnostics, "layout manifest: struct `" + layout_struct.name + "` missing from native module")
        else:
            target = updated_structs[target_index]
            replaced = NativeStruct(name=target.name, fields=target.fields, methods=target.methods,
                                    implements=target.implements, layout=layout_struct.layout)
            updated_structs = replace_native_struct(
                updated_structs, target_index, replaced)
        struct_index += 1
    updated_enums = enums
    enum_index = 0
    while True:
        ...
        if target_index < 0:
            diagnostics = append_string(
                diagnostics, "layout manifest: enum `" + layout_enum.name + "` missing from native module")
        else:
            target = updated_enums[target_index]
            replaced = NativeEnum(
                name=target.name, variants=target.variants, layout=layout_enum.layout)
            updated_enums = replace_native_enum(
                updated_enums, target_index, replaced)
        enum_index += 1
...
def lower_to_llvm_with_context(native_module, imported_manifests, imported_native_texts, imported_diagnostics):
    ...
    while True:
        ...
        function_effects = append_function_effect_entry(function_effects, FunctionEffectEntry(
            name=current_function.name, effects=effective_effects))
        lowered = emit_function(
            current_function, context_functions, effective_effects, type_context)
    ...
    if not has_add_function:
        ...
        function_lines = (function_lines) + (
            ["define double @add(double %a, double %b) {", "entry:", "  %t0 = fadd double %a, %b", "  ret double %t0", "}"])
...
def render_trait_metadata_comments(metadata):
    ...
    while True:
        ...
        if len(interface.type_parameters) > 0:
            header = header + "<" + \
                join_with_separator(interface.type_parameters, ", ") + ">"
        ...
        if len(interface.signatures) == 0:
            interface_lines = append_string(
                interface_lines, ";   ; no signatures recorded")
        index += 1
    struct_lines = []
    index = 0
    while True:
        ...
        line = "; struct " + implementation.struct_name + " implements " + \
    ...
    while True:
        ...
        if string_array_contains(used_targets, descriptor.target):
            if len(descriptor.effects) > 0:
                ...
                lines = append_string(lines, "; intrinsic " + descriptor.symbol +
                                      " requires capabilities: ![" + effects_text + "]")
            parameter_text = ""
            if len(descriptor.parameter_types) > 0:
                parameter_text = join_with_separator(
                    descriptor.parameter_types, ", ")
            line = "declare " + descriptor.return_type + " @" + \
                descriptor.symbol + "(" + parameter_text + ")"
...
def build_type_context(structs, enums, interfaces):
    ...
    while True:
        ...
        if definition.layout == None:
            diagnostics = append_string(diagnostics, "llvm lowering: struct `" +
                                        definition.name + "` missing layout metadata; skipping type emission")
            index += 1
            continue
        ...
        while True:
            ...
            if len(llvm_field_type) == 0:
                diagnostics = append_string(diagnostics, "llvm lowering: struct `" + definition.name + "` field `" +
                                            layout_field.name + "` uses unsupported type `" + layout_field.type_annotation + "`; lowering as `i8*`")
                llvm_field_type = "i8*"
            else:
                if ends_with_pointer_suffix(llvm_field_type):
                    layout_base_type = layout_annotation_base_type(
            ...
            fields = append_struct_field_info(fields, StructFieldInfo(
                name=layout_field.name, llvm_type=llvm_field_type, index=field_index, offset=layout_field.offset))
            field_index += 1
    ...
    while True:
        ...
        if enum_def.layout == None:
            diagnostics = append_string(diagnostics, "llvm lowering: enum `" +
                                        enum_def.name + "` missing layout metadata; skipping type emission")
            enum_index += 1
            continue
        ...
        while True:
            ...
            while True:
                ...
                mapped_variant_type = map_struct_field_annotation(
                    variant_field.type_annotation)
                llvm_variant_field_type = mapped_variant_type
                if len(llvm_variant_field_type) == 0:
                    diagnostics = append_string(diagnostics, "llvm lowering: enum `" + enum_def.name + "` variant `" + variant_layout.name +
                                                "` field `" + variant_field.name + "` uses unsupported type `" + variant_field.type_annotation + "`; lowering as `i8*`")
                    llvm_variant_field_type = "i8*"
                else:
                    if ends_with_pointer_suffix(llvm_variant_field_type):
                        variant_base_type = layout_annotation_base_type(
                            variant_field.type_annotation)
                        ...
                        if layout_annotation_represents_user_value(variant_field.type_annotation) and not references_declaring_enum:
                            stripped_variant = strip_pointer_suffix(
                                llvm_variant_field_type)
                ...
                variant_fields = append_struct_field_info(variant_fields, StructFieldInfo(
                    name=variant_field.name, llvm_type=llvm_variant_field_type, index=variant_field_index, offset=variant_field.offset))
                variant_field_index += 1
            variants = append_enum_variant_info(variants, EnumVariantInfo(name=variant_layout.name, tag=variant_layout.tag,
                                                offset=variant_layout.offset, size=variant_layout.size, align=variant_layout.align, fields=variant_fields))
            variant_index += 1
        enum_align_value = enum_layout.align
        if enum_align_value <= 0:
            enum_align_value = 1
        enum_entries = append_enum_type_info(enum_entries, EnumTypeInfo(name=enum_def.name, llvm_name=llvm_enum_name, tag_type=enum_layout.tag_type,
                                             tag_size=enum_layout.tag_size, tag_align=enum_layout.tag_align, size=enum_layout.size, align=enum_align_value, variants=variants))
        enum_index += 1
    interface_entries = []
    interface_index = 0
    while True:
    ...
    while True:
        ...
        while True:
            ...
            if interface_def_opt == None:
                diagnostics = append_string(diagnostics, "llvm lowering: struct `" +
                                            struct_def.name + "` implements unknown interface `" + interface_name + "`")
                implements_index += 1
                continue
            ...
            vtable_global_name = "@vtable." + sanitized_struct + \
                "." + sanitized_iface + ".const"
            ...
            while True:
                ...
                sig_index += 1
            vtable_entries = append_vtable_info(vtable_entries, VTableInfo(struct_name=struct_def.name, interface_name=interface_name,
                                                llvm_type_name=vtable_type_name, llvm_global_name=vtable_global_name, entries=vtable_method_entries))
            implements_index += 1
...
def flatten_struct_methods(structs):
    ...
    while True:
        ...
        while True:
            ...
            qualified = NativeFunction(name=qualified_name, parameters=method.parameters,
                                       return_type=method.return_type, effects=method.effects, instructions=method.instructions)
    ...
    while True:
        ...
        while True:
            if field_index >= len(info.fields):
                break
            field_types = append_string(
                field_types, info.fields[field_index].llvm_type)
            field_index += 1
        body = ""
...
def render_enum_type_definitions(context):
    ...
    while True:
        ...
        if max_payload_size > 0:
            payload_repr = ", [" + \
                number_to_string(max_payload_size) + " x i8]"
...
def render_interface_type_definitions(context):
    ...
    while True:
        ...
        body = "{ i8*, i8* }"
        lines = append_string(
            lines, interface_info.llvm_name + " = type " + body)
        index += 1
    return lines
    ...
    while True:
        ...
        while True:
            if entry_index >= len(vtable.entries):
                break
            entry_types = append_string(
                entry_types, vtable.entries[entry_index].function_pointer_type)
            entry_index += 1
        body = ""
...
def render_vtable_constants(context):
    ...
    while True:
        ...
        while True:
            ...
            cast_value = "bitcast (" + entry.function_pointer_type + " @" + \
                sanitized_method + " to " + entry.function_pointer_type + ")"
        ...
        lines = append_string(lines, vtable.llvm_global_name +
                              " = global " + vtable.llvm_type_name + " " + body)
        index += 1
    return lines
...
def resolve_struct_info_for_method_target(base, bindings, locals, context):
    ...
    if is_simple_identifier(trimmed):
        ...
        if parameter != None:
            info = resolve_struct_info_from_llvm_type(
                context, parameter.llvm_type)
            if info != None:
                return info
    ...
    if last_dot >= 0:
        ...
        if len(tail_full) > 0:
            ...
            if tail_generic_index >= 0:
                tail_base = trim_text(
                    substring(tail_full, 0, tail_generic_index))
                if len(tail_base) > 0:
...
def collect_function_call_graph(functions):
    ...
    while True:
        if index >= len(functions):
            break
        call_entry = collect_function_call_entry(
            functions[index], function_names)
...
def collect_function_call_entry(function, function_names):
    ...
    while True:
        ...
        callees = merge_effect_lists(
            callees, collect_instruction_calls(instruction, function_names))
        index += 1
...
def collect_function_runtime_helper_targets(function):
    ...
    while True:
        if index >= len(function.instructions):
            break
        instruction = function.instructions[index]
        instruction_helpers = collect_instruction_runtime_helper_targets(
            instruction)
...
def collect_instruction_runtime_helper_targets(instruction):
    if instruction.variant == "Let":
        if instruction.value != None:
            helpers = filter_runtime_helper_targets(
                extract_all_call_targets(instruction.value))
            helpers = merge_effect_lists(
                helpers, collect_runtime_property_targets(instruction.value))
            return helpers
        return []
    if instruction.variant == "Expression":
        helpers = filter_runtime_helper_targets(
            extract_all_call_targets(instruction.expression))
        helpers = merge_effect_lists(
            helpers, collect_runtime_property_targets(instruction.expression))
        return helpers
    if instruction.variant == "Return":
        trimmed = trim_text(instruction.expression)
        if len(trimmed) > 0:
            helpers = filter_runtime_helper_targets(
                extract_all_call_targets(trimmed))
            helpers = merge_effect_lists(
                helpers, collect_runtime_property_targets(trimmed))
            return helpers
        return []
    if instruction.variant == "If":
        helpers = filter_runtime_helper_targets(
            extract_all_call_targets(instruction.condition))
        helpers = merge_effect_lists(
            helpers, collect_runtime_property_targets(instruction.condition))
        return helpers
    if instruction.variant == "For":
        helpers = filter_runtime_helper_targets(
            extract_all_call_targets(instruction.iterable))
        helpers = merge_effect_lists(
            helpers, collect_runtime_property_targets(instruction.iterable))
        return helpers
    ...
    if instruction.variant == "Case":
        helpers = filter_runtime_helper_targets(
            extract_all_call_targets(instruction.pattern))
        helpers = merge_effect_lists(
            helpers, collect_runtime_property_targets(instruction.pattern))
        if instruction.guard != None:
            guard_targets = filter_runtime_helper_targets(
                extract_all_call_targets(instruction.guard))
            combined = merge_effect_lists(helpers, guard_targets)
            combined = merge_effect_lists(
                combined, collect_runtime_property_targets(instruction.guard))
            return combined
        return helpers
    return []
...
def collect_instruction_calls(instruction, function_names):
    ...
    if instruction.variant == "Let":
        if instruction.value != None:
            callees = merge_effect_lists(
                callees, extract_call_targets(instruction.value, function_names))
        return callees
    if instruction.variant == "Expression":
        callees = merge_effect_lists(callees, extract_call_targets(
            instruction.expression, function_names))
        return callees
    if instruction.variant == "Return":
        trimmed = trim_text(instruction.expression)
        if len(trimmed) > 0:
            callees = merge_effect_lists(
                callees, extract_call_targets(trimmed, function_names))
        return callees
    if instruction.variant == "If":
        callees = merge_effect_lists(callees, extract_call_targets(
            instruction.condition, function_names))
        return callees
    if instruction.variant == "For":
        callees = merge_effect_lists(callees, extract_call_targets(
            instruction.iterable, function_names))
        return callees
    if instruction.variant == "Match":
        callees = merge_effect_lists(callees, extract_call_targets(
            instruction.expression, function_names))
    ...
    if instruction.variant == "Case":
        callees = merge_effect_lists(callees, extract_call_targets(
            instruction.pattern, function_names))
        if instruction.guard != None:
            callees = merge_effect_lists(
                callees, extract_call_targets(instruction.guard, function_names))
        return callees
    return callees
            ...
            if len(candidate) > 0:
                while True:
                    ...
                    if dot_index == -1:
                        break
                    candidate = trim_text(
                        substring(candidate, dot_index + 1, len(candidate)))
                cursor = index
                while True:
    ...
    while True:
        ...
        if helper == None:
            ...
            if len(trimmed) >= len(length_suffix):
                maybe_length = substring(trimmed, len(
                    trimmed) - len(length_suffix), len(trimmed))
            ...
            if helper == None:
                ...
                if len(trimmed) >= len(concat_suffix):
                    maybe_concat = substring(trimmed, len(
                        trimmed) - len(concat_suffix), len(trimmed))
...
def propagate_function_effects(direct_effects, call_graph):
    ...
    while True:
        if index >= len(direct_effects):
            break
        aggregated = (aggregated) + ([FunctionEffectEntry(name=direct_effects[index].name,
                                                          effects=copy_string_array(direct_effects[index].effects))])
        index += 1
    changed = True
    while True:
        if not changed:
        ...
        while True:
            ...
            target_index = find_function_effect_entry_index(
                aggregated, call_entry.name)
            if target_index >= 0:
                ...
                while True:
                    ...
                    callee_pos = find_function_effect_entry_index(
                        aggregated, callee_name)
                    if callee_pos >= 0:
                        ...
                        while True:
                            if effect_index >= len(callee_entry.effects):
                                break
                            merged = append_unique_effect(
                                merged, callee_entry.effects[effect_index])
                            effect_index += 1
                    callee_index += 1
                if not string_arrays_equal(merged, target_entry.effects):
                    updated_entry = FunctionEffectEntry(
                        name=target_entry.name, effects=merged)
                    aggregated = replace_function_effect_entry(
                        aggregated, target_index, updated_entry)
                    changed = True
            call_index += 1
    return aggregated
...
def build_capability_manifest(entry_points, function_effects):
    ...
    while True:
        ...
        entries = append_manifest_entry(
            entries, CapabilityManifestEntry(symbol=symbol, effects=effects))
        index += 1
...
def runtime_helper_descriptors():
    ...
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
        target="console.info", symbol="sailfin_intrinsic_io_print", return_type="void", parameter_types=["i8*"], effects=["io"]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
        target="console.log", symbol="sailfin_intrinsic_io_print", return_type="void", parameter_types=["i8*"], effects=["io"]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
    ...
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(target="runtime_create_capability_grant",
                                        symbol="sailfin_runtime_create_capability_grant", return_type="i8*", parameter_types=["i8*"], effects=[]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(target="runtime_create_filesystem_bridge",
                                        symbol="sailfin_runtime_create_filesystem_bridge", return_type="i8*", parameter_types=["i8*"], effects=["io"]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
    ...
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(target="runtime_create_model_bridge",
                                        symbol="sailfin_runtime_create_model_bridge", return_type="i8*", parameter_types=["i8*"], effects=["model"]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
    ...
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
        target="fs_list_directory", symbol="sailfin_adapter_fs_list_directory", return_type="i8*", parameter_types=["i8*"], effects=["io"]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
        target="fs.listDirectory", symbol="sailfin_adapter_fs_list_directory", return_type="i8*", parameter_types=["i8*"], effects=["io"]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
    ...
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
        target="substring", symbol="sailfin_runtime_substring", return_type="i8*", parameter_types=["i8*", "i64", "i64"], effects=[]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
        target="len(string)", symbol="sailfin_runtime_string_length", return_type="i64", parameter_types=["i8*"], effects=[]))
    descriptors = append_runtime_helper(descriptors, RuntimeHelperDescriptor(
...
def collect_function_effect_entry(function):
    ...
    combined = merge_effect_lists(
        combined, collect_function_borrow_effects(function))
...
def collect_function_borrow_effects(function):
    ...
    while True:
        ...
        if instruction.variant == "Let":
            if instruction.value != None:
                effects = merge_effect_lists(
                    effects, collect_expression_borrow_effects(instruction.value))
        else:
            if instruction.variant == "Expression":
                effects = merge_effect_lists(
                    effects, collect_expression_borrow_effects(instruction.expression))
            else:
                if instruction.variant == "Return":
                    trimmed = trim_text(instruction.expression)
                    if len(trimmed) > 0:
                        effects = merge_effect_lists(
                            effects, collect_expression_borrow_effects(trimmed))
        index += 1
    return effects
...
def render_interface_signature(signature):
    ...
    if len(signature.type_parameters) > 0:
        line = line + "<" + \
            join_with_separator(signature.type_parameters, ", ") + ">"
...
def emit_function(function, functions, effects, context):
    ...
    if len(llvm_return) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unsupported return type `" + function.return_type + "` in " + function.name)
        empty_constants = empty_string_constant_set()
    ...
    lines = []
    if len(effects) > 0:
        lines = append_string(lines, "; fn " + function.name +
                              " effects: ![" + join_with_separator(effects, ", ") + "]")
    signature = join_with_separator(preparation.signature, ", ")
    if len(signature) == 0:
        signature = ""
    lines = append_string(lines, "define " + llvm_return +
                          " @" + sanitized + "(" + signature + ") {")
    lines = append_string(lines, "entry:")
    body = emit_body(function, llvm_return,
                     preparation.bindings, functions, context)
    lines = (lines) + (body.lines)
    diagnostics = (diagnostics) + (body.diagnostics)
    lifetime_diagnostics = validate_borrow_lifetimes(
        function, body.lifetime_regions)
...
def emit_body(function, llvm_return, bindings, functions, context):
    ...
    if not lowered.terminated:
        if llvm_return == "void":
            ...
            diagnostics = append_string(
                diagnostics, "llvm lowering: missing return in function `" + function.name + "`")
            lines = append_string(
                lines, "  ret " + llvm_return + " " + default_return_literal(llvm_return))
...
def validate_borrow_lifetimes(function, regions):
    ...
    while True:
        ...
        if violation:
            ...
            diagnostics = append_string(diagnostics, "llvm lowering: borrow `" + region.binding + "` of `" +
                                        region.base + "` escapes lifetime of `" + region.base + "` in `" + function.name + "`" + location)
        index += 1
    return diagnostics
...
def lower_instruction_range(function, start_index, end, llvm_return, bindings, locals, allocas, lines, temp_index, block_counter, next_local_id, next_region_id, functions, loop_stack, context, scope_id, scope_depth, initial_label):
    ...
    while True:
        ...
        instruction = function.instructions[index]
        if instruction.variant == "Let":
            lowered = lower_let_instruction(function, instruction, current_bindings, current_locals, current_allocas, current_lines,
                                            current_temp, current_next_local, current_next_region, functions, context, scope_id, scope_depth, current_label)
            diagnostics = (diagnostics) + (lowered.diagnostics)
            collected_lifetime_regions = (
                collected_lifetime_regions) + (lowered.lifetime_regions)
            ...
            collected_string_constants = merge_string_constants(
                temp_constants, lowered.string_constants)
            ...
            if instruction.variant == "Expression":
                ...
                if len(trimmed_expression) >= 4:
                    ...
                    if prefix == "let ":
                        inline_parse = parse_inline_let_expression(
                            trimmed_expression)
                        diagnostics = (diagnostics) + \
                            (inline_parse.diagnostics)
                        if inline_parse.success:
                            inline_instruction = runtime.enum_instantiate(NativeInstruction, 'Let', [runtime.enum_field('name', inline_parse.name), runtime.enum_field('mutable', inline_parse.mutable), runtime.enum_field(
                                'type_annotation', inline_parse.type_annotation), runtime.enum_field('value', inline_parse.initializer), runtime.enum_field('span', instruction.span), runtime.enum_field('value_span', instruction.span)])
                            lowered = lower_let_instruction(function, inline_instruction, current_bindings, current_locals, current_allocas, current_lines,
                                                            current_temp, current_next_local, current_next_region, functions, context, scope_id, scope_depth, current_label)
                            diagnostics = (diagnostics) + (lowered.diagnostics)
                            collected_lifetime_regions = (
                                collected_lifetime_regions) + (lowered.lifetime_regions)
                            ...
                            current_next_region = lowered.next_region_id
                            current_mutations = (
                                current_mutations) + (lowered.mutations)
                            temp_constants = collected_string_constants
                            collected_string_constants = merge_string_constants(
                                temp_constants, lowered.string_constants)
                            handled_inline_let = True
                if not handled_inline_let:
                    lowered = lower_expression_statement(function.name, instruction, trimmed_expression, current_bindings, current_locals,
                                                         current_temp, current_lines, current_next_region, scope_id, scope_depth, functions, context, current_label)
                    diagnostics = (diagnostics) + (lowered.diagnostics)
                    current_lines = lowered.lines
                    current_temp = lowered.temp_index
                    current_locals = lowered.locals
                    ...
                    collected_lifetime_regions = (
                        collected_lifetime_regions) + (lowered.lifetime_regions)
                    collected_lifetime_regions = apply_lifetime_release_events(
                        collected_lifetime_regions, lowered.lifetime_releases)
                    current_next_region = lowered.next_region_id
                    current_mutations = (
                        current_mutations) + (lowered.mutations)
                    temp_constants = collected_string_constants
                ...
                if instruction.variant == "Return":
                    lowered = lower_return_instruction(function, instruction, llvm_return, current_bindings, current_locals,
                                                       current_temp, current_lines, current_next_region, scope_id, scope_depth, functions, context)
                    diagnostics = (diagnostics) + (lowered.diagnostics)
                    current_lines = lowered.lines
                    current_temp = lowered.temp_index
                    current_locals = lowered.locals
                    ...
                    if index < end:
                        diagnostics = append_string(
                            diagnostics, "llvm lowering: instructions after return ignored in `" + function.name + "`")
                    break
                else:
                    if instruction.variant == "If":
                                    ...
                                    if instruction.variant == "EndLoop" or instruction.variant == "EndMatch" or instruction.variant == "EndFor":
                                        ...
                                        if instruction.variant == "Break":
                                            if len(current_loop_stack) == 0:
                                                diagnostics = append_string(
                                                    diagnostics, "llvm lowering: `break` outside loop in `" + function.name + "`")
                                            else:
                                                loop_context = last_loop_context(
                                                    current_loop_stack)
                                                current_lines = append_string(
                                                    current_lines, "  br label %" + loop_context.break_label)
                                                terminated = True
                                            index += 1
                                            if index < end and terminated:
                                                diagnostics = append_string(
                                                    diagnostics, "llvm lowering: instructions after break ignored in `" + function.name + "`")
                                            break
                                        else:
                                            if instruction.variant == "Continue":
                                                if len(current_loop_stack) == 0:
                                                    diagnostics = append_string(
                                                        diagnostics, "llvm lowering: `continue` outside loop in `" + function.name + "`")
                                                else:
                                                    loop_context = last_loop_context(
                                                        current_loop_stack)
                                                    current_lines = append_string(
                                                        current_lines, "  br label %" + loop_context.continue_label)
                                                    terminated = True
                                                index += 1
                                                if index < end and terminated:
                                                    diagnostics = append_string(
                                                        diagnostics, "llvm lowering: instructions after continue ignored in `" + function.name + "`")
                                                break
                                            else:
                                                if instruction.variant == "For":
                                                    ...
                                                    if instruction.variant == "Noop":
                                                        ...
                                                        diagnostics = append_string(
                                                            diagnostics, "llvm lowering: unsupported instruction `" + instruction.variant + "` in `" + function.name + "`")
        index += 1
...
def collect_if_structure(instructions, start_index, end, function_name):
    ...
    while True:
        if index >= limit:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unterminated `.if` in `" + function_name + "`")
        ...
        if current.variant == "Else":
            if depth == 0:
                if has_else:
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: duplicate `.else` in `.if` within `" + function_name + "`")
                else:
                    has_else = True
                    then_end = index
                    else_start = index + 1
    ...
    while True:
        if index >= limit:
            if limit <= 0:
                limit = 0
            diagnostics = append_string(
                diagnostics, "llvm lowering: unterminated `.loop` in `" + function_name + "`")
...
def lower_loop_instruction(function, start_index, llvm_return, bindings, locals, allocas, lines, temp_index, block_counter, next_local_id, next_region_id, functions, loop_stack, end, context, scope_id, scope_depth, current_label):
    ...
    collected_mutations = []
    collected_string_constants = empty_string_constant_set()
    structure = collect_loop_structure(
        function.instructions, start_index, end, function.name)
    ...
    while True:
        ...
        current_temp += 1
        preload_line = "  " + preload_temp + " = load " + \
            local.llvm_type + ", " + local.llvm_type + "* " + local.pointer
    ...
    loop_context = LoopContext(
        break_label=exit_label, continue_label=latch_label)
    ...
    collected_string_constants = merge_string_constants(
        collected_string_constants, body_result.string_constants)
    header_phis = []
    phi_stores = []
    mutated_names = []
    mut_idx = 0
    ...
    if not body_result.terminated:
        current_lines = append_string(
            current_lines, "  br label %" + latch_label)
    ...
    while True:
        ...
        if local != None:
            ...
            current_temp += 1
            load_line = "  " + load_temp + " = load " + local.llvm_type + \
                ", " + local.llvm_type + "* " + local.pointer
        ...
        if local != None and name_idx < len(latch_loads):
            ...
            if len(latch_value) > 0:
                ...
                if len(preload_value) > 0:
                    ...
                    current_temp += 1
                    phi_line = "  " + phi_temp + " = phi " + local.llvm_type + \
                        " [ " + preload_value + ", %" + current_label + \
                        " ], [ " + latch_value + ", %" + latch_label + " ]"
                    header_phis = append_string(header_phis, phi_line)
                    store_line = "  store " + local.llvm_type + " " + \
                        phi_temp + ", " + local.llvm_type + "* " + local.pointer
    ...
    while True:
        ...
        if local != None:
            ...
            current_temp += 1
            current_lines = append_string(current_lines, "  " + exit_load_temp + " = load " +
                                          local.llvm_type + ", " + local.llvm_type + "* " + local.pointer)
    ...
    while True:
        ...
        if len(exit_value) > 0:
            ...
            if len(llvm_type) == 0:
                llvm_type = "double"
            exit_mutation = LocalMutation(
                name=name, llvm_type=llvm_type, value_name=exit_value, span=original_span, originating_label=exit_label)
...
def lower_for_instruction(function, start_index, llvm_return, bindings, locals, allocas, lines, temp_index, block_counter, next_local_id, next_region_id, functions, loop_stack, end, context, scope_id, scope_depth, current_label):
    ...
    if len(raw_target) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: `.for` loop missing iteration binding in `" + function.name + "`")
    ...
    if not is_simple_identifier(raw_target):
        diagnostics = append_string(diagnostics, "llvm lowering: `.for` loop target `" +
                                    raw_target + "` is not a simple identifier in `" + function.name + "`")
        return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
    if find_local_binding(current_locals, raw_target) != None:
        diagnostics = append_string(diagnostics, "llvm lowering: `.for` loop target `" +
                                    raw_target + "` shadows existing local in `" + function.name + "`")
        return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
    ...
    if range_parse.success:
        ...
        start_result = lower_expression(range_parse.start, current_bindings,
                                        current_locals, current_temp, current_lines, functions, context, "")
        ...
        if start_result.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to lower `.for` range start in `" + function.name + "`")
            return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
        end_result = lower_expression(range_parse.end, current_bindings,
                                      current_locals, current_temp, current_lines, functions, context, "")
        ...
        if end_result.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to lower `.for` range end in `" + function.name + "`")
            return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
        start_coerced = coerce_operand_to_type(
            start_result.operand, "double", current_temp, current_lines)
        ...
        if start_coerced.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: `.for` start expression must evaluate to `number` in `" + function.name + "`")
            return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
        start_operand = start_coerced.operand
        end_coerced = coerce_operand_to_type(
            end_result.operand, "double", current_temp, current_lines)
        ...
        if end_coerced.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: `.for` end expression must evaluate to `number` in `" + function.name + "`")
            return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
        ...
        if len(stride_text) > 0:
            stride_result = lower_expression(
                stride_text, current_bindings, current_locals, current_temp, current_lines, functions, context, "")
            ...
            if stride_result.operand == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: `.for` stride expression did not produce a value in `" + function.name + "`")
                return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
            stride_coerced = coerce_operand_to_type(
                stride_result.operand, "double", current_temp, current_lines)
            ...
            if stride_coerced.operand == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: `.for` stride expression must evaluate to `number` in `" + function.name + "`")
                return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
            ...
            if is_number_literal(stride_text):
                ...
                if normalised_stride == "0.0":
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: `.for` stride must not be zero in `" + function.name + "`")
        stride_pointer = format_local_pointer_name(current_next_local)
        current_next_local += 1
        current_allocas = append_string(
            current_allocas, "  " + stride_pointer + " = alloca double")
        current_lines = append_string(
            current_lines, "  store double " + stride_operand.value + ", double* " + stride_pointer)
        ...
        loop_body_alloc = allocate_block_label(
            "forbody", current_block_counter)
        loop_body_label = loop_body_alloc.label
        current_block_counter = loop_body_alloc.next_counter
        loop_increment_alloc = allocate_block_label(
            "forinc", current_block_counter)
        loop_increment_label = loop_increment_alloc.label
        current_block_counter = loop_increment_alloc.next_counter
        loop_exit_alloc = allocate_block_label(
            "afterfor", current_block_counter)
        ...
        current_next_local += 1
        current_allocas = append_string(
            current_allocas, "  " + iteration_pointer + " = alloca double")
        current_lines = append_string(
            current_lines, "  store double " + start_operand.value + ", double* " + iteration_pointer)
        current_lines = append_string(
            current_lines, "  br label %" + loop_header_label)
        ...
        iteration_binding = LocalBinding(name=raw_target, pointer=iteration_pointer, llvm_type="double", type_annotation="number",
                                         ownership=None, consumed=False, scope_id=loop_body_scope_id, scope_depth=scope_depth + 1)
        header_load = load_local_operand(
            iteration_binding, current_temp, current_lines)
        ...
        if header_load.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: internal error loading `.for` iteration value in `" + function.name + "`")
            current_lines = append_string(
                current_lines, "  br label %" + loop_exit_label)
            current_lines = append_string(current_lines, loop_exit_label + ":")
            return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
        stride_header_name = format_temp_name(current_temp)
        current_lines = append_string(
            current_lines, "  " + stride_header_name + " = load double, double* " + stride_pointer)
        current_temp += 1
        stride_positive_name = format_temp_name(current_temp)
        current_lines = append_string(
            current_lines, "  " + stride_positive_name + " = fcmp ogt double " + stride_header_name + ", 0.0")
        current_temp += 1
        stride_negative_name = format_temp_name(current_temp)
        current_lines = append_string(
            current_lines, "  " + stride_negative_name + " = fcmp olt double " + stride_header_name + ", 0.0")
        current_temp += 1
        ascending_comparison = emit_comparison_instruction(
            "<", header_load.operand, end_operand, current_temp, current_lines)
        ...
        if ascending_comparison.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to compare `.for` range bounds in `" + function.name + "`")
            current_lines = append_string(
                current_lines, "  br label %" + loop_exit_label)
            current_lines = append_string(current_lines, loop_exit_label + ":")
            return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
        descending_comparison = emit_comparison_instruction(
            ">", header_load.operand, end_operand, current_temp, current_lines)
        ...
        if descending_comparison.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to compare descending `.for` range bounds in `" + function.name + "`")
            current_lines = append_string(
                current_lines, "  br label %" + loop_exit_label)
            current_lines = append_string(current_lines, loop_exit_label + ":")
            return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
        ascending_active_name = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + ascending_active_name +
                                      " = and i1 " + stride_positive_name + ", " + ascending_comparison.operand.value)
        current_temp += 1
        descending_active_name = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + descending_active_name +
                                      " = and i1 " + stride_negative_name + ", " + descending_comparison.operand.value)
        current_temp += 1
        continue_name = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + continue_name +
                                      " = or i1 " + ascending_active_name + ", " + descending_active_name)
        current_temp += 1
        current_lines = append_string(current_lines, "  br i1 " + continue_name +
                                      ", label %" + loop_body_label + ", label %" + loop_exit_label)
        current_lines = append_string(current_lines, loop_body_label + ":")
        loop_context = LoopContext(
            break_label=loop_exit_label, continue_label=loop_increment_label)
        stacked = append_loop_context(loop_stack, loop_context)
        ...
        if not body_result.terminated:
            current_lines = append_string(
                current_lines, "  br label %" + loop_increment_label)
        current_lines = append_string(
            current_lines, loop_increment_label + ":")
        increment_load = load_local_operand(
            iteration_binding, current_temp, current_lines)
        ...
        if increment_load.operand != None:
            ...
            current_lines = append_string(
                current_lines, "  " + increment_stride_name + " = load double, double* " + stride_pointer)
            current_temp += 1
            next_value_name = format_temp_name(current_temp)
            current_lines = append_string(current_lines, "  " + next_value_name +
                                          " = fadd double " + increment_load.operand.value + ", " + increment_stride_name)
            current_temp += 1
            current_lines = append_string(
                current_lines, "  store double " + next_value_name + ", double* " + iteration_pointer)
        else:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to increment `.for` iterator in `" + function.name + "`")
        current_lines = append_string(
            current_lines, "  br label %" + loop_header_label)
        current_lines = append_string(current_lines, loop_exit_label + ":")
        ...
        while True:
            ...
            if len(exit_value) > 0:
                ...
                if len(llvm_type) == 0:
                    llvm_type = "double"
                exit_mutation = LocalMutation(
                    name=name, llvm_type=llvm_type, value_name=exit_value, span=original_span, originating_label=loop_exit_label)
                exit_mutations = (exit_mutations) + ([exit_mutation])
            exit_idx += 1
        collected_mutations = exit_mutations
        current_locals = locals
        return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
    iterable_result = lower_expression(instruction.iterable, current_bindings,
                                       current_locals, current_temp, current_lines, functions, context, "")
    ...
    if iterable_result.operand == None:
        diagnostics = (diagnostics) + (range_parse.diagnostics)
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to lower `.for` iterable in `" + function.name + "`")
        return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
    ...
    if len(element_type) == 0:
        diagnostics = (diagnostics) + (range_parse.diagnostics)
        diagnostics = append_string(
            diagnostics, "llvm lowering: `.for` iterable must resolve to an array value in `" + function.name + "`")
        return BlockLoweringResult(lines=current_lines, allocas=current_allocas, locals=current_locals, bindings=current_bindings, temp_index=current_temp, block_counter=current_block_counter, diagnostics=diagnostics, terminated=False, next_local_id=current_next_local, lifetime_regions=lifetime_regions, next_lifetime_region_id=current_next_region, next_index=next_index, mutations=collected_mutations, string_constants=collected_string_constants)
    ...
    current_lines = append_string(current_lines, "  " + length_pointer_name + " = getelementptr " +
                                  array_struct_type + ", " + array_struct_type + "* " + iterable_operand.value + ", i32 0, i32 1")
    current_temp += 1
    length_load_name = format_temp_name(current_temp)
    current_lines = append_string(
        current_lines, "  " + length_load_name + " = load i64, i64* " + length_pointer_name)
    current_temp += 1
    length_operand = LLVMOperand(llvm_type="i64", value=length_load_name)
    data_pointer_name = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + data_pointer_name + " = getelementptr " +
                                  array_struct_type + ", " + array_struct_type + "* " + iterable_operand.value + ", i32 0, i32 0")
    current_temp += 1
    data_load_name = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + data_load_name + " = load " +
                                  data_pointer_type + ", " + data_pointer_pointer_type + " " + data_pointer_name)
    current_temp += 1
    data_operand = LLVMOperand(
        llvm_type=data_pointer_type, value=data_load_name)
    loop_header_alloc = allocate_block_label("for", current_block_counter)
    loop_header_label = loop_header_alloc.label
    current_block_counter = loop_header_alloc.next_counter
    ...
    current_next_local += 1
    current_allocas = append_string(
        current_allocas, "  " + index_pointer + " = alloca i64")
    current_lines = append_string(
        current_lines, "  store i64 0, i64* " + index_pointer)
    iteration_pointer = format_local_pointer_name(current_next_local)
    current_next_local += 1
    current_allocas = append_string(
        current_allocas, "  " + iteration_pointer + " = alloca " + element_type)
    current_lines = append_string(current_lines, "  store " + element_type + " " +
                                  default_return_literal(element_type) + ", " + element_type + "* " + iteration_pointer)
    current_lines = append_string(
        current_lines, "  br label %" + loop_header_label)
    current_lines = append_string(current_lines, loop_header_label + ":")
    header_index_name = format_temp_name(current_temp)
    current_lines = append_string(
        current_lines, "  " + header_index_name + " = load i64, i64* " + index_pointer)
    current_temp += 1
    comparison_name = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + comparison_name +
                                  " = icmp slt i64 " + header_index_name + ", " + length_operand.value)
    current_temp += 1
    current_lines = append_string(current_lines, "  br i1 " + comparison_name +
                                  ", label %" + loop_body_label + ", label %" + loop_exit_label)
    current_lines = append_string(current_lines, loop_body_label + ":")
    body_index_name = format_temp_name(current_temp)
    current_lines = append_string(
        current_lines, "  " + body_index_name + " = load i64, i64* " + index_pointer)
    current_temp += 1
    element_pointer_name = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + element_pointer_name + " = getelementptr " +
                                  element_type + ", " + element_type + "* " + data_operand.value + ", i64 " + body_index_name)
    current_temp += 1
    element_load_name = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + element_load_name +
                                  " = load " + element_type + ", " + element_type + "* " + element_pointer_name)
    current_temp += 1
    current_lines = append_string(current_lines, "  store " + element_type +
                                  " " + element_load_name + ", " + element_type + "* " + iteration_pointer)
    loop_context = LoopContext(
        break_label=loop_exit_label, continue_label=loop_increment_label)
    stacked = append_loop_context(loop_stack, loop_context)
    element_loop_scope_id = make_child_scope_id(scope_id, loop_body_label)
    iteration_binding = LocalBinding(name=raw_target, pointer=iteration_pointer, llvm_type=element_type, type_annotation="",
                                     ownership=None, consumed=False, scope_id=element_loop_scope_id, scope_depth=scope_depth + 1)
    body_locals = append_local_binding(current_locals, iteration_binding)
    body_result = lower_instruction_range(
        function,
        structure.body_start,
    ...
    current_lines = append_string(
        current_lines, "  " + increment_index_name + " = load i64, i64* " + index_pointer)
    current_temp += 1
    next_index_name = format_temp_name(current_temp)
    current_lines = append_string(
        current_lines, "  " + next_index_name + " = add i64 " + increment_index_name + ", 1")
    current_temp += 1
    current_lines = append_string(
        current_lines, "  store i64 " + next_index_name + ", i64* " + index_pointer)
    current_lines = append_string(
        current_lines, "  br label %" + loop_header_label)
    current_lines = append_string(current_lines, loop_exit_label + ":")
    mutated_names = collect_mutation_names(collected_mutations)
...
def collect_match_structure(instructions, start_index, end, function_name):
    ...
    while True:
        if index >= limit:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unterminated `.match` in `" + function_name + "`")
        ...
        if instruction.variant == "Case":
            ...
            current_case = MatchCaseStructure(pattern=instruction.pattern, guard=guard_text, body_start=index + 1,
                                              body_end=index + 1, is_default=is_default_pattern(instruction.pattern))
            index += 1
            continue
        index += 1
...
def lower_match_instruction(function, start_index, llvm_return, bindings, locals, allocas, lines, temp_index, block_counter, next_local_id, next_region_id, functions, loop_stack, end, context, scope_id, scope_depth, current_label):
    ...
    collected_mutations = []
    collected_string_constants = empty_string_constant_set()
    structure = collect_match_structure(
        function.instructions, start_index, end, function.name)
    diagnostics = (diagnostics) + (structure.diagnostics)
    subject_instruction = function.instructions[start_index]
    subject_result = lower_expression(subject_instruction.expression, bindings,
                                      current_locals, current_temp, current_lines, functions, context, "")
    ...
    collected_string_constants = merge_string_constants(
        collected_string_constants, subject_result.string_constants)
    if subject_result.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to lower match subject in `" + function.name + "`")
    ...
    if len(structure.cases) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: match without cases in `" + function.name + "`")
        ...
        current_lines = append_string(
            current_lines, "  br label %" + merge_label)
    ...
    while True:
        ...
        preload_index += 1
    current_lines = append_string(
        current_lines, "  br label %" + test_labels[0])
    all_terminated = True
    ...
    while True:
        ...
        collected_string_constants = merge_string_constants(
            collected_string_constants, lowered_condition.string_constants)
        if lowered_condition.is_default:
            has_unconditional_default = True
            current_lines = append_string(
                current_lines, "  br label %" + body_labels[index])
        else:
            if lowered_condition.operand != None:
                current_lines = append_string(current_lines, "  br i1 " + lowered_condition.operand.value +
                                              ", label %" + body_labels[index] + ", label %" + failure_target)
            else:
                current_lines = append_string(
                    current_lines, "  br label %" + failure_target)
        ...
        if len(lowered_condition.field_bindings) > 0 and lowered_condition.enum_info != None and lowered_condition.variant_info != None and subject_operand != None:
            ...
            current_temp += 1
            current_lines = append_string(
                current_lines, "  " + subject_alloca_temp + " = alloca " + subject_operand.llvm_type)
            current_lines = append_string(current_lines, "  store " + subject_operand.llvm_type + " " +
                                          subject_operand.value + ", " + subject_operand.llvm_type + "* " + subject_alloca_temp)
            binding_idx = 0
            while True:
                ...
                current_temp += 1
                current_lines = append_string(current_lines, "  " + payload_ptr_temp + " = getelementptr inbounds " +
                                              enum_info_val.llvm_name + ", " + enum_info_val.llvm_name + "* " + subject_alloca_temp + ", i32 0, i32 1")
                byte_ptr_temp = format_temp_name(current_temp)
                current_temp += 1
                current_lines = append_string(current_lines, "  " + byte_ptr_temp + " = bitcast [" + number_to_string(
                    variant_info_val.size) + " x i8]* " + payload_ptr_temp + " to i8*")
                ...
                if field_offset_in_payload > 0:
                    ...
                    current_temp += 1
                    current_lines = append_string(current_lines, "  " + offset_ptr_temp + " = getelementptr inbounds i8, i8* " +
                                                  byte_ptr_temp + ", i64 " + number_to_string(field_offset_in_payload))
                    field_ptr_temp = offset_ptr_temp
                typed_ptr_temp = format_temp_name(current_temp)
                current_temp += 1
                current_lines = append_string(current_lines, "  " + typed_ptr_temp +
                                              " = bitcast i8* " + field_ptr_temp + " to " + field_binding.field_type + "*")
                value_temp = format_temp_name(current_temp)
                current_temp += 1
                current_lines = append_string(current_lines, "  " + value_temp + " = load " +
                                              field_binding.field_type + ", " + field_binding.field_type + "* " + typed_ptr_temp)
                local_alloca_temp = format_temp_name(current_temp)
                current_temp += 1
                current_lines = append_string(
                    current_lines, "  " + local_alloca_temp + " = alloca " + field_binding.field_type)
                current_lines = append_string(current_lines, "  store " + field_binding.field_type +
                                              " " + value_temp + ", " + field_binding.field_type + "* " + local_alloca_temp)
                base_locals = (base_locals) + ([LocalBinding(name=field_binding.field_name, pointer=local_alloca_temp, llvm_type=field_binding.field_type,
                                                             type_annotation="", ownership=None, consumed=False, scope_id=make_child_scope_id(scope_id, body_labels[index]), scope_depth=scope_depth + 1)])
                base_local_id += 1
                binding_idx += 1
        body_result = lower_instruction_range(
            function,
        ...
        arm_mutations_list = append_match_arm_mutations(
            arm_mutations_list,
            MatchArmMutations(mutations=body_result.mutations,
                              label=body_labels[index], terminated=body_result.terminated)
        )
        if not body_result.terminated:
            merged_bindings = merge_parameter_bindings(
                merged_bindings, body_result.bindings)
            current_lines = append_string(
                current_lines, "  br label %" + merge_label)
...
def lower_match_case_condition(function_name, subject_operand, case, bindings, locals, temp_index, lines, functions, context):
    ...
    if not case.is_default:
        ...
        if enum_parse.recognized and enum_parse.success:
            enum_info = resolve_enum_info_for_literal(
                context, enum_parse.enum_name)
            if enum_info == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: match pattern references unknown enum `" + enum_parse.enum_name + "`")
            else:
                variant_info = resolve_enum_variant_info(
                    enum_info, enum_parse.variant_name)
                if variant_info == None:
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: enum `" + enum_parse.enum_name + "` has no variant `" + enum_parse.variant_name + "`")
                else:
                    tag_llvm_type = "i32"
                    if enum_info.tag_type == "i8":
                        tag_llvm_type = "i8"
                    ...
                    current_temp += 1
                    current_lines = append_string(
                        current_lines, "  " + tag_temp + " = extractvalue " + subject_operand.llvm_type + " " + subject_operand.value + ", 0")
                    tag_operand = LLVMOperand(
                        llvm_type=tag_llvm_type, value=tag_temp)
                    variant_tag_operand = LLVMOperand(
                        llvm_type=tag_llvm_type, value=number_to_string(variant_info.tag))
                    comparison = emit_comparison_instruction(
                        "==", tag_operand, variant_tag_operand, current_temp, current_lines)
                    ...
                    if len(variant_info.fields) > 0:
                        ...
                        while True:
                            ...
                            while True:
                                ...
                                variant_field_idx += 1
                            if found_field != None:
                                field_bindings = (field_bindings) + ([MatchFieldBinding(
                                    field_name=pattern_field.name, field_type=found_field.llvm_type, field_offset=found_field.offset)])
                            else:
                                diagnostics = append_string(diagnostics, "llvm lowering: match pattern field `" + pattern_field.name +
                                                            "` not found in variant `" + enum_parse.enum_name + "." + enum_parse.variant_name + "`")
                            field_idx += 1
                        matched_enum_info = enum_info
                        matched_variant_info = variant_info
        else:
            pattern_result = lower_expression(
                case.pattern, bindings, locals, current_temp, current_lines, functions, context, "")
            ...
            collected_string_constants = merge_string_constants(
                collected_string_constants, pattern_result.string_constants)
            if pattern_result.operand != None:
                harmonised = harmonise_operands(
                    subject_operand, pattern_result.operand, current_temp, current_lines)
                ...
                if harmonised.left != None and harmonised.right != None:
                    ...
                    comparison = emit_comparison_instruction(
                        "==", harmonised.left, harmonised.right, current_temp, current_lines)
                    diagnostics = (diagnostics) + (comparison.diagnostics)
                    current_lines = comparison.lines
                    current_temp = comparison.temp_index
                    condition_operand = comparison.operand
                ...
                diagnostics = append_string(
                    diagnostics, "llvm lowering: unable to lower match case pattern in `" + function_name + "`")
    if case.guard != None:
        guard_text = trim_text(case.guard)
        if len(guard_text) > 0:
            guard_condition = lower_condition_to_i1(
                function_name, guard_text, bindings, locals, current_temp, current_lines, functions, context)
            ...
            collected_string_constants = merge_string_constants(
                collected_string_constants, guard_condition.string_constants)
            if guard_condition.operand != None:
                if condition_operand == None:
                    condition_operand = guard_condition.operand
                else:
                    merged = emit_boolean_and(
                        condition_operand, guard_condition.operand, current_temp, current_lines)
                ...
                diagnostics = append_string(
                    diagnostics, "llvm lowering: unable to lower guard in match case for `" + function_name + "`")
...
def lower_condition_to_i1(function_name, expression, bindings, locals, temp_index, lines, functions, context):
    diagnostics = detect_suspension_conflicts(
        expression, locals, bindings, function_name, None)
    lowered = lower_expression(
        expression, bindings, locals, temp_index, lines, functions, context, "")
    diagnostics = (diagnostics) + (lowered.diagnostics)
    current_lines = lowered.lines
    current_temp = lowered.temp_index
    ...
    if lowered.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: condition produced no value in `" + function_name + "`")
    ...
    if operand.llvm_type == "double":
        ...
        current_lines = append_string(
            current_lines, "  " + temp_name + " = fcmp one double " + operand.value + ", 0.0")
    ...
    if operand.llvm_type == "i32" or operand.llvm_type == "i64":
        temp_name = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + temp_name + " = icmp ne " + operand.llvm_type +
                                      " " + operand.value + ", " + default_return_literal(operand.llvm_type) + "")
        converted = LLVMOperand(llvm_type="i1", value=temp_name)
        return ConditionConversion(lines=current_lines, temp_index=current_temp + 1, operand=converted, diagnostics=diagnostics, string_constants=string_constants)
    diagnostics = append_string(diagnostics, "llvm lowering: unsupported condition type `" +
                                operand.llvm_type + "` in `" + function_name + "`")
    return ConditionConversion(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=string_constants)
...
def build_phi_and_store(local, llvm_type, phi_inputs, temp_index):
    ...
    store_line = "  store " + llvm_type + " " + \
        phi_temp + ", " + llvm_type + "* " + local.pointer
...
def retarget_recent_mutations(mutations, start_index, target_label):
    ...
    while True:
        ...
        if index >= start_index:
            updated = LocalMutation(name=mutation.name, llvm_type=mutation.llvm_type,
                                    value_name=mutation.value_name, span=mutation.span, originating_label=target_label)
...
def materialize_mutation_values_at_exit(mutations, locals, lines, temp_index):
    ...
    while True:
        ...
        if local != None:
            ...
            updated = LocalMutation(name=mutation.name, llvm_type=mutation.llvm_type,
                                    value_name=load_temp, span=mutation.span, originating_label=mutation.originating_label)
...
def emit_phi_merges_for_straight_if(mutations, locals, preloaded_values, base_label, then_label, lines, temp_index):
    ...
    while True:
        ...
        local = find_local_binding(locals, mutation.name)
        if local != None:
            preloaded_value = find_preloaded_value(
                locals, preloaded_values, mutation.name)
            if preloaded_value != None and len(preloaded_value) > 0:
                ...
                phi_inputs = ["[ " + mutation.value_name + ", %" + then_source_label +
                              " ]", "[ " + preloaded_value + ", %" + base_label + " ]"]
                entry = build_phi_and_store(
                    local, mutation.llvm_type, phi_inputs, current_temp)
        ...
        if local != None:
                ...
                if then_mutation != None:
                    ...
                    if len(then_source_label) == 0:
                        if len(then_exit_label) > 0:
                            ...
                            then_source_label = then_label
                    phi_inputs = append_string(
                        phi_inputs, "[ " + then_mutation.value_name + ", %" + then_source_label + " ]")
                else:
                    if preloaded_value != None:
                        ...
                        phi_inputs = append_string(
                            phi_inputs, "[ " + preloaded_value + ", %" + fallback_label + " ]")
            if not else_terminated:
                if else_mutation != None:
                    ...
                    phi_inputs = append_string(
                        phi_inputs, "[ " + else_mutation.value_name + ", %" + else_source_label + " ]")
                else:
                    if preloaded_value != None:
            ...
            if len(phi_inputs) >= 2:
                entry = build_phi_and_store(
                    local, llvm_type, phi_inputs, current_temp)
                phi_lines = append_string(phi_lines, entry.phi_line)
                store_lines = append_string(store_lines, entry.store_line)
                current_temp += 1
        name_idx += 1
            ...
            while True:
                ...
                if not arm.terminated:
                    ...
                    if arm_mutation != None:
                        ...
                        phi_inputs = append_string(
                            phi_inputs, "[ " + arm_mutation.value_name + ", %" + arm.label + " ]")
                    else:
                        if preloaded_value != None:
                            phi_inputs = append_string(
                                phi_inputs, "[ " + preloaded_value + ", %" + arm.label + " ]")
                arm_scan_idx += 1
            if len(phi_inputs) >= 2:
                entry = build_phi_and_store(
                    local, llvm_type, phi_inputs, current_temp)
...
def lower_if_instruction(function, start_index, llvm_return, bindings, locals, allocas, lines, temp_index, block_counter, next_local_id, next_region_id, functions, loop_stack, end, context, scope_id, scope_depth, current_label):
    ...
    current_next_region = next_region_id
    collected_mutations = []
    structure = collect_if_structure(
        function.instructions, start_index, end, function.name)
    diagnostics = (diagnostics) + (structure.diagnostics)
    ...
    if condition.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to lower if condition in `" + function.name + "`")
    ...
    while True:
        ...
        preloaded_locals = append_string(preloaded_locals, preload_temp)
        preload_index += 1
    current_lines = append_string(current_lines, "  br i1 " + condition.operand.value +
                                  ", label %" + then_label + ", label %" + false_target)
    ...
    then_mutations = retarget_recent_mutations(
        then_result.mutations, 0, then_exit_label)
    if not then_result.terminated and len(then_mutations) > 0:
        materialized_then = materialize_mutation_values_at_exit(
            then_mutations, base_locals, current_lines, current_temp)
    ...
    collected_string_constants = merge_string_constants(
        collected_string_constants, then_result.string_constants)
    ...
    if structure.has_else:
        ...
        branch_mutations = retarget_recent_mutations(
            else_result.mutations, 0, exit_label)
        if not else_terminated and len(branch_mutations) > 0:
            materialized_else = materialize_mutation_values_at_exit(
                branch_mutations, base_locals, current_lines, current_temp)
        ...
        collected_string_constants = merge_string_constants(
            collected_string_constants, else_result.string_constants)
        if not else_terminated:
            current_lines = append_string(
                current_lines, "  br label %" + merge_label)
    terminated = False
    ...
    if not then_terminated:
        merged_bindings = merge_parameter_bindings(
            merged_bindings, then_bindings)
    if structure.has_else:
        if not else_terminated:
            merged_bindings = merge_parameter_bindings(
                merged_bindings, else_bindings)
...
def lower_let_instruction(function, instruction, bindings, locals, allocas, lines, temp_index, next_local_id, next_region_id, functions, context, scope_id, scope_depth, current_label):
    ...
    suspension_diagnostics = detect_suspension_conflicts(
        instruction.value, current_locals, current_bindings, function.name, initializer_span)
    ...
    if len(trimmed_annotation) > 0:
        ...
        if len(llvm_type) == 0:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unsupported local type for `" + instruction.name + "` in `" + function.name + "`")
    operand = None
    ownership_analysis = analyze_value_ownership(
        instruction.value, initializer_span, current_locals, current_bindings)
    ...
    if ownership != None:
        conflict_diagnostics = detect_borrow_conflicts(
            ownership, current_locals, instruction.name, function.name)
        diagnostics = (diagnostics) + (conflict_diagnostics)
        if ownership.variant == "Borrow":
            base_scope = infer_borrow_base_scope(
                ownership.base, current_locals, current_bindings, function.name)
            region_id = current_next_region
            current_next_region += 1
            ownership = OwnershipInfo(variant=ownership.variant, base=ownership.base,
                                      mutable=ownership.mutable, span=ownership.span, region_id=region_id)
            region = make_lifetime_region_metadata(
                region_id, instruction.name, ownership, scope_id, scope_depth, base_scope.scope_id, base_scope.scope_depth)
            lifetime_regions = append_lifetime_region(lifetime_regions, region)
        else:
            ownership = OwnershipInfo(variant=ownership.variant, base=ownership.base,
                                      mutable=ownership.mutable, span=ownership.span, region_id=-1)
    if consumption != None:
        if consumption.kind == "local":
            current_locals = mark_local_consumed(
                current_locals, consumption.name)
        else:
            if consumption.kind == "parameter":
                current_bindings = mark_parameter_consumed(
                    current_bindings, consumption.name)
    string_constants = empty_string_constant_set()
    if instruction.value == None or len(instruction.value) == 0:
        diagnostics = append_string(diagnostics, "llvm lowering: let `" +
                                    instruction.name + "` missing initializer in `" + function.name + "`")
    else:
        lowered = lower_expression(instruction.value, current_bindings, current_locals,
                                   current_temp, current_lines, functions, context, llvm_type)
        diagnostics = (diagnostics) + (lowered.diagnostics)
        current_lines = lowered.lines
        current_temp = lowered.temp_index
        operand = lowered.operand
    ...
    if len(llvm_type) == 0:
        llvm_type = "double"
    interface_info = find_interface_info_by_name(
        context, instruction.type_annotation)
    if interface_info != None and operand != None:
        struct_info = find_struct_info_by_llvm_type(context, operand.llvm_type)
        if struct_info != None:
            vtable = find_vtable_for_struct_interface(
                context, struct_info.name, interface_info.name)
            if vtable != None:
                ...
                current_temp += 1
                current_lines = append_string(
                    current_lines, "  " + struct_ptr_temp + " = alloca " + struct_info.llvm_name)
                current_lines = append_string(current_lines, "  store " + operand.llvm_type +
                                              " " + operand.value + ", " + operand.llvm_type + "* " + struct_ptr_temp)
                current_lines = append_string(
                    current_lines, "  " + data_ptr_temp + " = bitcast " + struct_info.llvm_name + "* " + struct_ptr_temp + " to i8*")
                current_lines = append_string(current_lines, "  " + vtable_ptr_temp + " = bitcast " +
                                              vtable.llvm_type_name + "* " + vtable.llvm_global_name + " to i8*")
                current_lines = append_string(current_lines, "  " + with_data_temp + " = insertvalue " +
                                              interface_info.llvm_name + " undef, i8* " + data_ptr_temp + ", 0")
                current_lines = append_string(current_lines, "  " + with_vtable_temp + " = insertvalue " +
                                              interface_info.llvm_name + " " + with_data_temp + ", i8* " + vtable_ptr_temp + ", 1")
                operand = LLVMOperand(
                    llvm_type=interface_info.llvm_name, value=with_vtable_temp)
                llvm_type = interface_info.llvm_name
            else:
                diagnostics = append_string(diagnostics, "llvm lowering: struct `" + struct_info.name +
                                            "` does not implement interface `" + interface_info.name + "`")
    pointer = format_local_pointer_name(next_local_id)
    current_allocas = append_string(
        current_allocas, "  " + pointer + " = alloca " + llvm_type)
    stored_value = default_return_literal(llvm_type)
    if operand != None:
        coerced = coerce_operand_to_type(
            operand, llvm_type, current_temp, current_lines)
        diagnostics = (diagnostics) + (coerced.diagnostics)
        current_lines = coerced.lines
        current_temp = coerced.temp_index
        if coerced.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to coerce initializer for `" + instruction.name + "` to `" + llvm_type + "`")
            current_lines = append_string(current_lines, "  store " + llvm_type + " " +
                                          default_return_literal(llvm_type) + ", " + llvm_type + "* " + pointer)
        else:
            stored = coerced.operand
            stored_value = stored.value
            current_lines = append_string(
                current_lines, "  store " + llvm_type + " " + stored.value + ", " + llvm_type + "* " + pointer)
    else:
        current_lines = append_string(current_lines, "  store " + llvm_type + " " +
                                      default_return_literal(llvm_type) + ", " + llvm_type + "* " + pointer)
    current_locals = append_local_binding(current_locals, LocalBinding(name=instruction.name, pointer=pointer, llvm_type=llvm_type,
                                          type_annotation=instruction.type_annotation, ownership=ownership, consumed=False, scope_id=scope_id, scope_depth=scope_depth))
    mutation = LocalMutation(name=instruction.name, llvm_type=llvm_type,
                             value_name=stored_value, span=initializer_span, originating_label=current_label)
...
def lower_expression_statement(function_name, instruction, expression, bindings, locals, temp_index, lines, next_region_id, scope_id, scope_depth, functions, context, current_label):
    ...
    diagnostics = detect_suspension_conflicts(
        expression, locals, bindings, function_name, suspension_span)
    current_lines = lines
    current_temp = temp_index
    current_locals = locals
    current_bindings = bindings
    ...
    if parsed_assignment.success:
        ...
        if member_parse.success:
            ...
            if len(parsed_assignment.operator) > 0:
                effective_value = parsed_assignment.target + " " + \
                    parsed_assignment.operator + " " + parsed_assignment.value
            lowered = lower_expression(effective_value, current_bindings,
                                       current_locals, current_temp, current_lines, functions, context, "")
            diagnostics = (diagnostics) + (lowered.diagnostics)
            string_constants = lowered.string_constants
            current_lines = lowered.lines
            ...
            if lowered.operand == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: failed to lower assignment value for member `" + parsed_assignment.target + "`")
            else:
                base_result = lower_expression(
                    member_parse.base, current_bindings, current_locals, current_temp, current_lines, functions, context, "")
                ...
                if base_result.operand == None:
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: member access assignment base `" + member_parse.base + "` produced no value")
                else:
                    struct_info = find_struct_info_by_llvm_type(
                        context, base_result.operand.llvm_type)
                    if struct_info == None:
                        diagnostics = append_string(
                            diagnostics, "llvm lowering: member access assignment on non-struct type `" + base_result.operand.llvm_type + "`")
                    else:
                        field_index = find_struct_field_index(
                            struct_info, member_parse.field)
                        if field_index < 0:
                            diagnostics = append_string(
                                diagnostics, "llvm lowering: struct `" + struct_info.name + "` has no field `" + member_parse.field + "`")
                        else:
                            field_info = struct_info.fields[field_index]
                            coerced = coerce_operand_to_type(
                                lowered.operand, field_info.llvm_type, current_temp, current_lines)
                            diagnostics = (diagnostics) + (coerced.diagnostics)
                            current_lines = coerced.lines
                            current_temp = coerced.temp_index
                            if coerced.operand != None:
                                ...
                                current_temp += 1
                                current_lines = append_string(current_lines, "  " + field_ptr_temp + " = getelementptr " + struct_info.llvm_name + ", " +
                                                              base_result.operand.llvm_type + " " + base_result.operand.value + ", i32 0, i32 " + number_to_string(field_index))
                                current_lines = append_string(current_lines, "  store " + coerced.operand.llvm_type +
                                                              " " + coerced.operand.value + ", " + coerced.operand.llvm_type + "* " + field_ptr_temp)
        ...
        if index_parse.success:
            ...
            if lowered.operand == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: failed to lower assignment value for array index `" + parsed_assignment.target + "`")
            else:
                base_result = lower_expression(
                    index_parse.base, current_bindings, current_locals, current_temp, current_lines, functions, context, "")
                diagnostics = (diagnostics) + (base_result.diagnostics)
                current_lines = base_result.lines
                current_temp = base_result.temp_index
                if base_result.operand == None:
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: array index assignment base `" + index_parse.base + "` produced no value")
                else:
                    index_result = lower_expression(
                        index_parse.index, current_bindings, current_locals, current_temp, current_lines, functions, context, "")
                    ...
                    if index_result.operand == None:
                        diagnostics = append_string(
                            diagnostics, "llvm lowering: array index `" + index_parse.index + "` produced no value")
                    else:
                        element_type = array_pointer_element_type(
                            base_result.operand.llvm_type)
                        if len(element_type) == 0:
                            diagnostics = append_string(
                                diagnostics, "llvm lowering: cannot determine element type for array assignment `" + base_result.operand.llvm_type + "`")
                        else:
                            coerced = coerce_operand_to_type(
                                lowered.operand, element_type, current_temp, current_lines)
                            diagnostics = (diagnostics) + (coerced.diagnostics)
                            current_lines = coerced.lines
                            current_temp = coerced.temp_index
                            if coerced.operand != None:
                                ...
                                current_temp += 1
                                current_lines = append_string(current_lines, "  " + data_ptr_temp + " = getelementptr " + array_struct_type_for_element(
                                    element_type) + ", " + base_result.operand.llvm_type + " " + base_result.operand.value + ", i32 0, i32 0")
                                data_ptr_loaded = format_temp_name(
                                    current_temp)
                                current_temp += 1
                                current_lines = append_string(
                                    current_lines, "  " + data_ptr_loaded + " = load " + element_type + "*, " + element_type + "** " + data_ptr_temp)
                                current_lines = append_string(current_lines, "  " + elem_ptr_temp + " = getelementptr " + element_type +
                                                              ", " + element_type + "* " + data_ptr_loaded + ", i64 " + index_result.operand.value)
                                current_lines = append_string(current_lines, "  store " + coerced.operand.llvm_type +
                                                              " " + coerced.operand.value + ", " + coerced.operand.llvm_type + "* " + elem_ptr_temp)
            return ExpressionStatementResult(lines=current_lines, temp_index=current_temp, locals=current_locals, bindings=current_bindings, diagnostics=diagnostics, lifetime_regions=lifetime_regions, lifetime_releases=lifetime_releases, next_region_id=current_next_region, mutations=mutations, string_constants=string_constants)
        binding = find_local_binding(current_locals, parsed_assignment.target)
        if binding == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: assignment to unknown local `" + parsed_assignment.target + "`")
        else:
            effective_value = parsed_assignment.value
            if len(parsed_assignment.operator) > 0:
                effective_value = parsed_assignment.target + " " + \
            ...
            ownership_analysis = analyze_value_ownership(
                effective_value, instruction.span, current_locals, current_bindings)
            diagnostics = (diagnostics) + (ownership_analysis.diagnostics)
            assignment_ownership = ownership_analysis.ownership
            consumption = ownership_analysis.consumption
            stored_value = default_return_literal(binding.llvm_type)
            lowered = lower_expression(effective_value, current_bindings, current_locals,
                                       current_temp, current_lines, functions, context, binding.llvm_type)
            diagnostics = (diagnostics) + (lowered.diagnostics)
            string_constants = lowered.string_constants
            current_lines = lowered.lines
            current_temp = lowered.temp_index
            if lowered.operand == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: failed to lower assignment expression for `" + parsed_assignment.target + "`")
            else:
                coerced = coerce_operand_to_type(
                    lowered.operand, binding.llvm_type, current_temp, current_lines)
                diagnostics = (diagnostics) + (coerced.diagnostics)
                current_lines = coerced.lines
                current_temp = coerced.temp_index
                if coerced.operand == None:
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: unable to coerce assignment value for `" + parsed_assignment.target + "`")
                    current_lines = append_string(current_lines, "  store " + binding.llvm_type + " " + default_return_literal(
                        binding.llvm_type) + ", " + binding.llvm_type + "* " + binding.pointer)
                else:
                    operand = coerced.operand
                    stored_value = operand.value
                    current_lines = append_string(current_lines, "  store " + binding.llvm_type +
                                                  " " + operand.value + ", " + binding.llvm_type + "* " + binding.pointer)
            if consumption != None:
                if consumption.kind == "local":
                    current_locals = mark_local_consumed(
                        current_locals, consumption.name)
            ...
            if previous_ownership != None:
                if previous_ownership.variant == "Borrow":
                    if previous_ownership.region_id >= 0:
                        release_event = LifetimeReleaseEvent(
                            region_id=previous_ownership.region_id, scope_id=scope_id, scope_depth=scope_depth)
                        lifetime_releases = append_lifetime_release_event(
                            lifetime_releases, release_event)
            if assignment_ownership != None:
                if assignment_ownership.variant == "Borrow":
                    region_id = current_next_region
                    current_next_region += 1
                    assignment_ownership = OwnershipInfo(variant=assignment_ownership.variant, base=assignment_ownership.base,
                                                         mutable=assignment_ownership.mutable, span=assignment_ownership.span, region_id=region_id)
                else:
                    assignment_ownership = OwnershipInfo(variant=assignment_ownership.variant, base=assignment_ownership.base,
                                                         mutable=assignment_ownership.mutable, span=assignment_ownership.span, region_id=-1)
            current_locals = reset_local_consumption(
                current_locals, parsed_assignment.target)
            current_locals = update_local_ownership(
                current_locals, parsed_assignment.target, assignment_ownership)
            if assignment_ownership != None:
                if assignment_ownership.variant == "Borrow":
                    base_scope = infer_borrow_base_scope(
                        assignment_ownership.base, current_locals, current_bindings, function_name)
                    region = make_lifetime_region_metadata(assignment_ownership.region_id, parsed_assignment.target,
                                                           assignment_ownership, binding.scope_id, binding.scope_depth, base_scope.scope_id, base_scope.scope_depth)
                    lifetime_regions = append_lifetime_region(
                        lifetime_regions, region)
            mutation = LocalMutation(name=parsed_assignment.target, llvm_type=binding.llvm_type,
                                     value_name=stored_value, span=instruction.span, originating_label=current_label)
            mutations = (mutations) + ([mutation])
        return ExpressionStatementResult(lines=current_lines, temp_index=current_temp, locals=current_locals, bindings=current_bindings, diagnostics=diagnostics, lifetime_regions=lifetime_regions, lifetime_releases=lifetime_releases, next_region_id=current_next_region, mutations=mutations, string_constants=string_constants)
    ownership_analysis = analyze_value_ownership(
        expression, instruction.span, current_locals, current_bindings)
    diagnostics = (diagnostics) + (ownership_analysis.diagnostics)
    consumption = ownership_analysis.consumption
    lowered = lower_expression(expression, current_bindings, current_locals,
                               current_temp, current_lines, functions, context, "")
    diagnostics = (diagnostics) + (lowered.diagnostics)
    string_constants = lowered.string_constants
    current_lines = lowered.lines
    current_temp = lowered.temp_index
...
def parse_inline_let_expression(expression):
    ...
    if len(trimmed) < 4:
        diagnostics = append_string(
            diagnostics, "llvm lowering: malformed let expression `" + expression + "`")
    ...
    if len(remainder) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: let expression missing binding name in `" + expression + "`")
    ...
    if len(tail) > 0:
        if len(tail) >= 2:
            ...
            if annotation_prefix == "->":
                ...
                if assign_index >= 0:
                    type_annotation = trim_text(
                        substring(tail, 0, assign_index))
                    value_text = trim_text(
                        substring(tail, assign_index + 1, len(tail)))
                    if len(value_text) > 0:
    ...
    if initializer == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: let expression missing initializer for `" + name + "`")
...
def lower_return_instruction(function, instruction, llvm_return, bindings, locals, temp_index, lines, next_region_id, scope_id, scope_depth, functions, context):
    ...
    if instruction.expression == None or len(instruction.expression) == 0:
        if llvm_return == "void":
            ...
            diagnostics = append_string(
                diagnostics, "llvm lowering: missing return value in `" + function.name + "`")
            current_lines = append_string(
                current_lines, "  ret " + llvm_return + " " + default_return_literal(llvm_return))
        empty_constants = empty_string_constant_set()
    ...
    current_bindings = bindings
    suspension_diagnostics = detect_suspension_conflicts(
        instruction.expression, current_locals, current_bindings, function.name, instruction.span)
    diagnostics = (diagnostics) + (suspension_diagnostics)
    ownership_analysis = analyze_value_ownership(
        instruction.expression, instruction.span, current_locals, current_bindings)
    diagnostics = (diagnostics) + (ownership_analysis.diagnostics)
    consumption = ownership_analysis.consumption
    lowered = lower_expression(instruction.expression, current_bindings, current_locals,
                               current_temp, current_lines, functions, context, llvm_return)
    diagnostics = (diagnostics) + (lowered.diagnostics)
    current_lines = lowered.lines
    current_temp = lowered.temp_index
    string_constants = lowered.string_constants
    if lowered.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unhandled return expression in `" + function.name + "`")
        current_lines = append_string(
            current_lines, "  ret " + llvm_return + " " + default_return_literal(llvm_return))
    ...
    if llvm_return == "void":
        diagnostics = append_string(
            diagnostics, "llvm lowering: void function `" + function.name + "` returned a value")
        current_lines = append_string(current_lines, "  ret void")
        return ExpressionStatementResult(lines=current_lines, temp_index=current_temp, locals=current_locals, bindings=current_bindings, diagnostics=diagnostics, lifetime_regions=lifetime_regions, lifetime_releases=lifetime_releases, next_region_id=current_next_region, mutations=[], string_constants=string_constants)
    coerced = coerce_operand_to_type(
        operand, llvm_return, current_temp, current_lines)
    diagnostics = (diagnostics) + (coerced.diagnostics)
    current_lines = coerced.lines
    current_temp = coerced.temp_index
    if coerced.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to coerce return expression to `" + llvm_return + "` in `" + function.name + "`")
        current_lines = append_string(
            current_lines, "  ret " + llvm_return + " " + default_return_literal(llvm_return))
        return ExpressionStatementResult(lines=current_lines, temp_index=current_temp, locals=current_locals, bindings=current_bindings, diagnostics=diagnostics, lifetime_regions=lifetime_regions, lifetime_releases=lifetime_releases, next_region_id=current_next_region, mutations=[], string_constants=string_constants)
    coerced_operand = coerced.operand
    current_lines = append_string(
        current_lines, "  ret " + coerced_operand.llvm_type + " " + coerced_operand.value)
    if consumption != None:
        if consumption.kind == "local":
            current_locals = mark_local_consumed(
                current_locals, consumption.name)
...
def prepare_parameters(function, context):
    ...
    while True:
        ...
        if len(parameter.type_annotation) == 0:
            diagnostics = append_string(diagnostics, "llvm lowering: parameter `" +
                                        parameter.name + "` missing type annotation; defaulting to `" + llvm_type + "`")
        sanitized = sanitize_symbol(parameter.name)
        llvm_name = "%" + sanitized
        signature = append_string(signature, llvm_type + " " + llvm_name)
        bindings = append_parameter_binding(bindings, ParameterBinding(name=parameter.name, llvm_name=llvm_name,
                                            llvm_type=llvm_type, type_annotation=parameter.type_annotation, consumed=False, span=parameter.span))
        index += 1
    ...
    while True:
        ...
        if not local.consumed and local.ownership != None:
            ...
            if details.variant == "Borrow" and details.mutable:
                ...
                location = format_suspension_location(
                    keyword, details.span, suspension_span)
                diagnostics = append_string(diagnostics, "llvm lowering: " + keyword + " suspends while mutable borrow `" +
                                            local.name + "` of `" + base_name + "` remains active in `" + function_name + "`" + location)
        index += 1
    parameter_index = 0
    while True:
        ...
        if not binding.consumed:
            if is_mutable_borrow_annotation(binding.type_annotation):
                location = format_suspension_location(
                    keyword, binding.span, suspension_span)
                diagnostics = append_string(diagnostics, "llvm lowering: " + keyword + " suspends while mutable borrow parameter `" +
                                            binding.name + "` remains active in `" + function_name + "`" + location)
        parameter_index += 1
    return diagnostics
...
def mark_parameter_consumed(bindings, name):
    ...
    while True:
        ...
        binding = bindings[index]
        if binding.name == name:
            updated = ParameterBinding(name=binding.name, llvm_name=binding.llvm_name, llvm_type=binding.llvm_type,
                                       type_annotation=binding.type_annotation, consumed=True, span=binding.span)
            result = (result) + ([updated])
        else:
...
def mark_local_consumed(locals, name):
    ...
    while True:
        ...
        entry = locals[index]
        if entry.name == name:
            updated = LocalBinding(name=entry.name, pointer=entry.pointer, llvm_type=entry.llvm_type, type_annotation=entry.type_annotation,
                                   ownership=entry.ownership, consumed=True, scope_id=entry.scope_id, scope_depth=entry.scope_depth)
            result = (result) + ([updated])
        else:
            result = (result) + ([entry])
        index += 1
...
def reset_local_consumption(locals, name):
    ...
    while True:
        ...
        if entry.name == name:
            updated = LocalBinding(name=entry.name, pointer=entry.pointer, llvm_type=entry.llvm_type, type_annotation=entry.type_annotation,
                                   ownership=entry.ownership, consumed=False, scope_id=entry.scope_id, scope_depth=entry.scope_depth)
            result = (result) + ([updated])
        else:
            result = (result) + ([entry])
        index += 1
...
def update_local_ownership(locals, name, ownership):
    ...
    while True:
        ...
        if entry.name == name:
            updated = LocalBinding(name=entry.name, pointer=entry.pointer, llvm_type=entry.llvm_type, type_annotation=entry.type_annotation,
                                   ownership=ownership, consumed=entry.consumed, scope_id=entry.scope_id, scope_depth=entry.scope_depth)
            result = (result) + ([updated])
        else:
            result = (result) + ([entry])
        index += 1
...
def lower_logical_not_with_operand(operand, temp_index, lines, diagnostics, string_constants):
    ...
    if coerced.operand != None:
        ...
        current_lines = append_string(
            current_lines, "  " + result_temp + " = xor i1 " + bool_operand.value + ", 1")
        current_temp += 1
...
def lower_expression(expression, bindings, locals, temp_index, lines, functions, context, expected_type):
    ...
    if len(trimmed) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: empty expression encountered")
    ...
    if len(stripped) > 0:
        ...
        if first_ch == "!":
            ...
            if len(operand_text) > 0:
                lowered_operand = lower_expression(
                    operand_text, bindings, locals, temp_index, lines, functions, context, "")
    ...
    if enum_parse.recognized and enum_parse.success:
        enum_info = resolve_enum_info_for_literal(
            context, enum_parse.enum_name)
        if enum_info != None:
            lowered_enum = lower_enum_literal(
                enum_parse, bindings, locals, temp_index, lines, functions, context, expected_type)
    ...
    if struct_parse.recognized:
        if not struct_parse.success:
            ...
            return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
        lowered_struct = lower_struct_literal(
            struct_parse, bindings, locals, temp_index, lines, functions, context, expected_type)
    ...
    if parameter != None:
        operand = LLVMOperand(llvm_type=parameter.llvm_type,
    ...
    if literal_candidate == "fs":
        ...
        field_result = lower_dynamic_member_access(
            "fs", runtime_result.operand, runtime_result.temp_index, runtime_result.lines, diagnostics, runtime_result.string_constants)
    ...
    if is_string_literal(literal_candidate):
        if is_character_literal(literal_candidate):
            ...
            operand = LLVMOperand(
                llvm_type="i8", value=number_to_string(char_value))
            return ExpressionResult(lines=lines, temp_index=temp_index, operand=operand, diagnostics=diagnostics, string_constants=empty_constants)
    ...
    if is_number_literal(literal_candidate):
        ...
        return ExpressionResult(lines=lines, temp_index=temp_index, operand=operand, diagnostics=diagnostics, string_constants=empty_constants)
    diagnostics = append_string(
        diagnostics, "llvm lowering: unsupported expression `" + literal_candidate + "`")
    return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
...
def parse_borrow_expression(text):
    ...
    if starts_with(trimmed, "borrow"):
        ...
        if len(remainder) == 0:
            issue = append_string(
                diagnostics, "llvm lowering: borrow expression missing target")
        ...
        if len(remainder) > 0 and remainder[0] == "(":
            ...
            if len(argument_text) == 0:
                messages = append_string(
                    messages, "llvm lowering: borrow expression missing target")
                return BorrowParseResult(recognized=True, success=False, target="", mutable=mutable_flag, diagnostics=messages)
...
def extract_borrow_argument(text):
    ...
    while True:
        ...
        if ch == ")":
            if depth == 0:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: borrow expression missing opening `(`")
    ...
    if closing_index < 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: borrow expression missing closing `)`")
        return BorrowArgumentParse(success=False, argument="", diagnostics=diagnostics)
    inner = substring(text, 1, closing_index)
    remainder = trim_text(substring(text, closing_index + 1, len(text)))
    if len(remainder) > 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: borrow expression trailing content `" + remainder + "`")
        return BorrowArgumentParse(success=False, argument="", diagnostics=diagnostics)
...
def parse_ternary_expression(text):
    ...
    if question_index >= colon_index:
        diagnostics = append_string(
            diagnostics, "llvm lowering: malformed ternary expression - `:` before `?`")
        return TernaryParseResult(success=False, condition="", true_value="", false_value="", diagnostics=diagnostics)
    ...
    if len(condition) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: ternary expression missing condition")
        return TernaryParseResult(success=False, condition="", true_value="", false_value="", diagnostics=diagnostics)
    if len(true_value) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: ternary expression missing true branch value")
        return TernaryParseResult(success=False, condition="", true_value="", false_value="", diagnostics=diagnostics)
    if len(false_value) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: ternary expression missing false branch value")
        return TernaryParseResult(success=False, condition="", true_value="", false_value="", diagnostics=diagnostics)
...
def analyze_value_ownership(initializer, span, locals, bindings):
    ...
    if parse.recognized:
        ...
        ownership = OwnershipInfo(
            variant="Borrow", base=resolved_base, mutable=parse.mutable, span=span, region_id=-1)
    ...
    if is_simple_identifier(trimmed):
        ...
        if local != None:
            if local.consumed:
                diagnostics = append_string(
                    diagnostics, format_use_after_move_message(name, span))
                return OwnershipAnalysis(ownership=None, consumption=None, diagnostics=diagnostics)
...
def format_span_location(span):
    start = number_to_string(span.start_line) + ":" + \
        number_to_string(span.start_column)
    end = number_to_string(span.end_line) + ":" + \
            ...
            if details.variant == "Borrow":
                ...
                if existing_base == base:
                    if ownership.mutable:
                        if details.mutable:
                            diagnostics = append_string(diagnostics, "llvm lowering: mutable borrow `" + binding_name +
                                                        "` conflicts with active mutable borrow `" + existing.name + "` of `" + base + "` in `" + function_name + "`")
                        else:
                            diagnostics = append_string(diagnostics, "llvm lowering: mutable borrow `" + binding_name +
                                                        "` conflicts with active shared borrow `" + existing.name + "` of `" + base + "` in `" + function_name + "`")
                    else:
                        if details.mutable:
                            diagnostics = append_string(diagnostics, "llvm lowering: shared borrow `" + binding_name +
                                                        "` conflicts with active mutable borrow `" + existing.name + "` of `" + base + "` in `" + function_name + "`")
        index += 1
    return diagnostics
...
def lower_ternary_expression(parse, bindings, locals, temp_index, lines, functions, context, expected_type):
    ...
    cond_result = lower_expression(
        parse.condition, bindings, locals, label_temp_offset, lines, functions, context, "")
    ...
    cond_bool = coerce_operand_to_type(
        cond_result.operand, "i1", cond_result.temp_index, cond_result.lines)
    ...
    current_lines = append_string(current_lines, "  br i1 " + cond_bool.operand.value +
                                  ", label %" + then_label + ", label %" + else_label)
    current_lines = append_string(current_lines, "")
    current_lines = append_string(current_lines, then_label + ":")
    then_result = lower_expression(parse.true_value, bindings, locals,
                                   cond_bool.temp_index, current_lines, functions, context, expected_type)
    diagnostics = (diagnostics) + (then_result.diagnostics)
    string_constants = merge_string_constants(
        string_constants, then_result.string_constants)
    ...
    current_lines = append_string(
        current_lines, "  br label %" + then_end_label)
    current_lines = append_string(current_lines, "")
    current_lines = append_string(current_lines, then_end_label + ":")
    current_lines = append_string(current_lines, "  br label %" + merge_label)
    current_lines = append_string(current_lines, "")
    current_lines = append_string(current_lines, else_label + ":")
    else_result = lower_expression(parse.false_value, bindings, locals,
                                   then_result.temp_index, current_lines, functions, context, expected_type)
    diagnostics = (diagnostics) + (else_result.diagnostics)
    string_constants = merge_string_constants(
        string_constants, else_result.string_constants)
    ...
    current_lines = append_string(
        current_lines, "  br label %" + else_end_label)
    current_lines = append_string(current_lines, "")
    ...
    if then_result.operand.llvm_type != else_result.operand.llvm_type:
        diagnostics = append_string(diagnostics, "llvm lowering: ternary expression branches have incompatible types: `" +
                                    then_result.operand.llvm_type + "` vs `" + else_result.operand.llvm_type + "`")
    ...
    current_lines = append_string(current_lines, "  " + result_temp + " = phi " + result_type +
                                  " [ " + then_result.operand.value + ", %" + then_end_label + " ], [ " + else_result.operand.value + ", %" + else_end_label + " ]")
...
def lower_binary_operation(expression, match, bindings, locals, temp_index, lines, functions, context):
    ...
    right_text = trim_text(
        substring(expression, match.index + 1, len(expression)))
    diagnostics = []
    if len(left_text) == 0 or len(right_text) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: malformed binary expression `" + expression + "`")
        empty_constants = empty_string_constant_set()
        return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
    left_result = lower_expression(
        left_text, bindings, locals, temp_index, lines, functions, context, "")
    ...
    right_result = lower_expression(
        right_text, bindings, locals, left_result.temp_index, left_result.lines, functions, context, "")
    diagnostics = (diagnostics) + (right_result.diagnostics)
    string_constants = merge_string_constants(
        string_constants, right_result.string_constants)
    ...
    harmonised = harmonise_operands(
        left_result.operand, right_result.operand, right_result.temp_index, right_result.lines)
    diagnostics = (diagnostics) + (harmonised.diagnostics)
    ...
    if match.symbol == "+" and harmonised.result_type == "i8*":
        ...
        line = "  " + temp_name + \
            ...
            left_aligned.value + ", i8* " + right_aligned.value + ")"
    ...
    temp_name = format_temp_name(harmonised.temp_index)
    line = "  " + temp_name + " = " + operation + " " + harmonised.result_type + \
        " " + left_aligned.value + ", " + right_aligned.value
    updated_lines = append_string(harmonised.lines, line)
...
def lower_comparison_operation(expression, match, bindings, locals, temp_index, lines, functions, context):
    ...
    left_text = trim_text(substring(expression, 0, match.index))
    right_text = trim_text(
        substring(expression, match.index + operator_length, len(expression)))
    diagnostics = []
    if len(left_text) == 0 or len(right_text) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: malformed comparison expression `" + expression + "`")
        empty_constants = empty_string_constant_set()
        return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
    left_result = lower_expression(
        left_text, bindings, locals, temp_index, lines, functions, context, "")
    ...
    if harmonised.left == None or harmonised.right == None:
        return ExpressionResult(lines=harmonised.lines, temp_index=harmonised.temp_index, operand=None, diagnostics=diagnostics, string_constants=string_constants)
    comparison = emit_comparison_instruction(
        match.symbol, harmonised.left, harmonised.right, harmonised.temp_index, harmonised.lines)
    diagnostics = (diagnostics) + (comparison.diagnostics)
...
def lower_logical_and(expression, match, bindings, locals, temp_index, lines, functions, context):
    left_text = trim_text(substring(expression, 0, match.index))
    right_text = trim_text(
        substring(expression, match.index + 2, len(expression)))
    diagnostics = []
    string_constants = empty_string_constant_set()
    if len(left_text) == 0 or len(right_text) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: malformed && expression `" + expression + "`")
    ...
    label_temp_offset = temp_index + 1
    left_result = lower_expression(
        left_text, bindings, locals, label_temp_offset, lines, functions, context, "")
    diagnostics = (diagnostics) + (left_result.diagnostics)
    string_constants = left_result.string_constants
    if left_result.operand == None:
        return ExpressionResult(lines=left_result.lines, temp_index=left_result.temp_index, operand=None, diagnostics=diagnostics, string_constants=string_constants)
    left_bool = coerce_operand_to_type(
        left_result.operand, "i1", left_result.temp_index, left_result.lines)
    ...
    current_lines = append_string(current_lines, "  br i1 " + left_bool.operand.value +
                                  ", label %" + check_right_label + ", label %" + merge_label)
    current_lines = append_string(current_lines, "")
    current_lines = append_string(current_lines, check_right_label + ":")
    right_result = lower_expression(
        right_text, bindings, locals, left_bool.temp_index, current_lines, functions, context, "")
    diagnostics = (diagnostics) + (right_result.diagnostics)
    string_constants = merge_string_constants(
        string_constants, right_result.string_constants)
    if right_result.operand == None:
        return ExpressionResult(lines=right_result.lines, temp_index=right_result.temp_index, operand=None, diagnostics=diagnostics, string_constants=string_constants)
    right_bool = coerce_operand_to_type(
        right_result.operand, "i1", right_result.temp_index, right_result.lines)
    ...
    current_lines = append_string(
        current_lines, "  br label %" + right_end_label)
    current_lines = append_string(current_lines, "")
    ...
    current_lines = append_string(current_lines, "  " + result_temp +
                                  " = phi i1 [ false, %" + entry_label + " ], [ " + right_bool.operand.value + ", %" + right_end_label + " ]")
...
def lower_logical_or(expression, match, bindings, locals, temp_index, lines, functions, context):
    ...
    current_lines = append_string(current_lines, entry_label + ":")
    current_lines = append_string(current_lines, "  br i1 " + left_bool.operand.value +
                                  ", label %" + merge_label + ", label %" + check_right_label)
    current_lines = append_string(current_lines, "")
    current_lines = append_string(current_lines, check_right_label + ":")
    right_result = lower_expression(
        right_text, bindings, locals, left_bool.temp_index, current_lines, functions, context, "")
    ...
    result_temp = format_temp_name(right_bool.temp_index)
    current_lines = append_string(current_lines, "  " + result_temp +
                                  " = phi i1 [ true, %" + entry_label + " ], [ " + right_bool.operand.value + ", %" + right_end_label + " ]")
    operand = LLVMOperand(llvm_type="i1", value=result_temp)
    return ExpressionResult(lines=current_lines, temp_index=right_bool.temp_index + 1, operand=operand, diagnostics=diagnostics, string_constants=string_constants)
...
def lower_call_expression(target, arguments, bindings, locals, temp_index, lines, functions, context):
        ...
        if enum_info != None:
            ...
            if len(arguments) > 0:
                ...
                if variant_info != None and len(variant_info.fields) == len(arguments):
                    ...
                    pass
            enum_parse = EnumLiteralParse(recognized=True, success=True, enum_name=enum_name,
                                          variant_name=variant_name, fields=fields, diagnostics=[])
    ...
    if method_parse.success:
        method_info = resolve_struct_info_for_method_target(
            method_parse.base, bindings, locals, context)
        interface_info = resolve_interface_info_for_method_target(
            method_parse.base, bindings, locals, context)
        if method_info != None:
            lowered_self = lower_expression(
                method_parse.base, bindings, locals, current_temp, current_lines, functions, context, "")
            diagnostics = (diagnostics) + (lowered_self.diagnostics)
            collected_string_constants = merge_string_constants(
                collected_string_constants, lowered_self.string_constants)
            ...
            if lowered_self.operand != None:
                ...
                diagnostics = append_string(
                    diagnostics, "llvm lowering: method call base `" + method_parse.base + "` produced no value")
            ...
            if interface_info != None:
                ...
                if lowered_self.operand != None:
                    method_operand = lowered_self.operand
                    trimmed_target = "trait_dispatch::" + \
                        interface_info.name + "::" + method_parse.field
                    injected_argument_count = 1
                else:
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: method call base `" + method_parse.base + "` produced no value")
                ...
                if helper_candidate != None:
                    ...
                    if lowered_self.operand != None:
                        ...
                        array_element_type = array_pointer_element_type(
    ...
    if substring(trimmed_target, 0, 16) == "trait_dispatch::":
        ...
        if double_colon_pos > 0 and substring(after_prefix, double_colon_pos - 1, double_colon_pos + 1) == "::":
            ...
            method_name = substring(
                after_prefix, double_colon_pos + 1, len(after_prefix))
            interface_info = find_interface_info_by_name(
    ...
    if not is_trait_dispatch:
        if function_entry != None:
            ...
            if len(llvm_return) == 0:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: unsupported return type in call to `" + trimmed_target + "`")
                llvm_return = "double"
            expected_params = collect_parameter_types(
                context, function_entry.parameters)
        else:
            if helper_descriptor != None:
                ...
                diagnostics = append_string(
                    diagnostics, "llvm lowering: call to unknown function `" + trimmed_target + "`")
    index = 0
    while True:
        ...
        if len(argument_text) == 0:
            diagnostics = append_string(
                diagnostics, "llvm lowering: empty argument in call to `" + trimmed_target + "`")
        ...
        lowered = lower_expression(argument_text, bindings, locals, current_temp,
                                   current_lines, functions, context, argument_expected_type)
        diagnostics = (diagnostics) + (lowered.diagnostics)
        collected_string_constants = merge_string_constants(
            collected_string_constants, lowered.string_constants)
        current_lines = lowered.lines
        current_temp = lowered.temp_index
        if lowered.operand != None:
            operands = append_llvm_operand(operands, lowered.operand)
        else:
            diagnostics = append_string(diagnostics, "llvm lowering: failed to lower argument " +
                                        number_to_string(index) + " for call to `" + trimmed_target + "`")
        index += 1
    if method_operand != None:
        operand_value = method_operand
        combined_operands = []
        combined_operands = append_llvm_operand(
            combined_operands, operand_value)
        operand_index = 0
        while True:
            if operand_index >= len(operands):
                break
            combined_operands = append_llvm_operand(
                combined_operands, operands[operand_index])
            operand_index += 1
    ...
    if len(operands) != expected_operand_count:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to emit call to `" + trimmed_target + "` due to argument errors")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
            ...
            if self_operand.llvm_type != expected_self_type:
                if ends_with_pointer_suffix(self_operand.llvm_type):
                    ...
                    if stripped == expected_self_type:
                        ...
                        current_lines = append_string(current_lines, "  " + load_name + " = load " +
                                                      expected_self_type + ", " + expected_self_type + "* " + self_operand.value)
                        current_temp += 1
                        updated_operand = LLVMOperand(
                            llvm_type=expected_self_type, value=load_name)
                        operands = replace_llvm_operand(
                            operands, 0, updated_operand)
                    else:
                        diagnostics = append_string(diagnostics, "llvm lowering: method call expects `" +
                                                    expected_self_type + "` but base `" + self_operand.llvm_type + "` is incompatible")
                else:
                    diagnostics = append_string(diagnostics, "llvm lowering: method call expects `" +
                                                expected_self_type + "` but base `" + self_operand.llvm_type + "` is incompatible")
    coerced_operands = []
    ...
    while True:
        ...
        if is_trait_dispatch and index == 0:
            ...
            if len(target_type) == 0:
                coerced_operands = append_llvm_operand(
                    coerced_operands, operand)
            else:
                coerced = coerce_operand_to_type(
                    operand, target_type, current_temp, current_lines)
                diagnostics = (diagnostics) + (coerced.diagnostics)
                current_lines = coerced.lines
                current_temp = coerced.temp_index
                if coerced.operand == None:
                    diagnostics = append_string(diagnostics, "llvm lowering: unable to coerce argument " +
                                                number_to_string(index) + " for call to `" + trimmed_target + "`")
                    placeholder = LLVMOperand(llvm_type="i8*", value="null")
                    coerced_operands = append_llvm_operand(
                        coerced_operands, placeholder)
                else:
                    coerced_operands = append_llvm_operand(
                        coerced_operands, coerced.operand)
        index += 1
    operands = coerced_operands
    if function_entry != None or helper_descriptor != None:
        if len(expected_params) != len(operands):
            diagnostics = append_string(diagnostics, "llvm lowering: call to `" + trimmed_target + "` expected " +
                                        number_to_string(len(expected_params)) + " arguments but received " + number_to_string(len(operands)))
    rendered_args = []
    index = 0
    while True:
        if index >= len(operands):
            break
        rendered_args = append_string(
            rendered_args, format_typed_operand(operands[index]))
        index += 1
    ...
    if substring(trimmed_target, 0, 16) == "trait_dispatch::":
        ...
        if double_colon_pos > 0 and substring(after_prefix, double_colon_pos - 1, double_colon_pos + 1) == "::":
            ...
            if len(operands) > 0:
                ...
                current_temp += 1
                current_lines = append_string(
                    current_lines, "  " + data_ptr_temp + " = extractvalue " + trait_object.llvm_type + " " + trait_object.value + ", 0")
                vtable_ptr_temp = "%t" + number_to_string(current_temp)
                current_temp += 1
                current_lines = append_string(current_lines, "  " + vtable_ptr_temp +
                                              " = extractvalue " + trait_object.llvm_type + " " + trait_object.value + ", 1")
                interface_info = find_interface_info_by_name(
                    context, interface_name)
                if interface_info != None:
                    method_index = -1
                    ...
                    if method_index >= 0:
                        ...
                        while True:
                            if arg_idx >= len(operands):
                                break
                            fp_param_types = append_string(
                                fp_param_types, operands[arg_idx].llvm_type)
                            arg_idx += 1
                        fp_params = join_with_separator(fp_param_types, ", ")
                        function_type = llvm_return + " (" + fp_params + ")*"
                        vtable_cast_temp = "%t" + \
                            number_to_string(current_temp)
                        current_temp += 1
                        current_lines = append_string(
                            current_lines, "  " + vtable_cast_temp + " = bitcast i8* " + vtable_ptr_temp + " to " + function_type + "*")
                        method_ptr_temp = "%t" + number_to_string(current_temp)
                        current_temp += 1
                        current_lines = append_string(
                            current_lines, "  " + method_ptr_temp + " = load " + function_type + ", " + function_type + "* " + vtable_cast_temp)
                        call_args = ["i8* " + data_ptr_temp]
                        arg_idx = 1
                        while True:
                            if arg_idx >= len(rendered_args):
                                break
                            call_args = append_string(
                                call_args, rendered_args[arg_idx])
                            arg_idx += 1
                        call_arg_text = join_with_separator(call_args, ", ")
                        if llvm_return == "void":
                            current_lines = append_string(
                                current_lines, "  call void " + method_ptr_temp + "(" + call_arg_text + ")")
                            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
                        else:
                            result_temp = format_temp_name(current_temp)
                            current_lines = append_string(
                                current_lines, "  " + result_temp + " = call " + llvm_return + " " + method_ptr_temp + "(" + call_arg_text + ")")
                            result_operand = LLVMOperand(
                                llvm_type=llvm_return, value=result_temp)
                        ...
                        diagnostics = append_string(
                            diagnostics, "llvm lowering: method `" + method_name + "` not found in interface `" + interface_name + "`")
                else:
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: interface `" + interface_name + "` not found for trait dispatch")
            else:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: trait dispatch requires at least one argument (the trait object)")
        else:
            diagnostics = append_string(
                diagnostics, "llvm lowering: malformed trait dispatch target `" + trimmed_target + "`")
    ...
    if llvm_return == "void":
        ...
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    temp_name = format_temp_name(current_temp)
    call_line = "  " + temp_name + " = call " + llvm_return + \
        " @" + call_symbol + "(" + argument_text + ")"
    current_lines = append_string(current_lines, call_line)
...
def lower_member_access(parse, bindings, locals, temp_index, lines, functions, context, expected_type):
    base_result = lower_expression(
        parse.base, bindings, locals, temp_index, lines, functions, context, "")
    ...
    if parse.field == "length" and ends_with_pointer_suffix(base_operand.llvm_type):
        ...
        if starts_with(inner_type, "{") and len(inner_type) > 0 and inner_type[len(inner_type) - 1] == "}":
            struct_inner = trim_text(
                substring(inner_type, 1, len(inner_type) - 1))
            if contains_text(struct_inner, ", i64"):
                loaded_array = format_temp_name(current_temp)
                current_lines = append_string(current_lines, "  " + loaded_array + " = load " +
                                              inner_type + ", " + base_operand.llvm_type + " " + base_operand.value)
                current_temp += 1
                length_temp = format_temp_name(current_temp)
                current_lines = append_string(
                    current_lines, "  " + length_temp + " = extractvalue " + inner_type + " " + loaded_array + ", 1")
                current_temp += 1
    ...
    if parse.field == "length" and base_operand.llvm_type == "i8*":
        length_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + length_temp +
                                      " = call i64 @sailfin_runtime_string_length(i8* " + base_operand.value + ")")
        current_temp += 1
        operand = LLVMOperand(llvm_type="i64", value=length_temp)
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=operand, diagnostics=diagnostics, string_constants=collected_string_constants)
    struct_info = None
    ...
    if ends_with_pointer_suffix(base_operand.llvm_type):
        ...
        if struct_info == None:
            enum_info_candidate = find_enum_info_by_llvm_type(
                context, candidate)
            ...
            if base_operand.llvm_type != "double" and base_operand.llvm_type != "i8":
                diagnostics = append_string(
                    diagnostics, "llvm lowering: member access base `" + base_operand.llvm_type + "` lacks struct metadata")
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        pointer_available = True
    else:
        struct_info = find_struct_info_by_llvm_type(
            context, base_operand.llvm_type)
        if struct_info == None:
            enum_info_candidate = find_enum_info_by_llvm_type(
                context, base_operand.llvm_type)
            if enum_info_candidate != None:
    ...
    if field_info == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: struct `" + info.name + "` has no field `" + parse.field + "`")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    if pointer_available:
        struct_type = info.llvm_name
        gep_name = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + gep_name + " = getelementptr " + struct_type + ", " +
                                      struct_type + "* " + pointer_operand.value + ", i32 0, i32 " + number_to_string(field_info.index))
        current_temp += 1
        load_name = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + load_name + " = load " +
                                      field_info.llvm_type + ", " + field_info.llvm_type + "* " + gep_name)
        current_temp += 1
        operand = LLVMOperand(llvm_type=field_info.llvm_type, value=load_name)
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=operand, diagnostics=diagnostics, string_constants=collected_string_constants)
    extract_name = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + extract_name + " = extractvalue " +
                                  info.llvm_name + " " + base_operand.value + ", " + number_to_string(field_info.index))
    current_temp += 1
...
def lower_dynamic_member_access(field, base_operand, temp_index, lines, diagnostics, string_constants):
    ...
    existing_constant = find_string_constant_by_name(
        dynamic_string_constants, constant_name)
    field_constant = StringConstant(
        name=constant_name, content=field, byte_count=len(field))
    if existing_constant != None:
        field_constant = existing_constant
    else:
        dynamic_string_constants = append_string_constant(
            dynamic_string_constants, field_constant)
    pointer_result = emit_string_constant_pointer(
        field_constant, current_temp, current_lines)
    ...
    current_lines = append_string(current_lines, "  " + call_temp +
                                  " = call i8* @sailfin_runtime_get_field(i8* " + base_operand.value + ", i8* " + field_pointer + ")")
    current_temp += 1
...
def lower_runtime_global_reference(temp_index, lines):
    ...
    current_lines = lines
    current_lines = append_string(
        current_lines, "  " + runtime_temp + " = load i8*, i8** @runtime")
...
def lower_enum_member_access(parse, enum_info, base_operand, pointer_available, temp_index, lines, diagnostics, string_constants, expected_type):
    ...
    if pointer_available:
        ...
        current_lines = append_string(current_lines, "  " + tag_ptr + " = getelementptr inbounds " +
                                      enum_info.llvm_name + ", " + enum_info.llvm_name + "* " + base_operand.value + ", i32 0, i32 0")
        current_temp += 1
        tag_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + tag_temp + " = load " +
                                      enum_info.tag_type + ", " + enum_info.tag_type + "* " + tag_ptr)
        current_temp += 1
        tag_operand = LLVMOperand(llvm_type=enum_info.tag_type, value=tag_temp)
    else:
        tag_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + tag_temp + " = extractvalue " +
                                      enum_info.llvm_name + " " + base_operand.value + ", 0")
        current_temp += 1
        tag_operand = LLVMOperand(llvm_type=enum_info.tag_type, value=tag_temp)
    if parse.field == "variant":
        ...
        existing_default = find_string_constant_by_name(
            enum_string_constants, default_constant_name)
        default_constant = StringConstant(
            name=default_constant_name, content="", byte_count=0)
        if existing_default != None:
            default_constant = existing_default
        else:
            default_constant = StringConstant(
                name=default_constant_name, content="", byte_count=0)
            enum_string_constants = append_string_constant(
                enum_string_constants, default_constant)
        default_pointer_result = emit_string_constant_pointer(
            default_constant, current_temp, current_lines)
        ...
        while True:
            ...
            existing_variant = find_string_constant_by_name(
                enum_string_constants, constant_name)
            variant_constant = StringConstant(
                name=constant_name, content="", byte_count=0)
            if existing_variant != None:
                variant_constant = existing_variant
            else:
                variant_constant = StringConstant(
                    name=constant_name, content=variant_info.name, byte_count=len(variant_info.name))
                enum_string_constants = append_string_constant(
                    enum_string_constants, variant_constant)
            pointer_result = emit_string_constant_pointer(
                variant_constant, current_temp, current_lines)
            current_lines = pointer_result.lines
            current_temp = pointer_result.temp_index
            variant_pointer = pointer_result.pointer
            compare_temp = format_temp_name(current_temp)
            current_lines = append_string(current_lines, "  " + compare_temp + " = icmp eq " +
                                          tag_operand.llvm_type + " " + tag_operand.value + ", " + number_to_string(variant_info.tag))
            current_temp += 1
            select_temp = format_temp_name(current_temp)
            current_lines = append_string(current_lines, "  " + select_temp + " = select i1 " +
                                          compare_temp + ", i8* " + variant_pointer + ", i8* " + current_pointer)
            current_temp += 1
    ...
    while True:
        ...
        candidate_field = find_variant_field_info(
            candidate_variant, parse.field)
    ...
    if len(matched_variants) == 0:
        messages = append_string(messages, "llvm lowering: enum `" + enum_info.name +
                                 "` has no field `" + parse.field + "` on any variant")
        ...
        while True:
            ...
            if candidate_field.llvm_type == expected_type:
                filtered_variants = (filtered_variants) + \
                    ([matched_variants[selection_index]])
    ...
    if len(selected_variants) == 0:
        if len(expected_type) > 0:
            messages = append_string(messages, "llvm lowering: enum `" + enum_info.name + "` has no field `" +
                                     parse.field + "` compatible with expected type `" + expected_type + "`")
        else:
            messages = append_string(messages, "llvm lowering: enum member access for `" +
                                     enum_info.name + "." + parse.field + "` uses incompatible field types across variants")
        return make_expression_result(current_lines, current_temp, None, messages, enum_string_constants)
    ...
    if not pointer_available:
        ...
        current_lines = append_string(
            current_lines, "  " + alloca_temp + " = alloca " + enum_info.llvm_name)
        current_temp += 1
        current_lines = append_string(current_lines, "  store " + enum_info.llvm_name +
                                      " " + base_operand.value + ", " + enum_info.llvm_name + "* " + alloca_temp)
    ...
    while True:
        ...
        payload_ptr_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + payload_ptr_temp + " = getelementptr inbounds " +
                                      enum_info.llvm_name + ", " + enum_info.llvm_name + "* " + enum_pointer_value + ", i32 0, i32 1")
        current_temp += 1
        byte_ptr_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + byte_ptr_temp + " = bitcast [" + number_to_string(
            variant_info.size) + " x i8]* " + payload_ptr_temp + " to i8*")
        current_temp += 1
        ...
        typed_ptr_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + typed_ptr_temp +
                                      " = bitcast i8* " + field_ptr_temp + " to " + storage_type + "*")
        current_temp += 1
        ...
        if pointer_coercion:
            if ends_with_pointer_suffix(storage_type):
                value_temp = format_temp_name(current_temp)
                current_lines = append_string(
                    current_lines, "  " + value_temp + " = load " + storage_type + ", " + storage_type + "* " + typed_ptr_temp)
                current_temp += 1
        else:
            value_temp = format_temp_name(current_temp)
            current_lines = append_string(
        ...
        current_temp += 1
        select_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + select_temp + " = select i1 " + compare_temp +
                                      ", " + field_type + " " + value_temp + ", " + field_type + " " + current_value_text)
        current_temp += 1
...
def emit_string_constant_pointer(constant, temp_index, lines):
    ...
    current_lines = append_string(current_lines, "  " + temp_name + " = getelementptr inbounds " +
                                  array_type + ", " + array_type + "* " + constant.name + ", i32 0, i32 0")
...
def lower_index_expression(parse, bindings, locals, temp_index, lines, functions, context):
    ...
    base_operand = base_result.operand
    index_result = lower_expression(
        parse.index, bindings, locals, current_temp, current_lines, functions, context, "")
    diagnostics = (diagnostics) + (index_result.diagnostics)
    collected_string_constants = merge_string_constants(
        collected_string_constants, index_result.string_constants)
    current_lines = index_result.lines
    current_temp = index_result.temp_index
    if index_result.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: index expression produced no value")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    index_operand = index_result.operand
    index_coercion = coerce_operand_to_type(
        index_operand, "i64", current_temp, current_lines)
    ...
    if index_coercion.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to coerce index expression to i64")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        ...
        if ends_with_pointer_suffix(array_type):
            ...
            if starts_with(inner_type, "{") and len(inner_type) > 0 and inner_type[len(inner_type) - 1] == "}":
                ...
                while True:
                    ...
                    i += 1
                if comma_pos >= 0:
                    first_field = trim_text(
                        substring(struct_inner, 0, comma_pos))
                    if ends_with_pointer_suffix(first_field):
                        element_type = strip_pointer_suffix(first_field)
                        is_heap_array = True
    if len(element_type) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to determine element type for array indexing on type `" + array_type + "`")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    if is_heap_array:
        data_ptr = ""
        length_value = ""
        ...
        loaded_array = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + loaded_array + " = load " +
                                      array_struct_type + ", " + array_type + " " + base_operand.value)
        current_temp += 1
        data_ptr_extracted = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + data_ptr_extracted +
                                      " = extractvalue " + array_struct_type + " " + loaded_array + ", 0")
        current_temp += 1
        data_ptr = data_ptr_extracted
        length_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + length_temp +
                                      " = extractvalue " + array_struct_type + " " + loaded_array + ", 1")
        current_temp += 1
        length_value = length_temp
        bounds_check = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + bounds_check +
                                      " = icmp uge i64 " + coerced_index.value + ", " + length_value)
        current_temp += 1
        current_lines = append_string(
            current_lines, "  ; bounds check: " + bounds_check + " (if true, out of bounds)")
        current_lines = append_string(
            current_lines, "  call void @sailfin_runtime_bounds_check(i64 " + coerced_index.value + ", i64 " + length_value + ")")
        element_ptr = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + element_ptr + " = getelementptr " +
                                      element_type + ", " + element_type + "* " + data_ptr + ", i64 " + coerced_index.value)
        current_temp += 1
        element_value = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + element_value +
                                      " = load " + element_type + ", " + element_type + "* " + element_ptr)
        current_temp += 1
        ...
        if is_string:
            ...
            current_lines = append_string(current_lines, "  " + char_pointer +
                                          " = getelementptr i8, i8* " + base_operand.value + ", i64 " + coerced_index.value)
            current_temp += 1
            loaded_char = format_temp_name(current_temp)
            current_lines = append_string(
                current_lines, "  " + loaded_char + " = load i8, i8* " + char_pointer)
            current_temp += 1
            operand = LLVMOperand(llvm_type="i8", value=loaded_char)
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=operand, diagnostics=diagnostics, string_constants=collected_string_constants)
    diagnostics = append_string(
        diagnostics, "llvm lowering: unsupported array type for indexing: `" + array_type + "`")
    return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
...
def lower_array_literal(text, bindings, locals, temp_index, lines, functions, context, expected_type):
    ...
    if len(expected_element_type) > 0:
        expected_struct_type = array_struct_type_for_element(
            expected_element_type)
    ...
    while True:
        ...
        if len(element_text) == 0:
            diagnostics = append_string(
                diagnostics, "llvm lowering: empty element in array literal")
            index += 1
            continue
        lowered = lower_expression(
            element_text, bindings, locals, current_temp, current_lines, functions, context, "")
        diagnostics = (diagnostics) + (lowered.diagnostics)
        collected_string_constants = merge_string_constants(
            collected_string_constants, lowered.string_constants)
        current_lines = lowered.lines
        ...
        if lowered.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: array literal element produced no value")
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        operands = append_llvm_operand(operands, lowered.operand)
        inferred_element_type = dominant_type(
            inferred_element_type, lowered.operand.llvm_type)
        index += 1
    if len(operands) == 0:
        if len(inferred_element_type) == 0 and len(expected_element_type) > 0:
            inferred_element_type = expected_element_type
    ...
    if len(inferred_element_type) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unsupported array literal element type")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    ...
    while True:
        ...
        if ends_with_pointer_suffix(element_type) and not ends_with_pointer_suffix(operand.llvm_type):
            boxed = box_aggregate_operand(
                operand, element_type, current_temp, current_lines, context)
            diagnostics = (diagnostics) + (boxed.diagnostics)
            current_lines = boxed.lines
            current_temp = boxed.temp_index
            if boxed.operand == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: array literal element could not be boxed to `" + element_type + "`")
                return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
            prepared_operand = boxed.operand
        coerced = coerce_operand_to_type(
            prepared_operand, element_type, current_temp, current_lines)
        diagnostics = (diagnostics) + (coerced.diagnostics)
        current_lines = coerced.lines
        current_temp = coerced.temp_index
        if coerced.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: array literal element could not be coerced to `" + element_type + "`")
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        coerced_operands = append_llvm_operand(
            coerced_operands, coerced.operand)
        index += 1
    ...
    current_lines = append_string(
        current_lines, "  " + array_alloca + " = alloca " + array_type)
    current_temp += 1
    data_pointer = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + data_pointer + " = getelementptr " +
                                  array_type + ", " + array_type + "* " + array_alloca + ", i32 0, i32 0")
    current_temp += 1
    index = 0
    while True:
        if index >= len(operands):
        ...
        current_lines = append_string(current_lines, "  " + element_pointer + " = getelementptr " +
                                      element_type + ", " + element_type + "* " + data_pointer + ", i64 " + number_to_string(index))
        current_temp += 1
        current_lines = append_string(current_lines, "  store " + element_type +
                                      " " + operands[index].value + ", " + element_type + "* " + element_pointer)
        index += 1
    ...
    current_lines = append_string(
        current_lines, "  " + struct_alloca + " = alloca " + struct_type)
    current_temp += 1
    data_field_pointer = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + data_field_pointer + " = getelementptr " +
                                  struct_type + ", " + struct_type + "* " + struct_alloca + ", i32 0, i32 0")
    current_temp += 1
    data_pointer_type = element_type + "*"
    data_pointer_pointer_type = data_pointer_type + "*"
    current_lines = append_string(current_lines, "  store " + data_pointer_type +
                                  " " + data_pointer + ", " + data_pointer_pointer_type + " " + data_field_pointer)
    length_field_pointer = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + length_field_pointer + " = getelementptr " +
                                  struct_type + ", " + struct_type + "* " + struct_alloca + ", i32 0, i32 1")
    current_temp += 1
    current_lines = append_string(
        current_lines, "  store i64 " + length_text + ", i64* " + length_field_pointer)
...
def parse_struct_literal(text):
    ...
    if close_index < 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: struct literal missing closing `}`")
        snippet = trimmed
        if len(snippet) > 120:
            snippet = substring(snippet, 0, 117) + "..."
        diagnostics = append_string(
            diagnostics, "llvm lowering: struct literal context `" + snippet + "`")
        fatal = True
    type_name = trim_text(substring(trimmed, 0, open_index))
    if len(type_name) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: struct literal missing type name")
        fatal = True
    fields = []
    if not fatal and close_index >= 0:
        ...
        while True:
            ...
            if colon_index < 0:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: struct literal field `" + entry + "` missing `:`")
                continue
            field_name = trim_text(substring(entry, 0, colon_index))
            value_text = trim_text(
                substring(entry, colon_index + 1, len(entry)))
            if len(field_name) == 0:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: struct literal field missing name")
                continue
            if string_array_contains(seen, field_name):
                diagnostics = append_string(
                    diagnostics, "llvm lowering: struct literal field `" + field_name + "` repeated")
                continue
            if len(value_text) == 0:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: struct literal field `" + field_name + "` missing value")
                continue
            seen = append_string(seen, field_name)
            fields = (fields) + \
                ([StructLiteralField(name=field_name, value=value_text)])
    if not fatal and close_index >= 0:
        remainder = trim_text(
            substring(trimmed, close_index + 1, len(trimmed)))
        if len(remainder) > 0:
            diagnostics = append_string(
                diagnostics, "llvm lowering: struct literal trailing content `" + remainder + "`")
            fatal = True
    if fatal:
        fields = []
...
def parse_enum_literal(text):
    ...
    while True:
        ...
        index += 1
    if open_index < 0:
        variant_name_part = trim_text(
            substring(trimmed, dot_index + 1, len(trimmed)))
    ...
    if close_index < 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: enum literal missing closing `}`")
        fatal = True
    variant_name_part = trim_text(
        substring(trimmed, dot_index + 1, open_index))
    if len(variant_name_part) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: enum literal missing variant name")
        fatal = True
    fields = []
    if not fatal and close_index >= 0:
        body = substring(trimmed, open_index + 1, close_index)
        ...
        while True:
            ...
            if colon_index < 0:
                ...
                if len(field_name) == 0:
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: enum literal field missing name")
                    continue
                if string_array_contains(seen, field_name):
                    diagnostics = append_string(
                        diagnostics, "llvm lowering: enum literal field `" + field_name + "` repeated")
                    continue
                seen = append_string(seen, field_name)
                fields = (fields) + \
                    ([StructLiteralField(name=field_name, value="")])
                continue
            field_name = trim_text(substring(entry, 0, colon_index))
            value_text = trim_text(
                substring(entry, colon_index + 1, len(entry)))
            ...
            if len(value_text) == 0:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: enum literal field `" + field_name + "` missing value")
                continue
            seen = append_string(seen, field_name)
            fields = (fields) + \
                ([StructLiteralField(name=field_name, value=value_text)])
    if not fatal and close_index >= 0:
        ...
        if len(remainder) > 0:
            diagnostics = append_string(
                diagnostics, "llvm lowering: enum literal trailing content `" + remainder + "`")
            fatal = True
    if fatal:
        fields = []
...
def lower_struct_literal(parse, bindings, locals, temp_index, lines, functions, context, expected_type):
    ...
    if info == None:
        if len(parse.type_name) > 0:
            diagnostics = append_string(
                diagnostics, "llvm lowering: struct literal references unknown struct `" + parse.type_name + "`")
        else:
            diagnostics = append_string(
                diagnostics, "llvm lowering: struct literal references unknown struct")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    if len(info.fields) == 0:
        operand = LLVMOperand(llvm_type=info.llvm_name,
    ...
    while True:
        ...
        if literal_index >= 0:
            ...
            if expected.name == "string_constants":
                ...
                while True:
                    if local_idx >= len(locals):
                        break
                    local_names = append_string(
                        local_names, locals[local_idx].name)
                    local_idx += 1
                print.info("[sfn-debug] lower_struct_literal locals=" +
                           join_strings(local_names, ", "))
            lowered = lower_expression(literal_field.value, bindings, locals,
                                       current_temp, current_lines, functions, context, expected.llvm_type)
            diagnostics = (diagnostics) + (lowered.diagnostics)
            current_lines = lowered.lines
            current_temp = lowered.temp_index
            collected_string_constants = merge_string_constants(
            ...
            if lowered.operand != None:
                coerced = coerce_operand_to_type(
                    lowered.operand, expected.llvm_type, current_temp, current_lines)
                diagnostics = (diagnostics) + (coerced.diagnostics)
                current_lines = coerced.lines
                current_temp = coerced.temp_index
                if coerced.operand != None:
            ...
            diagnostics = append_string(diagnostics, "llvm lowering: struct literal for `" +
                                        info.name + "` missing field `" + expected.name + "`")
        ...
        temp_name = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + temp_name + " = insertvalue " + info.llvm_name + " " +
                                      aggregate_source + ", " + expected.llvm_type + " " + value_text + ", " + number_to_string(field_index))
        current_temp += 1
    ...
    while True:
        ...
        if not string_array_contains(used_names, literal_field.name):
            diagnostics = append_string(diagnostics, "llvm lowering: struct literal for `" +
                                        info.name + "` provides unknown field `" + literal_field.name + "`")
        extra_index += 1
...
def box_aggregate_operand(operand, expected_pointer_type, temp_index, lines, context):
    ...
    if allocation == None:
        diagnostics = append_string(diagnostics, "llvm lowering: unable to allocate heap storage for `" +
                                    operand.llvm_type + "` when assigning to `" + expected_pointer_type + "`")
        return HeapBoxResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics)
    if allocation.size <= 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: computed heap allocation size for `" + operand.llvm_type + "` is zero")
        return HeapBoxResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics)
    malloc_temp = format_temp_name(current_temp)
    current_temp += 1
    current_lines = append_string(current_lines, "  " + malloc_temp +
                                  " = call noalias i8* @malloc(i64 " + number_to_string(allocation.size) + ")")
    typed_ptr_temp = format_temp_name(current_temp)
    current_temp += 1
    current_lines = append_string(current_lines, "  " + typed_ptr_temp +
                                  " = bitcast i8* " + malloc_temp + " to " + operand.llvm_type + "*")
    current_lines = append_string(current_lines, "  store " + operand.llvm_type +
                                  " " + operand.value + ", " + operand.llvm_type + "* " + typed_ptr_temp)
    ...
    if len(trimmed_expected) > 0 and trimmed_expected != "i8*":
        ...
        current_temp += 1
        current_lines = append_string(
            current_lines, "  " + cast_temp + " = bitcast i8* " + malloc_temp + " to " + trimmed_expected)
        pointer_operand = LLVMOperand(
            llvm_type=trimmed_expected, value=cast_temp)
...
def lower_enum_literal(parse, bindings, locals, temp_index, lines, functions, context, expected_type):
    ...
    if enum_info == None:
        if len(parse.enum_name) > 0:
            diagnostics = append_string(
                diagnostics, "llvm lowering: enum literal references unknown enum `" + parse.enum_name + "`")
        else:
            diagnostics = append_string(
                diagnostics, "llvm lowering: enum literal references unknown enum")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    variant_info = resolve_enum_variant_info(enum_info, parse.variant_name)
    if variant_info == None:
        diagnostics = append_string(diagnostics, "llvm lowering: enum `" +
                                    parse.enum_name + "` has no variant `" + parse.variant_name + "`")
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    tag_llvm_type = "i32"
    if enum_info.tag_type == "i8":
        tag_llvm_type = "i8"
    ...
    if len(variant_info.fields) > 0:
        ...
        current_temp += 1
        current_lines = append_string(current_lines, "  " + tag_ptr_temp + " = getelementptr inbounds " +
                                      enum_info.llvm_name + ", " + enum_info.llvm_name + "* " + alloca_temp + ", i32 0, i32 0")
        current_lines = append_string(current_lines, "  store " + tag_llvm_type + " " +
                                      number_to_string(variant_info.tag) + ", " + tag_llvm_type + "* " + tag_ptr_temp)
    else:
        tag_temp = format_temp_name(current_temp)
        current_lines = append_string(current_lines, "  " + tag_temp + " = insertvalue " + enum_info.llvm_name +
                                      " " + enum_value + ", " + tag_llvm_type + " " + number_to_string(variant_info.tag) + ", 0")
        current_temp += 1
        enum_value = tag_temp
    if len(variant_info.fields) > 0:
            ...
            if literal_index >= 0:
                ...
                if lowered.operand != None:
                    ...
                    if ends_with_pointer_suffix(expected.llvm_type) and not ends_with_pointer_suffix(field_operand.llvm_type):
                        boxed = box_aggregate_operand(
                            field_operand, expected.llvm_type, current_temp, current_lines, context)
                        diagnostics = (diagnostics) + (boxed.diagnostics)
                        current_lines = boxed.lines
                        current_temp = boxed.temp_index
                        if boxed.operand != None:
                    ...
                    coerced = coerce_operand_to_type(
                        field_operand, expected.llvm_type, current_temp, current_lines)
                    diagnostics = (diagnostics) + (coerced.diagnostics)
                    current_lines = coerced.lines
                    current_temp = coerced.temp_index
                    if coerced.operand != None and alloca_ptr != None:
                        ...
                        current_temp += 1
                        current_lines = append_string(current_lines, "  " + payload_ptr_temp + " = getelementptr inbounds " +
                                                      enum_info.llvm_name + ", " + enum_info.llvm_name + "* " + alloca_ptr + ", i32 0, i32 1")
                        byte_array_ptr_temp = format_temp_name(current_temp)
                        current_temp += 1
                        current_lines = append_string(current_lines, "  " + byte_array_ptr_temp + " = bitcast [" + number_to_string(
                            variant_info.size) + " x i8]* " + payload_ptr_temp + " to i8*")
                        ...
                        if field_offset_in_payload > 0:
                            ...
                            current_temp += 1
                            current_lines = append_string(current_lines, "  " + offset_ptr_temp + " = getelementptr inbounds i8, i8* " +
                                                          byte_array_ptr_temp + ", i64 " + number_to_string(field_offset_in_payload))
                            field_byte_ptr_temp = offset_ptr_temp
                        field_ptr_temp = format_temp_name(current_temp)
                        current_temp += 1
                        current_lines = append_string(
                            current_lines, "  " + field_ptr_temp + " = bitcast i8* " + field_byte_ptr_temp + " to " + expected.llvm_type + "*")
                        current_lines = append_string(current_lines, "  store " + expected.llvm_type +
                                                      " " + coerced.operand.value + ", " + expected.llvm_type + "* " + field_ptr_temp)
                if not string_array_contains(used_names, expected.name):
                    used_names = append_string(used_names, expected.name)
            else:
                diagnostics = append_string(diagnostics, "llvm lowering: enum literal for `" +
                                            parse.enum_name + "." + parse.variant_name + "` missing field `" + expected.name + "`")
            field_index += 1
        extra_index = 0
        while True:
            if extra_index >= len(parse.fields):
            ...
            literal_field = parse.fields[extra_index]
            if not string_array_contains(used_names, literal_field.name):
                diagnostics = append_string(diagnostics, "llvm lowering: enum literal for `" + parse.enum_name +
                                            "." + parse.variant_name + "` provides unknown field `" + literal_field.name + "`")
            extra_index += 1
        if alloca_ptr != None:
            load_temp = format_temp_name(current_temp)
            current_temp += 1
            current_lines = append_string(current_lines, "  " + load_temp + " = load " +
                                          enum_info.llvm_name + ", " + enum_info.llvm_name + "* " + alloca_ptr)
...
def emit_comparison_instruction(symbol, left_operand, right_operand, temp_index, lines):
    ...
    if left_operand.llvm_type != right_operand.llvm_type:
        diagnostics = append_string(diagnostics, "llvm lowering: comparison operands have mismatched types `" +
                                    left_operand.llvm_type + "` and `" + right_operand.llvm_type + "`")
    llvm_type = left_operand.llvm_type
    predicate = comparison_predicate_for_symbol(symbol, llvm_type)
    if len(predicate) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unsupported comparison operator `" + symbol + "` for type `" + llvm_type + "`")
        return ComparisonEmission(lines=current_lines, temp_index=temp_index, operand=None, diagnostics=diagnostics)
    temp_name = format_temp_name(temp_index)
    current_lines = append_string(current_lines, "  " + temp_name + " = " + predicate +
                                  " " + llvm_type + " " + left_operand.value + ", " + right_operand.value)
...
def emit_boolean_and(left_operand, right_operand, temp_index, lines):
    ...
    if left_operand.llvm_type != "i1":
        diagnostics = append_string(
            diagnostics, "llvm lowering: expected boolean operand for `and`, found `" + left_operand.llvm_type + "`")
    if right_operand.llvm_type != "i1":
        diagnostics = append_string(
            diagnostics, "llvm lowering: expected boolean operand for `and`, found `" + right_operand.llvm_type + "`")
    temp_name = format_temp_name(temp_index)
    current_lines = append_string(current_lines, "  " + temp_name +
                                  " = and i1 " + left_operand.value + ", " + right_operand.value)
    operand = LLVMOperand(llvm_type="i1", value=temp_name)
    return ComparisonEmission(lines=current_lines, temp_index=temp_index + 1, operand=operand, diagnostics=diagnostics)
...
def collect_parameter_types(context, parameters):
    ...
    while True:
        if index >= len(parameters):
            break
        llvm_type = map_parameter_type(
            context, parameters[index].type_annotation)
        if len(llvm_type) == 0:
...
def coerce_operand_to_type(operand, target_type, temp_index, lines):
    ...
    if not ends_with_pointer_suffix(target_type) and ends_with_pointer_suffix(operand.llvm_type):
        ...
        if source_base == target_type:
            temp_name = format_temp_name(temp_index)
            current_lines = append_string(
                current_lines, "  " + temp_name + " = load " + target_type + ", " + operand.llvm_type + " " + operand.value)
    ...
    if target_type == "double":
        if operand.llvm_type == "i64":
            temp_name = format_temp_name(temp_index)
            current_lines = append_string(
                current_lines, "  " + temp_name + " = sitofp i64 " + operand.value + " to double")
            coerced = LLVMOperand(llvm_type="double", value=temp_name)
            return CoercionResult(lines=current_lines, temp_index=temp_index + 1, operand=coerced, diagnostics=diagnostics)
        if operand.llvm_type == "i1":
            temp_name = format_temp_name(temp_index)
            current_lines = append_string(
                current_lines, "  " + temp_name + " = uitofp i1 " + operand.value + " to double")
            coerced = LLVMOperand(llvm_type="double", value=temp_name)
            return CoercionResult(lines=current_lines, temp_index=temp_index + 1, operand=coerced, diagnostics=diagnostics)
    if target_type == "i64":
        if operand.llvm_type == "double":
            temp_name = format_temp_name(temp_index)
            current_lines = append_string(
                current_lines, "  " + temp_name + " = fptosi double " + operand.value + " to i64")
            coerced = LLVMOperand(llvm_type="i64", value=temp_name)
            return CoercionResult(lines=current_lines, temp_index=temp_index + 1, operand=coerced, diagnostics=diagnostics)
        if operand.llvm_type == "i1":
            temp_name = format_temp_name(temp_index)
            current_lines = append_string(
                current_lines, "  " + temp_name + " = zext i1 " + operand.value + " to i64")
            coerced = LLVMOperand(llvm_type="i64", value=temp_name)
            return CoercionResult(lines=current_lines, temp_index=temp_index + 1, operand=coerced, diagnostics=diagnostics)
        if operand.llvm_type == "i8":
            temp_name = format_temp_name(temp_index)
            current_lines = append_string(
                current_lines, "  " + temp_name + " = sext i8 " + operand.value + " to i64")
            coerced = LLVMOperand(llvm_type="i64", value=temp_name)
            return CoercionResult(lines=current_lines, temp_index=temp_index + 1, operand=coerced, diagnostics=diagnostics)
    if target_type == "i1":
        if operand.llvm_type == "double":
        ...
        if operand.llvm_type == "i64":
            temp_name = format_temp_name(temp_index)
            current_lines = append_string(
                current_lines, "  " + temp_name + " = icmp ne i64 " + operand.value + ", 0")
            coerced = LLVMOperand(llvm_type="i1", value=temp_name)
            return CoercionResult(lines=current_lines, temp_index=temp_index + 1, operand=coerced, diagnostics=diagnostics)
    if target_type == "i8" and operand.llvm_type == "i8*":
        ...
        current_lines = append_string(
            current_lines, "  " + char_ptr_temp + " = getelementptr i8, i8* " + operand.value + ", i64 0")
        current_lines = append_string(
            current_lines, "  " + char_value_temp + " = load i8, i8* " + char_ptr_temp)
    ...
    if target_type == "i8*" and operand.llvm_type == "i8":
        ...
        current_lines = append_string(
            current_lines, "  " + alloca_temp + " = alloca [2 x i8], align 1")
        current_lines = append_string(
            current_lines, "  " + char_ptr + " = getelementptr [2 x i8], [2 x i8]* " + alloca_temp + ", i32 0, i32 0")
        current_lines = append_string(
            current_lines, "  store i8 " + operand.value + ", i8* " + char_ptr)
        current_lines = append_string(
            current_lines, "  " + null_ptr + " = getelementptr [2 x i8], [2 x i8]* " + alloca_temp + ", i32 0, i32 1")
        current_lines = append_string(
            current_lines, "  store i8 0, i8* " + null_ptr)
        current_lines = append_string(current_lines, "  " + result_ptr +
                                      " = getelementptr [2 x i8], [2 x i8]* " + alloca_temp + ", i32 0, i32 0")
    ...
    if not ends_with_pointer_suffix(operand.llvm_type) and ends_with_pointer_suffix(target_type):
        ...
        if expected_value_type == operand.llvm_type:
            alloca_temp = format_temp_name(temp_index)
            current_lines = append_string(
                current_lines, "  " + alloca_temp + " = alloca " + operand.llvm_type)
            current_lines = append_string(current_lines, "  store " + operand.llvm_type +
                                          " " + operand.value + ", " + target_type + " " + alloca_temp)
    ...
    if ends_with_pointer_suffix(operand.llvm_type) and ends_with_pointer_suffix(target_type):
        ...
        if len(source_element) > 0 and len(target_element) > 0:
            temp_name = format_temp_name(temp_index)
            current_lines = append_string(current_lines, "  " + temp_name + " = bitcast " +
                                          operand.llvm_type + " " + operand.value + " to " + target_type)
            coerced = LLVMOperand(llvm_type=target_type, value=temp_name)
            return CoercionResult(lines=current_lines, temp_index=temp_index + 1, operand=coerced, diagnostics=diagnostics)
        temp_name = format_temp_name(temp_index)
        current_lines = append_string(current_lines, "  " + temp_name + " = bitcast " +
    ...
    if not ends_with_pointer_suffix(target_type) and ends_with_pointer_suffix(operand.llvm_type):
        ...
        if base_type != target_type:
            ...
            current_lines = append_string(current_lines, "  " + bitcast_temp + " = bitcast " +
                                          pointer_type + " " + pointer_value + " to " + desired_pointer_type)
        ...
        current_lines = append_string(current_lines, "  " + load_temp +
                                      " = load " + target_type + ", " + pointer_type + " " + pointer_value)
    ...
    diagnostics = append_string(diagnostics, "llvm lowering: unable to coerce operand of type `" +
                                operand.llvm_type + "` to `" + target_type + "`")
...
def harmonise_operands(left, right, temp_index, lines):
    ...
    target_type = dominant_type(
        left_operand.llvm_type, right_operand.llvm_type)
    left_coercion = coerce_operand_to_type(
        left_operand, target_type, current_temp, current_lines)
    diagnostics = (diagnostics) + (left_coercion.diagnostics)
    if left_coercion.operand == None:
        diagnostics = append_string(diagnostics, "llvm lowering: unable to convert left operand from `" +
                                    left_operand.llvm_type + "` to `" + target_type + "`")
    ...
    right_coercion = coerce_operand_to_type(
        right_operand, target_type, current_temp, current_lines)
    diagnostics = (diagnostics) + (right_coercion.diagnostics)
    if right_coercion.operand == None:
        diagnostics = append_string(diagnostics, "llvm lowering: unable to convert right operand from `" +
                                    right_operand.llvm_type + "` to `" + target_type + "`")
                        ...
                        if ch == "{":
                            ...
                            if ch == "}":
                                ...
                                if ch == "," and paren_depth == 0 and bracket_depth == 0 and brace_depth == 0:
                                    entries = append_string(
                                        entries, trim_text(current))
                                    current = ""
                                else:
                                    current = current + ch
        index += 1
...
def map_metadata_annotation(context, annotation):
    ...
    if len(trimmed) > 2:
        ...
        if suffix == "[]":
            inner_annotation = trim_text(
                substring(trimmed, 0, len(trimmed) - 2))
...
def parse_range_iterable(iterable):
    ...
    if len(trimmed) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: `.for` iterable expression is empty")
    ...
    if first_separator < 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: `.for` iterable must use `start..end` range syntax")
        return RangeIterableParse(success=False, start="", end="", stride="", diagnostics=diagnostics)
    second_separator = find_top_level_range_separator_from(
        trimmed, first_separator + 2)
    ...
    if second_separator >= 0:
        end_text = trim_text(
            substring(trimmed, first_separator + 2, second_separator))
        stride_text = trim_text(
            substring(trimmed, second_separator + 2, len(trimmed)))
        has_stride = len(stride_text) > 0
    if len(start_text) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: `.for` range missing start expression")
    if len(end_text) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: `.for` range missing end expression")
    if has_stride and len(stride_text) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: `.for` range missing stride expression")
    success = len(start_text) > 0 and len(end_text) > 0 and (
        not has_stride or len(stride_text) > 0)
...
def mark_lifetime_region_released(regions, region_id, scope_id, scope_depth):
    ...
    while True:
        ...
        if entry.id == region_id:
            updated = LifetimeRegionMetadata(id=entry.id, binding=entry.binding, base=entry.base, mutable=entry.mutable, start_span=entry.start_span, scope_id=entry.scope_id,
                                             scope_depth=entry.scope_depth, base_scope_id=entry.base_scope_id, base_scope_depth=entry.base_scope_depth, end_scope_id=scope_id, end_scope_depth=scope_depth, released=True)
...
def apply_lifetime_release_events(regions, releases):
    ...
    while True:
        ...
        current_regions = mark_lifetime_region_released(
            current_regions, release.region_id, release.scope_id, release.scope_depth)
        index += 1
...
def number_to_string(value):
    if value == 0:
        return "0"
    digits = "0123456789"
    powers = [1000000000000000, 100000000000000, 10000000000000, 1000000000000, 100000000000,
...
def lower_string_literal(literal, temp_index, lines):
    ...
    gep_instr = "  %s" + number_to_string(temp_index) + " = getelementptr inbounds " + \
        array_type + ", " + array_type + "* " + global_name + ", i32 0, i32 0"
    updated_lines = append_string(lines, gep_instr)
    operand = LLVMOperand(llvm_type="i8*", value="%s" +
                          number_to_string(temp_index))
...
def merge_parameter_bindings(first, second):
    ...
    while True:
        ...
        result = append_parameter_binding(result, ParameterBinding(name=primary.name, llvm_name=primary.llvm_name,
                                          llvm_type=primary.llvm_type, type_annotation=primary.type_annotation, consumed=consumed_flag, span=primary.span))
        index += 1
    return result
