...
def collect_instruction_runtime_helper_targets(instruction):
    ...
    if instruction.variant == "Case":
        ...
        if instruction.guard != None:
            helpers = merge_effect_lists(helpers, filter_runtime_helper_targets(
    ...
    while True:
        ...
        if local != None:
            ...
            if preloaded_value != None and len(preloaded_value) > 0:
                phi_inputs = ["[ " + mutation.value_name + ", %" + then_label +
            ...
            if not then_terminated:
                if then_mutation != None:
                    phi_inputs = append_string(
                        phi_inputs, "[ " + then_mutation.value_name + ", %" + then_label + " ]")
                else:
                    if preloaded_value != None:
                        phi_inputs = append_string(
                            phi_inputs, "[ " + preloaded_value + ", %" + then_label + " ]")
            if not else_terminated:
                if else_mutation != None:
                    phi_inputs = append_string(
                        phi_inputs, "[ " + else_mutation.value_name + ", %" + else_label + " ]")
                else:
                    if preloaded_value != None:
                        phi_inputs = append_string(
                            phi_inputs, "[ " + preloaded_value + ", %" + else_label + " ]")
            if len(phi_inputs) >= 2:
                entry = build_phi_and_store(
...
def format_suspension_location(keyword, borrow_span, suspension_span):
    locations = []
    if borrow_span != None:
        locations.append("borrow at " + format_span_location(borrow_span))
    if suspension_span != None:
        label = keyword if len(keyword) > 0 else "suspend"
        locations.append(label + " at " +
                         format_span_location(suspension_span))
    if len(locations) == 0:
        return ""
