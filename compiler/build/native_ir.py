...
class NativeFunction:
    def __init__(self, name, is_async, parameters, return_type, effects, decorators, is_extern, instructions):
        self.name = name
...
def parse_native_artifact(text):
    ...
    while True:
        ...
        if starts_with(line, ".fn "):
            ...
            header = strip_prefix(line, ".fn ")
            is_async = False
            trimmed_header = trim_text(header)
            if starts_with(trimmed_header, "async "):
                is_async = True
            current = NativeFunction(name=parse_function_name(header), is_async=is_async, parameters=[], return_type="void", effects=[], decorators=pending_decorators, is_extern=False, instructions=[])
            pending_decorators = []
            index += 1
            continue
        if starts_with(line, ".test "):
            ...
            if len(name) == 0:
                ...
                index += 1
                continue
            current = NativeFunction(name="test:" + name, is_async=False, parameters=[], return_type="void", effects=[], decorators=[], is_extern=False, instructions=[])
            index += 1
            continue
...
def apply_meta(function, entry):
    ...
    if trimmed == "extern":
        return NativeFunction(name=function.name, is_async=function.is_async, parameters=function.parameters, return_type=function.return_type, effects=function.effects, decorators=function.decorators, is_extern=True, instructions=function.instructions)
    if trimmed == "unsafe":
        return function
...
def update_function_meta(function, return_type, effects):
    return NativeFunction(name=function.name, is_async=function.is_async, parameters=function.parameters, return_type=return_type, effects=effects, decorators=function.decorators, is_extern=function.is_extern, instructions=function.instructions)
...
def parse_struct_definition(lines, start_index):
    ...
    while True:
        ...
        if starts_with(raw_line, ".method "):
            if current_method != None:
                diagnostics = append_string(diagnostics, "nested method declaration in struct " + struct_name)
            header = strip_prefix(raw_line, ".method ")
            trimmed = trim_text(header)
            is_async = False
            if starts_with(trimmed, "async "):
                is_async = True
            method_name = parse_function_name(header)
            current_method = NativeFunction(name=method_name, is_async=is_async, parameters=[], return_type="void", effects=[], decorators=method_pending_decorators, is_extern=False, instructions=[])
            method_pending_decorators = []
            method_pending_span = None
            method_pending_value_span = None
            index += 1
