...
from compiler.build.string_utils import substring, char_code, char_at, index_of
...
def parse_index_expression(expression):
    ...
    index_value = trim_text(
        substring(trimmed, open_bracket + 1, len(trimmed) - 1))
...
def parse_inline_let_expression(expression):
    ...
    if not assignment.success:
        diagnostics = append_string(
            diagnostics, "llvm lowering: inline let expression missing initializer")
    ...
    if len(name) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: inline let expression missing variable name")
...
def parse_raw_address_expression(text):
    ...
    if not ends_with(trimmed, ")"):
        diagnostics = [
            "llvm lowering: raw_address expression missing closing `)`"]
...
def parse_cast_expression(text):
    ...
    if len(value) == 0 or len(annotation) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: malformed cast expression `" + text + "`")
