...
from compiler.build.native_ir import NativeStruct, NativeEnum, NativeEnumLayout, NativeEnumVariantLayout, NativeInterface, NativeFunction, NativeInterfaceSignature
...
def synthesize_unit_enum_layout(enum_def):
    ...
    while True:
        if index >= len(enum_def.variants):
            break
        variant = enum_def.variants[index]
        layouts = (layouts) + ([NativeEnumVariantLayout(name=variant.name,
                                                        tag=index, offset=0, size=tag_size, align=tag_align, fields=[])])
        index += 1
    ...
    while True:
        ...
        if enum_def.layout == None:
            if is_unit_enum(enum_def):
                fixed = NativeEnum(name=enum_def.name, variants=enum_def.variants,
                                   layout=synthesize_unit_enum_layout(enum_def))
                diagnostics = (diagnostics) + (["llvm lowering: enum `" + enum_def.name +
...
def build_type_context(structs, enums, interfaces):
    ...
    while True:
        ...
        while True:
            ...
            fields = append_struct_field_info(fields, StructFieldInfo(name=layout_field.name, llvm_type=llvm_field_type,
                                              type_annotation=layout_field.type_annotation, index=field_index, offset=layout_field.offset))
            field_index += 1
    ...
    while True:
        ...
        while True:
            ...
            payload_scan += 1
        enum_entries = append_enum_type_info(enum_entries, EnumTypeInfo(name=enum_def.name, llvm_name=llvm_enum_name, tag_type=enum_layout.tag_type, tag_size=enum_layout.tag_size,
                                             tag_align=enum_layout.tag_align, max_payload_size=max_payload_size, size=enum_layout.size, align=enum_align_value, variants=variants))
        enum_index += 1
    interface_entries = []
    interface_index = 0
    while True:
                    ...
                    while True:
                        ...
                        while True:
                            if param_idx >= len(signature.parameters):
                                break
                            param = signature.parameters[param_idx]
                            mapped_type = map_type_annotation(
                                param.type_annotation)
                            param_types = append_string(
                                param_types, mapped_type)
                            param_idx += 1
                        return_type = map_type_annotation(
