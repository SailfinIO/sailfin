...
from compiler.build.native_ir import NativeFunction, NativeParameter, NativeInterfaceSignature, NativeImport
from compiler.build.string_utils import substring, find_last_index_of_char
...
def render_runtime_helper_declarations(used_targets, local_functions):
    ...
    while True:
        if local_index >= len(local_functions):
            break
        local_symbols = append_string(
            local_symbols, sanitize_symbol(local_functions[local_index].name))
        local_index += 1
    lines = []
...
def render_imported_function_declarations(imported_functions, local_functions, context):
    ...
    while True:
        if helper_index >= len(helper_descriptors):
            break
        helper_symbols = append_string(
            helper_symbols, helper_descriptors[helper_index].symbol)
        helper_index += 1
    local_symbols = []
    local_index = 0
    while True:
    ...
    while True:
        ...
        if string_array_contains(emitted_declarations, sanitized_name):
            index += 1
            continue
        emitted_declarations = append_string(
            emitted_declarations, sanitized_name)
        return_type = map_return_type(context, function.return_type)
        param_types = collect_parameter_types(
            context, function.parameters, function.name)
        param_text = join_with_separator(param_types, ", ")
        line = "declare " + return_type + " @" + \
            sanitized_name + "(" + param_text + ")"
        lines = append_string(lines, line)
        index += 1
    return lines
...
def render_struct_type_definitions(context, functions):
    ...
    while True:
        ...
        while True:
            ...
            while True:
                ...
                extracted = extract_struct_type_from_llvm(field_type)
                if len(extracted) > 0 and not string_array_contains(referenced_types, extracted):
                    referenced_types = append_string(
                        referenced_types, extracted)
    ...
    while True:
        ...
        if len(extracted_return) > 0 and not string_array_contains(referenced_types, extracted_return):
            referenced_types = append_string(
                referenced_types, extracted_return)
        param_index = 0
        while True:
            if param_index >= len(func.parameters):
                break
            param_llvm_type = map_parameter_type(
                context, func.parameters[param_index].type_annotation)
            ...
            if len(extracted_param) > 0 and not string_array_contains(referenced_types, extracted_param):
                referenced_types = append_string(
                    referenced_types, extracted_param)
            param_index += 1
        func_index += 1
    ref_index = 0
    while True:
        ...
        if not is_defined:
            ...
            while True:
                if check_index >= len(context.enums):
                    break
                enum_llvm_name = "%" + \
                    sanitize_symbol(context.enums[check_index].name)
...
def render_vtable_constants(context):
    ...
    while True:
        ...
        lines = append_string(lines, vtable.llvm_global_name +
                              " = internal constant " + vtable.llvm_type_name + " " + body)
        index += 1
    return lines
