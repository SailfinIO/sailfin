...
def emit_program(program):
    ...
    while True:
        ...
        if index + 1 < len(program.statements):
            current = program.statements[index]
            next = program.statements[index + 1]
            current_is_import = current.variant == "ImportDeclaration"  or  current.variant == "ExportDeclaration"
            next_is_import = next.variant == "ImportDeclaration"  or  next.variant == "ExportDeclaration"
            if not current_is_import  and  next_is_import:
                builder = builder_emit_blank(builder)
        index += 1
