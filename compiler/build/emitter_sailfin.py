...
def emit_program(program):
    ...
    while True:
        ...
        if last_emitted_was_import  and  not is_import:
            builder = builder_emit_blank(builder)
        builder = emit_statement(builder, statement)
        last_emitted_was_import = is_import
        if index + 1 < len(program.statements):
            next_statement = program.statements[index + 1]
            next_is_import = is_import_like(next_statement)
            if not is_import  and  next_is_import:
                builder = builder_emit_blank(builder)
        index += 1
