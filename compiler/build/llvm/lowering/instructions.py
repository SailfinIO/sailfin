    ...
    while True:
        ...
        if instruction.variant == "Let":
            lowered = lower_let_instruction(function, instruction, current_bindings, current_locals, current_allocas, current_lines,
                                            current_temp, current_next_local, current_next_region, functions, context, scope_id, scope_depth, active_label)
            ...
            if instruction.variant == "Expression":
                ...
                if not handled_inline_let:
                    lowered = lower_expression_statement(function.name, instruction, trimmed_expression, current_bindings, current_locals,
                                                         current_temp, current_lines, current_next_region, scope_id, scope_depth, functions, context, active_label)
                    diagnostics = (diagnostics) + (lowered.diagnostics)
                    current_lines = lowered.lines
                    current_temp = lowered.temp_index
                    current_locals = lowered.locals
                ...
                if instruction.variant == "Return":
                    ...
                    if instruction.variant == "If":
                        ...
                        active_label = find_last_label(
                            current_lines, active_label)
                        if terminated:
                            break
                        continue
                    else:
                            ...
                            if instruction.variant == "Match":
                                ...
                                if instruction.variant == "Try":
                                    ...
                                    if instruction.variant == "Throw":
                                        ...
                                        if index < end:
                                            diagnostics = append_string(
                                                diagnostics, "llvm lowering: instructions after throw ignored in `" + function.name + "`")
                                        break
                                    else:
...
def lower_throw_instruction(function_name, instruction, llvm_return, bindings, locals, temp_index, lines, functions, context):
    ...
    if lowered_expr.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unsupported throw expression in `" + function_name + "`")
    ...
    coerced = coerce_operand_to_type(
        lowered_expr.operand, "i8*", current_temp, current_lines)
    diagnostics = (diagnostics) + (coerced.diagnostics)
    current_lines = coerced.lines
    current_temp = coerced.temp_index
    if coerced.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to coerce throw value to string in `" + function_name + "`")
        current_lines = append_string(current_lines, "  unreachable")
        return ThrowLoweringResult(lines=current_lines, temp_index=current_temp, diagnostics=diagnostics, terminated=True, string_constants=collected_string_constants)
    current_lines = append_string(
        current_lines, "  call void @sailfin_runtime_set_exception(i8* " + coerced.operand.value + ")")
    if llvm_return == "void":
...
def collect_try_structure(instructions, start_index, end, function_name):
    ...
    if end_index == -1:
        diagnostics = append_string(
            diagnostics, "llvm lowering: try without endtry in `" + function_name + "`")
        end_index = end - 1
...
def lower_try_instruction(function, start_index, llvm_return, bindings, locals, allocas, lines, temp_index, block_counter, next_local_id, next_region_id, functions, loop_stack, end, context, scope_id, scope_depth, current_label):
    diagnostics = []
    structure = collect_try_structure(
    ...
    collected_string_constants = empty_string_constant_set()
    current_lines = append_string(
        current_lines, "  call void @sailfin_runtime_clear_exception()")
    ...
    if structure.catch_index != -1:
        catch_label_alloc = allocate_block_label(
            "catch", current_block_counter)
        catch_label = catch_label_alloc.label
        current_block_counter = catch_label_alloc.next_counter
    finally_label = ""
    if structure.finally_index != -1:
        finally_label_alloc = allocate_block_label(
            "finally", current_block_counter)
    ...
    collected_string_constants = merge_string_constants(
        collected_string_constants, try_result.string_constants)
    if not try_result.terminated:
        has_exc = format_temp_name(current_temp)
        current_temp += 1
        current_lines = append_string(
            current_lines, "  " + has_exc + " = call i1 @sailfin_runtime_has_exception()")
        normal_target = merge_label
        if structure.finally_index != -1:
            normal_target = finally_label
        if structure.catch_index != -1:
            current_lines = append_string(
                current_lines, "  br i1 " + has_exc + ", label %" + catch_label + ", label %" + normal_target)
        else:
            current_lines = append_string(
                current_lines, "  br label %" + normal_target)
    if structure.catch_index != -1:
        ...
        current_temp += 1
        current_lines = append_string(
            current_lines, "  " + exception_temp + " = call i8* @sailfin_runtime_take_exception()")
        ...
        if len(trimmed_name) > 0 and trimmed_name != "_":
            ...
            current_allocas = append_string(
                current_allocas, "  " + err_slot + " = alloca i8*")
            current_lines = append_string(
                current_lines, "  store i8* " + exception_temp + ", i8** " + err_slot)
            catch_locals = (catch_locals) + ([LocalBinding(name=trimmed_name, pointer=err_slot, llvm_type="i8*",
                                                           type_annotation="string", ownership=None, consumed=False, scope_id=catch_scope_id, scope_depth=scope_depth + 1)])
        ...
        collected_string_constants = merge_string_constants(
            collected_string_constants, catch_result.string_constants)
        if not catch_result.terminated:
            target = merge_label
            if structure.finally_index != -1:
                target = finally_label
            current_lines = append_string(
                current_lines, "  br label %" + target)
    if structure.finally_index != -1:
        ...
        lifetime_regions = (lifetime_regions) + \
            (finally_result.lifetime_regions)
        ...
        collected_string_constants = merge_string_constants(
            collected_string_constants, finally_result.string_constants)
    ...
    current_temp += 1
    current_lines = append_string(
        current_lines, "  " + has_exc_after + " = call i1 @sailfin_runtime_has_exception()")
    propagate_label_alloc = allocate_block_label(
        "try.propagate", current_block_counter)
    propagate_label = propagate_label_alloc.label
    current_block_counter = propagate_label_alloc.next_counter
    continue_label_alloc = allocate_block_label(
        "try.continue", current_block_counter)
    continue_label = continue_label_alloc.label
    current_block_counter = continue_label_alloc.next_counter
    current_lines = append_string(current_lines, "  br i1 " + has_exc_after +
                                  ", label %" + propagate_label + ", label %" + continue_label)
...
def lower_match_instruction(function, start_index, llvm_return, bindings, locals, allocas, lines, temp_index, block_counter, next_local_id, next_region_id, functions, loop_stack, end, context, scope_id, scope_depth, current_label):
    ...
    subject_enum_info = find_enum_info_by_llvm_type(
        context, subject_operand.llvm_type)
    matched_enum_tags = []
    ...
    while True:
        ...
        if lowered_condition.union_variant_index >= 0 and not guard_has_text:
            ...
            if not seen:
                matched_union_variants = (
                    matched_union_variants) + ([lowered_condition.union_variant_index])
        ...
        if len(lowered_condition.field_bindings) > 0 and lowered_condition.enum_info != None and lowered_condition.variant_info != None and subject_operand != None:
            ...
            while True:
                ...
                current_lines = append_string(current_lines, "  " + byte_ptr_temp + " = bitcast [" + number_to_string(
                    enum_info_val.max_payload_size) + " x i8]* " + payload_ptr_temp + " to i8*")
        ...
        if lowered_condition.union_variant_index >= 0 and lowered_condition.union_struct_info != None and len(lowered_condition.union_struct_bindings) > 0:
            ...
            current_lines = append_string(current_lines, "  " + payload_temp + " = extractvalue " +
                                          subject_operand.llvm_type + " " + subject_operand.value + ", " + number_to_string(payload_index))
            ...
            if payload_type == "i8*":
                ...
                current_temp += 1
                current_lines = append_string(
                    current_lines, "  " + cast_temp + " = bitcast i8* " + payload_temp + " to " + expected_ptr_type)
            ...
            while True:
                ...
                current_temp += 1
                current_lines = append_string(current_lines, "  " + gep_temp + " = getelementptr " + struct_info_val.llvm_name +
                                              ", " + expected_ptr_type + " " + typed_payload_ptr + ", i32 0, i32 " + number_to_string(binding.field_index))
                load_temp = format_temp_name(current_temp)
                current_temp += 1
                current_lines = append_string(
                    current_lines, "  " + load_temp + " = load " + binding.field_type + ", " + binding.field_type + "* " + gep_temp)
                local_alloca_temp = format_temp_name(current_temp)
                current_temp += 1
                current_lines = append_string(
                    current_lines, "  " + local_alloca_temp + " = alloca " + binding.field_type)
                current_lines = append_string(current_lines, "  store " + binding.field_type +
                                              " " + load_temp + ", " + binding.field_type + "* " + local_alloca_temp)
                base_locals = (base_locals) + ([LocalBinding(name=binding.field_name, pointer=local_alloca_temp, llvm_type=binding.field_type, type_annotation="",
                                                             ownership=None, consumed=False, scope_id=make_child_scope_id(scope_id, body_labels[index]), scope_depth=scope_depth + 1)])
                base_local_id += 1
                binding_idx += 1
        if lowered_condition.is_default and len(union_payload_types) > 0 and len(subject_name) > 0:
            ...
            if len(remaining) == 1:
                ...
                current_temp += 1
                current_lines = append_string(
                    current_lines, "  " + local_alloca_temp + " = alloca " + payload_type)
                current_lines = append_string(current_lines, "  store " + payload_type +
                                              " " + payload_temp + ", " + payload_type + "* " + local_alloca_temp)
                base_locals = (base_locals) + ([LocalBinding(name=subject_name, pointer=local_alloca_temp, llvm_type=payload_type, type_annotation="",
                                                             ownership=None, consumed=False, scope_id=make_child_scope_id(scope_id, body_labels[index]), scope_depth=scope_depth + 1)])
                base_local_id += 1
        body_result = lower_instruction_range(
            function,
    ...
    if not match_exhaustive and not has_guarded_cases:
        if subject_enum_info != None:
            match_exhaustive = len(matched_enum_tags) == len(
                subject_enum_info.variants)
        else:
            if len(union_payload_types) > 0:
                match_exhaustive = len(matched_union_variants) == len(
                    union_payload_types)
    ...
    if not case.is_default:
        ...
        if enum_parse.recognized and enum_parse.success:
            ...
            if is_union_llvm_type(subject_operand.llvm_type):
                union_parts = parse_union_payload_types(
                ...
                if struct_parse.recognized and struct_parse.success:
                    info = find_struct_info_by_name(
                        context, struct_parse.type_name)
                    if info != None:
                        idx = 0
                        found_index = -1
                        while True:
                        ...
                        if found_index >= 0:
                            ...
                            current_lines = append_string(
                                current_lines, "  " + tag_temp + " = extractvalue " + subject_operand.llvm_type + " " + subject_operand.value + ", 0")
                            tag_operand = LLVMOperand(
                                llvm_type="i32", value=tag_temp)
                            expected_tag = LLVMOperand(
                                llvm_type="i32", value=number_to_string(found_index))
                            comparison = emit_comparison_instruction(
                                "==", tag_operand, expected_tag, current_temp, current_lines)
                            diagnostics = (diagnostics) + \
                            ...
                            while True:
                                ...
                                if len(pattern_field.value) == 0:
                                    ...
                                    while True:
                                        ...
                                        sfi += 1
                                    if field_info != None:
                                        union_struct_bindings = (union_struct_bindings) + ([MatchStructFieldBinding(
                                            field_name=pattern_field.name, field_type=field_info.llvm_type, field_index=field_info.index)])
                                    else:
                                        diagnostics = append_string(
                                            diagnostics, "llvm lowering: match pattern field `" + pattern_field.name + "` not found in struct `" + struct_parse.type_name + "`")
                                else:
                                    diagnostics = append_string(
                                        diagnostics, "llvm lowering: match struct field literal patterns are not supported for `" + pattern_field.name + "`")
                                fidx += 1
                            union_variant_index = found_index
                            union_struct_info = info
                        else:
                            diagnostics = append_string(diagnostics, "llvm lowering: match struct pattern `" +
                                                        struct_parse.type_name + "` does not match union type `" + subject_operand.llvm_type + "`")
                    else:
                        diagnostics = append_string(
                            diagnostics, "llvm lowering: match pattern references unknown struct `" + struct_parse.type_name + "`")
...
def lower_condition_to_i1(function_name, expression, bindings, locals, temp_index, lines, functions, context):
    ...
    if lowered.operand == None:
        ...
        if len(rendered_condition) == 0:
            ...
            diagnostics = append_string(
                diagnostics, "llvm lowering: condition produced no value in `" + function_name + "` for `" + rendered_condition + "`")
...
def lower_if_instruction(function, start_index, llvm_return, bindings, locals, allocas, lines, temp_index, block_counter, next_local_id, next_region_id, functions, loop_stack, end, context, scope_id, scope_depth, current_label):
    ...
    if condition.operand == None:
        ...
        if len(rendered_condition) == 0:
            ...
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to lower if condition in `" + function.name + "` for `" + rendered_condition + "`")
