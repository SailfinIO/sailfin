...
from compiler.build.llvm.expression_lowering_stage2.core_helpers import collect_parameter_types, operation_name_for_symbol
...
def lower_expression(expression, bindings, locals, temp_index, lines, functions, context, expected_type):
    ...
    if is_number_literal(stripped):
        ...
        if trimmed_expected == "i64" or trimmed_expected == "i32" and is_integer_literal(stripped):
            operand = LLVMOperand(
                llvm_type=trimmed_expected, value=trim_text(stripped))
            return ExpressionResult(lines=lines, temp_index=temp_index, operand=operand, diagnostics=diagnostics, string_constants=empty_constants)
    ...
    if is_string_literal(stripped):
        interpolated = try_lower_interpolated_string_literal(
            stripped, bindings, locals, temp_index, lines, functions, context)
    ...
    if starts_with(stripped, "await"):
        if len(stripped) == 5:
            diagnostics = append_string(
                diagnostics, "llvm lowering: await expression missing operand")
            return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
        ...
        if next == " " or next == "\t" or next == "(":
            ...
            lowered = lower_expression(
                remainder, bindings, locals, temp_index, lines, functions, context, expected_type)
            ...
            await_symbol = await_symbol_for_future_pointer_type(
                future_operand.llvm_type)
            if len(await_symbol) == 0:
                with_issue = append_string(
                    combined, "llvm lowering: await expects a Future pointer, got `" + future_operand.llvm_type + "`")
            ...
            if future_operand.llvm_type == "%SailfinFutureVoid*":
                awaited_lines = append_string(
                    lowered.lines, "  call void @" + await_symbol + "(%SailfinFutureVoid* " + future_operand.value + ")")
            ...
            awaited_call = "  " + result_temp + " = call " + result_type + " @" + \
                await_symbol + "(" + future_operand.llvm_type + \
            ...
            if future_operand.llvm_type == "%SailfinFuturePtr*" and len(trimmed_expected) > 0 and trimmed_expected != "i8*" and not ends_with_pointer_suffix(trimmed_expected):
                ...
                awaited_lines2 = append_string(
                    awaited_lines2, "  " + typed_ptr_temp + " = bitcast i8* " + result_temp + " to " + trimmed_expected + "*")
                awaited_lines2 = append_string(awaited_lines2, "  " + load_temp + " = load " +
                                               trimmed_expected + ", " + trimmed_expected + "* " + typed_ptr_temp)
                awaited_lines2 = append_string(
                    awaited_lines2, "  call void @free(i8* " + result_temp + ")")
                out_operand_unboxed = LLVMOperand(
                    llvm_type=trimmed_expected, value=load_temp)
            ...
            out_operand2 = LLVMOperand(
                llvm_type=result_type, value=result_temp)
    ...
    if len(stripped) > 0:
        ...
        if char_code_at_text(stripped, 0) == char_code("*"):
            ...
            if len(operand_text) == 0:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: deref expression missing operand")
                return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
            lowered_ptr = lower_expression(
                operand_text, bindings, locals, temp_index, lines, functions, context, "")
            ...
            if not ends_with_pointer_suffix(ptr_operand.llvm_type):
                with_issue = append_string(
                    combined, "llvm lowering: cannot dereference non-pointer type `" + ptr_operand.llvm_type + "`")
            ...
            updated_lines = append_string(lowered_ptr.lines, "  " + load_name +
                                          " = load " + element_type + ", " + element_type + "* " + ptr_operand.value)
    ...
    diagnostics = append_string(
        diagnostics, "llvm lowering: unable to lower expression `" + expression + "`")
    return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
...
def lower_borrow_expression(parse, bindings, locals, temp_index, lines):
    ...
    if local == None:
        ...
        if parameter == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: borrow target `" + target + "` not found")
            return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
        if ends_with(parameter.llvm_type, "*"):
            operand = LLVMOperand(
                llvm_type=parameter.llvm_type, value=parameter.llvm_name)
            return ExpressionResult(lines=lines, temp_index=temp_index, operand=operand, diagnostics=diagnostics, string_constants=empty_constants)
        diagnostics = append_string(
            diagnostics, "llvm lowering: cannot borrow non-addressable parameter `" + target + "` (type `" + parameter.llvm_type + "`)")
        return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
    ...
    if match.symbol == "+":
        ...
        if lhs.llvm_type == "i8*" and rhs.llvm_type == "i8":
            ...
            coerced = coerce_operand_to_type(
                rhs, "i8*", current_temp, current_lines)
            ...
            current_temp += 1
            current_lines = append_string(current_lines, "  " + concat_name +
                                          " = call i8* @sailfin_runtime_string_concat(i8* " + lhs.value + ", i8* " + coerced.operand.value + ")")
        ...
        if lhs.llvm_type == "i8" and rhs.llvm_type == "i8*":
            ...
            current_temp = right_result.temp_index
            coerced = coerce_operand_to_type(
                lhs, "i8*", current_temp, current_lines)
            diagnostics = (diagnostics) + (coerced.diagnostics)
            current_lines = coerced.lines
            current_temp = coerced.temp_index
            if coerced.operand == None:
            ...
            current_temp += 1
            current_lines = append_string(current_lines, "  " + concat_name +
                                          " = call i8* @sailfin_runtime_string_concat(i8* " + coerced.operand.value + ", i8* " + rhs.value + ")")
            operand = LLVMOperand(llvm_type="i8*", value=concat_name)
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=operand, diagnostics=diagnostics, string_constants=string_constants)
        if lhs.llvm_type == "i8" and rhs.llvm_type == "i8":
            current_lines = right_result.lines
            current_temp = right_result.temp_index
            lhs_coerced = coerce_operand_to_type(
                lhs, "i8*", current_temp, current_lines)
            ...
            if lhs_coerced.operand == None:
                return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=string_constants)
            rhs_coerced = coerce_operand_to_type(
                rhs, "i8*", current_temp, current_lines)
            ...
            concat_name = format_temp_name(current_temp)
            current_temp += 1
            current_lines = append_string(current_lines, "  " + concat_name + " = call i8* @sailfin_runtime_string_concat(i8* " +
                                          lhs_coerced.operand.value + ", i8* " + rhs_coerced.operand.value + ")")
            operand = LLVMOperand(llvm_type="i8*", value=concat_name)
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=operand, diagnostics=diagnostics, string_constants=string_constants)
            ...
            if not ends_with_pointer_suffix(rhs.llvm_type):
                if rhs.llvm_type == "double" or starts_with(rhs.llvm_type, "i") or rhs.llvm_type == "int" and rhs.llvm_type != "i8" and rhs.llvm_type != "i1":
                    ...
                    if rhs.llvm_type != "i64":
                        coerced = coerce_operand_to_type(
                            rhs, "i64", current_temp, current_lines)
                        diagnostics = (diagnostics) + (coerced.diagnostics)
                        current_lines = coerced.lines
                        current_temp = coerced.temp_index
                        if coerced.operand == None:
                    ...
                    if match.symbol == "-":
                        ...
                        current_temp += 1
                        current_lines = append_string(
                            current_lines, "  " + neg_name + " = sub i64 0, " + rhs_i64.value)
                        index_value = neg_name
                    element_type = strip_pointer_suffix(left_type)
                    gep_name = format_temp_name(current_temp)
                    current_temp += 1
                    current_lines = append_string(current_lines, "  " + gep_name + " = getelementptr " +
                                                  element_type + ", " + left_type + " " + left_result.operand.value + ", i64 " + index_value)
...
def lower_comparison_operation(expression, match, bindings, locals, temp_index, lines, functions, context):
    ...
    if is_equality and left_operand != None and right_operand != None:
        if left_operand.llvm_type == right_operand.llvm_type:
            enum_info = find_enum_info_by_llvm_type(
                context, left_operand.llvm_type)
            if enum_info != None:
                tag_llvm_type = "i32"
                ...
                alignment_temp += 1
                alignment_lines = append_string(
                    alignment_lines, "  " + left_tag_temp + " = extractvalue " + left_operand.llvm_type + " " + left_operand.value + ", 0")
                right_tag_temp = format_temp_name(alignment_temp)
                alignment_temp += 1
                alignment_lines = append_string(alignment_lines, "  " + right_tag_temp +
                                                " = extractvalue " + right_operand.llvm_type + " " + right_operand.value + ", 0")
                ...
                if len(match.symbol) == 2:
                    ...
                    if c0 == char_code("!"):
                        predicate = "icmp ne"
                alignment_lines = append_string(alignment_lines, "  " + compare_temp + " = " +
                                                predicate + " " + tag_llvm_type + " " + left_tag_temp + ", " + right_tag_temp)
        ...
        if left_is_char and right_is_pointer:
            coercion = coerce_operand_to_type(
                right_operand, "i8", alignment_temp, alignment_lines)
            ...
            if right_is_char and left_is_pointer:
                coercion = coerce_operand_to_type(
                    left_operand, "i8", alignment_temp, alignment_lines)
                diagnostics = (diagnostics) + (coercion.diagnostics)
                if coercion.operand == None:
                    return ExpressionResult(lines=coercion.lines, temp_index=coercion.temp_index, operand=None, diagnostics=diagnostics, string_constants=string_constants)
                ...
                alignment_temp = coercion.temp_index
    harmonised = harmonise_operands(
        left_operand, right_operand, alignment_temp, alignment_lines)
    diagnostics = (diagnostics) + (harmonised.diagnostics)
    if harmonised.left == None or harmonised.right == None:
        return ExpressionResult(lines=harmonised.lines, temp_index=harmonised.temp_index, operand=None, diagnostics=diagnostics, string_constants=string_constants)
...
def lower_logical_and(expression, match, bindings, locals, temp_index, lines, functions, context):
    ...
    if right_result.operand == None:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unable to lower && RHS in `" + expression + "`")
        fallback_lines = right_result.lines
        if not lines_end_with_terminator(fallback_lines):
            fallback_lines = append_string(
                fallback_lines, "  br label %" + merge_label)
...
def lower_call_expression(target, arguments, bindings, locals, temp_index, lines, functions, context):
    ...
    if ends_with(canonical_fused, "concat") and len(canonical_fused) > 6:
        receiver_name = trim_text(
            substring(canonical_fused, 0, len(canonical_fused) - 6))
        if len(receiver_name) > 0 and is_simple_identifier(receiver_name):
            ...
            if local != None:
                load_result = load_local_operand(
                    local, current_temp, current_lines)
                diagnostics = (diagnostics) + (load_result.diagnostics)
            ...
            if receiver_operand != None:
                ...
                if len(element_type) > 0:
                    ...
                    if not ends_with_pointer_suffix(element_type):
                        helper_descriptor = RuntimeHelperDescriptor(target="concat", symbol="", return_type=receiver_operand.llvm_type, parameter_types=[
                                                                    receiver_operand.llvm_type, receiver_operand.llvm_type], effects=[])
            ...
            if len(array_element_type) > 0:
                if method_parse.field == "concat":
                    ...
                    if not ends_with_pointer_suffix(array_element_type):
                        helper_descriptor = RuntimeHelperDescriptor(target="concat", symbol="", return_type=base_operand.llvm_type, parameter_types=[
                                                                    base_operand.llvm_type, base_operand.llvm_type], effects=[])
                else:
                    method_operand = base_operand
                    trimmed_target = "push"
                    injected_argument_count = 1
                    ...
                    is_array_push = True
                    helper_descriptor = RuntimeHelperDescriptor(target="push", symbol="", return_type=base_operand.llvm_type, parameter_types=[
                                                                base_operand.llvm_type, array_element_type], effects=[])
        ...
        if helper_descriptor != None:
            ...
            if function_entry != None:
                if function_entry.is_async:
                    future_return = future_pointer_type_for_return_type(
                        function_entry.return_type)
                    if len(future_return) == 0:
                        diagnostics = append_string(diagnostics, "llvm lowering: async function `" +
                                                    trimmed_target + "` has unsupported return type `" + function_entry.return_type + "`")
                        llvm_return = "i8*"
                    else:
                        llvm_return = future_return
                    expected_params = collect_parameter_types(
                        context, function_entry.parameters, trimmed_target)
                else:
                    llvm_return = map_return_type(
                        context, function_entry.return_type)
                    if len(llvm_return) == 0:
                        diagnostics = append_string(
        ...
        if len(element_type) > 0:
            ...
            if not ends_with_pointer_suffix(element_type):
                ...
                helper_descriptor = RuntimeHelperDescriptor(
                    target="concat", symbol="", return_type=receiver_type, parameter_types=expected_params, effects=[])
                function_entry = None
    index = 0
    while True:
    ...
    if trimmed_target == "concat" and len(operands) >= 2:
        inferred_element_type = array_pointer_element_type(
            operands[0].llvm_type)
        if len(inferred_element_type) > 0:
            ...
            if not ends_with_pointer_suffix(inferred_element_type):
                ...
                expected_params = [
                    operands[0].llvm_type, operands[0].llvm_type]
                helper_descriptor = RuntimeHelperDescriptor(
                    target="concat", symbol="", return_type=operands[0].llvm_type, parameter_types=expected_params, effects=[])
    if function_entry != None and len(operands) < len(expected_params):
        ...
        while True:
            ...
            lowered_default = lower_expression(
                param.default_value, bindings, locals, current_temp, current_lines, functions, context, expected_type)
            diagnostics = (diagnostics) + (lowered_default.diagnostics)
            collected_string_constants = merge_string_constants(
                collected_string_constants, lowered_default.string_constants)
            current_lines = lowered_default.lines
            current_temp = lowered_default.temp_index
            if lowered_default.operand == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: failed to lower default argument for parameter `" + param.name + "` in call to `" + trimmed_target + "`")
                break
            ...
            if self_operand.llvm_type != expected_self_type:
                if ends_with_pointer_suffix(self_operand.llvm_type):
                    ...
                    if ends_with_pointer_suffix(expected_self_type):
                        base_name = trim_text(method_parse.base)
                        if is_simple_identifier(base_name):
                            local_pointer = find_local_binding(
                                locals, base_name)
                            if local_pointer != None and local_pointer.llvm_type + "*" == expected_self_type:
                                updated_operand = LLVMOperand(
                                    llvm_type=expected_self_type, value=local_pointer.pointer)
                                operands = replace_llvm_operand(
                                    operands, 0, updated_operand)
                            else:
                                parameter_pointer = find_parameter_binding(
                                    bindings, base_name)
                                if parameter_pointer != None and parameter_pointer.llvm_type == expected_self_type:
                                    updated_operand = LLVMOperand(
                                        llvm_type=expected_self_type, value=parameter_pointer.llvm_name)
                                    operands = replace_llvm_operand(
                                        operands, 0, updated_operand)
                                else:
                            ...
                            receiver_base = strip_pointer_suffix(
                                expected_self_type)
    ...
    if is_array_concat and len(array_concat_element_type) > 0 and not ends_with_pointer_suffix(array_concat_element_type):
        if len(operands) >= 2:
            concat_result = lower_struct_array_concat(
                operands[0], operands[1], array_concat_element_type, current_lines, current_temp)
            ...
            diagnostics = append_string(
                diagnostics, "llvm lowering: concat requires two operands")
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    if is_array_push and len(array_push_element_type) > 0:
        if len(operands) >= 2:
            push_result = lower_array_push_in_place(
                operands[0], operands[1], array_push_element_type, current_lines, current_temp)
            ...
            diagnostics = append_string(
                diagnostics, "llvm lowering: push requires two operands")
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
            ...
            if len(operands) > 0:
                ...
                if interface_info != None:
                    ...
                    if method_index >= 0:
                        ...
                        current_temp += 1
                        current_lines = append_string(
                            current_lines, "  " + vtable_slots_temp + " = bitcast i8* " + vtable_ptr_temp + " to i8**")
                        slot_ptr_temp = "%t" + number_to_string(current_temp)
                        current_temp += 1
                        current_lines = append_string(
                            current_lines, "  " + slot_ptr_temp + " = getelementptr i8*, i8** " + vtable_slots_temp + ", i64 " + number_to_string(method_index))
                        raw_ptr_temp = "%t" + number_to_string(current_temp)
                        current_temp += 1
                        current_lines = append_string(
                            current_lines, "  " + raw_ptr_temp + " = load i8*, i8** " + slot_ptr_temp)
                        method_ptr_temp = "%t" + number_to_string(current_temp)
                        current_temp += 1
                        current_lines = append_string(
                            current_lines, "  " + method_ptr_temp + " = bitcast i8* " + raw_ptr_temp + " to " + function_type)
...
def lower_member_access(parse, bindings, locals, temp_index, lines, functions, context, expected_type):
    ...
    if pointer_available:
        ...
        if field_info.llvm_type == "i8*" and len(field_info.type_annotation) > 0:
            ...
            if enum_info != None:
                ...
                current_lines = append_string(
                    current_lines, "  " + cast_name + " = bitcast i8* " + load_name + " to " + enum_info.llvm_name + "*")
                current_temp += 1
                typed_operand = LLVMOperand(
                    llvm_type=enum_info.llvm_name + "*", value=cast_name)
            ...
            struct_info_candidate = find_struct_info_by_name(
                context, base_type)
            if struct_info_candidate != None:
                cast_name = format_temp_name(current_temp)
                current_lines = append_string(
                    current_lines, "  " + cast_name + " = bitcast i8* " + load_name + " to " + struct_info_candidate.llvm_name + "*")
                current_temp += 1
                typed_operand = LLVMOperand(
                    llvm_type=struct_info_candidate.llvm_name + "*", value=cast_name)
                return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=typed_operand, diagnostics=diagnostics, string_constants=collected_string_constants)
    ...
    if field_info.llvm_type == "i8*" and len(field_info.type_annotation) > 0:
        ...
        if enum_info != None:
            cast_name = format_temp_name(current_temp)
            current_lines = append_string(
                current_lines, "  " + cast_name + " = bitcast i8* " + extract_name + " to " + enum_info.llvm_name + "*")
            current_temp += 1
            typed_operand = LLVMOperand(
                llvm_type=enum_info.llvm_name + "*", value=cast_name)
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=typed_operand, diagnostics=diagnostics, string_constants=collected_string_constants)
        ...
        if struct_info_candidate != None:
            cast_name = format_temp_name(current_temp)
            current_lines = append_string(current_lines, "  " + cast_name + " = bitcast i8* " +
                                          extract_name + " to " + struct_info_candidate.llvm_name + "*")
            current_temp += 1
            typed_operand = LLVMOperand(
                llvm_type=struct_info_candidate.llvm_name + "*", value=cast_name)
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=typed_operand, diagnostics=diagnostics, string_constants=collected_string_constants)
...
def lower_enum_member_access(parse, enum_info, base_operand, pointer_available, temp_index, lines, diagnostics, string_constants, expected_type):
        ...
        if candidate_field != None:
            ...
            if len(resolved_field.llvm_type) == 0:
                ...
                if layout_annotation_represents_user_value(resolved_field.type_annotation) and ends_with_pointer_suffix(mapped):
                    ...
                    if len(stripped) > 0:
                        mapped = stripped
                resolved_field = StructFieldInfo(name=resolved_field.name, llvm_type=mapped,
                                                 type_annotation=resolved_field.type_annotation, index=resolved_field.index, offset=resolved_field.offset)
    ...
    while True:
        ...
        current_lines = append_string(current_lines, "  " + byte_ptr_temp + " = bitcast [" + number_to_string(
            enum_info.max_payload_size) + " x i8]* " + payload_ptr_temp + " to i8*")
        current_temp += 1
...
def lower_index_expression(parse, bindings, locals, temp_index, lines, functions, context):
    ...
    if len(element_type) == 0:
        ...
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
    if is_string:
        index_as_double = coerce_operand_to_type(
            index_operand, "double", current_temp, current_lines)
        ...
        if index_as_double.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to coerce string index expression to double")
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        call_temp = format_temp_name(current_temp)
        current_lines = append_string(
            current_lines,
            "  " + call_temp +
            " = call i8* @sailfin_runtime_grapheme_at(i8* " + base_operand.value +
            ", double " + index_as_double.operand.value + ")"
        )
        current_temp += 1
        operand = LLVMOperand(llvm_type="i8*", value=call_temp)
    ...
    if is_heap_array:
        ...
        if is_string:
            ...
            current_lines = append_string(
                current_lines, "  " + char_ptr + " = getelementptr i8, i8* " + base_operand.value + ", i64 " + coerced_index.value)
            current_temp += 1
            ch_value = format_temp_name(current_temp)
            current_lines = append_string(
                current_lines, "  " + ch_value + " = load i8, i8* " + char_ptr)
            current_temp += 1
            buf = format_temp_name(current_temp)
            current_lines = append_string(
                current_lines, "  " + buf + " = call i8* @malloc(i64 2)")
            current_temp += 1
            buf0 = format_temp_name(current_temp)
            current_lines = append_string(
                current_lines, "  " + buf0 + " = getelementptr i8, i8* " + buf + ", i64 0")
            current_temp += 1
            current_lines = append_string(
                current_lines, "  store i8 " + ch_value + ", i8* " + buf0)
            buf1 = format_temp_name(current_temp)
            current_lines = append_string(
                current_lines, "  " + buf1 + " = getelementptr i8, i8* " + buf + ", i64 1")
            current_temp += 1
            current_lines = append_string(
                current_lines, "  store i8 0, i8* " + buf1)
...
def lower_array_literal(text, bindings, locals, temp_index, lines, functions, context, expected_type):
    ...
    if trimmed_expected == "i8*":
        ...
        while True:
            ...
            if len(element_text) == 0:
                ...
                continue
            if index > 0:
                sep = lower_string_literal(
                    "\", \"", current_temp, current_lines)
                current_lines = sep.lines
                current_temp = sep.temp_index
                if sep.operand == None:
                    return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
                with_sep = emit_string_concat(
                    current_operand, sep.operand, current_temp, current_lines)
            ...
            if lowered.operand == None:
                ...
                return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
            as_string = coerce_operand_to_string(
                lowered.operand, current_temp, current_lines)
            diagnostics = (diagnostics) + (as_string.diagnostics)
            collected_string_constants = merge_string_constants(
                collected_string_constants, as_string.string_constants)
            current_lines = as_string.lines
            current_temp = as_string.temp_index
            if as_string.operand == None:
                diagnostics = append_string(
                    diagnostics, "llvm lowering: unable to stringify array literal element")
                return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
            appended = emit_string_concat(
                current_operand, as_string.operand, current_temp, current_lines)
        ...
        if close.operand == None:
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        finished = emit_string_concat(
            current_operand, close.operand, current_temp, current_lines)
    ...
    current_lines = append_string(current_lines, "  " + array_size_ptr +
                                  " = getelementptr " + array_type + ", " + array_type + "* null, i32 1")
    current_temp += 1
    array_bytes = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + array_bytes +
                                  " = ptrtoint " + array_type + "* " + array_size_ptr + " to i64")
    current_temp += 1
    array_bytes_is_zero = format_temp_name(current_temp)
    current_lines = append_string(
        current_lines, "  " + array_bytes_is_zero + " = icmp eq i64 " + array_bytes + ", 0")
    current_temp += 1
    adjusted_array_bytes = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + adjusted_array_bytes +
                                  " = select i1 " + array_bytes_is_zero + ", i64 1, i64 " + array_bytes)
    current_temp += 1
    raw_array_ptr = format_temp_name(current_temp)
    current_lines = append_string(
        current_lines, "  " + raw_array_ptr + " = call i8* @malloc(i64 " + adjusted_array_bytes + ")")
    current_temp += 1
    data_pointer = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + data_pointer +
                                  " = bitcast i8* " + raw_array_ptr + " to " + element_type + "*")
    current_temp += 1
    index = 0
    while True:
        if index >= len(operands):
    ...
    current_lines = append_string(current_lines, "  " + struct_size_ptr +
                                  " = getelementptr " + struct_type + ", " + struct_type + "* null, i32 1")
    current_temp += 1
    struct_bytes = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + struct_bytes +
                                  " = ptrtoint " + struct_type + "* " + struct_size_ptr + " to i64")
    current_temp += 1
    raw_struct_ptr = format_temp_name(current_temp)
    current_lines = append_string(
        current_lines, "  " + raw_struct_ptr + " = call i8* @malloc(i64 " + struct_bytes + ")")
    current_temp += 1
    struct_pointer = format_temp_name(current_temp)
    current_lines = append_string(current_lines, "  " + struct_pointer +
                                  " = bitcast i8* " + raw_struct_ptr + " to " + struct_type + "*")
    current_temp += 1
    ...
    current_lines = append_string(current_lines, "  " + data_field_pointer + " = getelementptr " +
                                  struct_type + ", " + struct_type + "* " + struct_pointer + ", i32 0, i32 0")
    current_temp += 1
    ...
    current_lines = append_string(current_lines, "  " + length_field_pointer + " = getelementptr " +
                                  struct_type + ", " + struct_type + "* " + struct_pointer + ", i32 0, i32 1")
    current_temp += 1
    current_lines = append_string(
...
def lower_cast_expression(parse, bindings, locals, temp_index, lines, functions, context):
    ...
    if len(target_type) == 0:
        diagnostics = append_string(
            diagnostics, "llvm lowering: unsupported cast target type `" + parse.type_annotation + "`")
        return ExpressionResult(lines=lines, temp_index=temp_index, operand=None, diagnostics=diagnostics, string_constants=empty_constants)
    lowered = lower_expression(
        parse.value, bindings, locals, temp_index, lines, functions, context, "")
    diagnostics = (diagnostics) + (lowered.diagnostics)
    if lowered.operand == None:
    ...
    if ends_with_pointer_suffix(operand.llvm_type) and ends_with_pointer_suffix(target_type):
        current_lines = append_string(current_lines, "  " + cast_name + " = bitcast " +
    ...
    if not ends_with_pointer_suffix(operand.llvm_type) and ends_with_pointer_suffix(target_type) and starts_with(operand.llvm_type, "i"):
        current_lines = append_string(current_lines, "  " + cast_name + " = inttoptr " +
                                      operand.llvm_type + " " + operand.value + " to " + target_type)
        casted = LLVMOperand(llvm_type=target_type, value=cast_name)
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=casted, diagnostics=diagnostics, string_constants=lowered.string_constants)
    if ends_with_pointer_suffix(operand.llvm_type) and not ends_with_pointer_suffix(target_type) and starts_with(target_type, "i"):
        current_lines = append_string(current_lines, "  " + cast_name + " = ptrtoint " +
                                      operand.llvm_type + " " + operand.value + " to " + target_type)
        casted = LLVMOperand(llvm_type=target_type, value=cast_name)
        return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=casted, diagnostics=diagnostics, string_constants=lowered.string_constants)
    diagnostics = append_string(diagnostics, "llvm lowering: unsupported cast from `" +
...
def try_lower_interpolated_string_literal(literal, bindings, locals, temp_index, lines, functions, context):
    ...
    while True:
        ...
        lowered_expr = lower_expression(
            expr_text, bindings, locals, current_temp, current_lines, functions, context, "i8*")
        diagnostics = (diagnostics) + (lowered_expr.diagnostics)
        current_lines = lowered_expr.lines
        current_temp = lowered_expr.temp_index
        collected_string_constants = merge_string_constants(
            collected_string_constants, lowered_expr.string_constants)
        if lowered_expr.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to lower interpolated expression `" + expr_text + "`")
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        coerced = coerce_operand_to_string(
            lowered_expr.operand, current_temp, current_lines)
        diagnostics = (diagnostics) + (coerced.diagnostics)
        current_lines = coerced.lines
        current_temp = coerced.temp_index
        collected_string_constants = merge_string_constants(
            collected_string_constants, coerced.string_constants)
        if coerced.operand == None:
            diagnostics = append_string(
                diagnostics, "llvm lowering: unable to stringify interpolated expression `" + expr_text + "`")
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        concat1 = emit_string_concat(
            current_operand, coerced.operand, current_temp, current_lines)
        ...
        part_literal = encode_string_literal_for_sailfin(
            parsed.parts[index + 1])
        part_value = lower_string_literal(
            part_literal, current_temp, current_lines)
        current_lines = part_value.lines
        current_temp = part_value.temp_index
        if part_value.operand == None:
            return ExpressionResult(lines=current_lines, temp_index=current_temp, operand=None, diagnostics=diagnostics, string_constants=collected_string_constants)
        concat2 = emit_string_concat(
            current_operand, part_value.operand, current_temp, current_lines)
