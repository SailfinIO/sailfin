...
- Fix bugs in the compiler source, never by adding fixup passes to the build driver. (The prior `scripts/build.sh` orchestrator was retired in Stage E PR7 / #383; the same rule applies to the driver in `compiler/src/cli/` + `compiler/src/capsule_resolver.sfn`.)
