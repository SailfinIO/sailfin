...
NativeArtifact = getattr(__module_1, 'NativeArtifact')
...
class NativeParameter:
    def __init__(self, name, type_annotation, mutable, default_value):
        self.name = name
...
class EnumParseResult:
    def __init__(self, definition, next_index, diagnostics):
...
def parse_native_artifact(text):
    ...
    while True:
        ...
        if starts_with(line, '.struct '):
            ...
            diagnostics = list(diagnostics) + list(struct_result.diagnostics)
        ...
        if starts_with(line, '.enum '):
            ...
            diagnostics = list(diagnostics) + list(enum_result.diagnostics)
...
def append_function(functions, value):
    return list(functions) + list([value])

def append_import(imports, value):
    return list(imports) + list([value])

def append_struct(structs, value):
    return list(structs) + list([value])

def append_enum(enums, value):
    return list(enums) + list([value])

def append_enum_variant(variants, value):
    return list(variants) + list([value])

def append_enum_variant_field(fields, value):
    return list(fields) + list([value])
...
def append_instruction(function, instruction):
    instructions = list(function.instructions) + list([instruction])
...
def parse_inline_case_instruction(line):
    ...
    instructions = []
    instructions = list(instructions) + list([NativeInstruction.Case(pattern=split.pattern, guard=split.guard)])
    if (len(remainder) == 0):
        instructions = list(instructions) + list([NativeInstruction.Noop()])
        return instructions
    body_instruction = parse_inline_case_body_instruction(remainder)
    instructions = list(instructions) + list([body_instruction])
    return instructions
...
def append_parameter_array(values, parameter):
    return list(values) + list([parameter])
