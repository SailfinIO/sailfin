...
__module_1 = importlib.import_module('compiler.build.lexer')
lex = getattr(__module_1, 'lex')
__module_2 = importlib.import_module('compiler.build.token')
Token = getattr(__module_2, 'Token')
TokenKind = getattr(__module_2, 'TokenKind')
__module_3 = importlib.import_module('compiler.build.ast')
Block = getattr(__module_3, 'Block')
Decorator = getattr(__module_3, 'Decorator')
Expression = getattr(__module_3, 'Expression')
FieldDeclaration = getattr(__module_3, 'FieldDeclaration')
FunctionSignature = getattr(__module_3, 'FunctionSignature')
EnumVariant = getattr(__module_3, 'EnumVariant')
MethodDeclaration = getattr(__module_3, 'MethodDeclaration')
ModelDeclaration = getattr(__module_3, 'ModelDeclaration')
ModelProperty = getattr(__module_3, 'ModelProperty')
ObjectField = getattr(__module_3, 'ObjectField')
Parameter = getattr(__module_3, 'Parameter')
PipelineDeclaration = getattr(__module_3, 'PipelineDeclaration')
ForClause = getattr(__module_3, 'ForClause')
Program = getattr(__module_3, 'Program')
Statement = getattr(__module_3, 'Statement')
TypeParameter = getattr(__module_3, 'TypeParameter')
TestDeclaration = getattr(__module_3, 'TestDeclaration')
TypeAnnotation = getattr(__module_3, 'TypeAnnotation')
ToolDeclaration = getattr(__module_3, 'ToolDeclaration')
MatchCase = getattr(__module_3, 'MatchCase')
ElseBranch = getattr(__module_3, 'ElseBranch')
WithClause = getattr(__module_3, 'WithClause')
__module_4 = importlib.import_module('compiler.build.decorator_semantics')
evaluate_decorators = getattr(__module_4, 'evaluate_decorators')
infer_effects = getattr(__module_4, 'infer_effects')
class Parser:
...
class StructFieldParseResult:
    def __init__(self, parser, field, success):
        self.parser = parser
        self.field = field
        self.success = success

class ModelPropertyParseResult:
    def __init__(self, parser, property, success):
        self.parser = parser
        self.property = property
        self.success = success

class MethodParseResult:
    def __init__(self, parser, method, success):
        self.parser = parser
        self.method = method
        self.success = success

class InterfaceMemberParseResult:
    def __init__(self, parser, signature, success):
        self.parser = parser
        self.signature = signature
        self.success = success

class EnumVariantParseResult:
    def __init__(self, parser, variant, success):
        self.parser = parser
...
class BlockStatementParseResult:
    def __init__(self, parser, statement, success):
        self.parser = parser
        self.statement = statement
        self.success = success
...
class MatchCaseParseResult:
    def __init__(self, parser, case, success):
        self.parser = parser
        self.case = case
        self.success = success
...
def parse_import(parser):
    ...
    if ((parser_peek_raw(parser).kind.variant == 'Symbol') and (parser_peek_raw(parser).kind.value == ';')):
        parser = parser_advance_raw(parser)
    statement = Statement.ImportDeclaration(items=items, source=source)
...
def parse_variable(parser):
    ...
    if ((parser_peek_raw(parser).kind.variant == 'Symbol') and (parser_peek_raw(parser).kind.value == ';')):
        parser = parser_advance_raw(parser)
    statement = Statement.VariableDeclaration(name=name, mutable=mutable_flag, type_annotation=type_annotation, initializer=initializer)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_struct(parser, decorators):
    ...
    statement = Statement.StructDeclaration(name=name, type_parameters=type_parameters, implements_types=implements_types, fields=fields, methods=methods, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_type_alias(parser, decorators):
    ...
    if (not ((equals.kind.variant == 'Symbol') and (equals.kind.value == '='))):
        return parse_unknown(original)
    parser = parser_advance_raw(parser)
    capture = collect_until(skip_trivia(parser), [';'])
    parser = capture.parser
    ...
    if ((parser_peek_raw(parser).kind.variant == 'Symbol') and (parser_peek_raw(parser).kind.value == ';')):
        parser = parser_advance_raw(parser)
    statement = Statement.TypeAliasDeclaration(name=name, type_parameters=type_parameters, aliased_type=aliased_type, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_interface(parser, decorators):
    ...
    while True:
        ...
        if member_result.success:
            ...
            continue
        parser = skip_struct_member(member_start)
    statement = Statement.InterfaceDeclaration(name=name, type_parameters=type_parameters, members=members, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_enum(parser, decorators):
    ...
    statement = Statement.EnumDeclaration(name=name, type_parameters=type_parameters, variants=variants, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_enum_variant(parser):
    ...
    if ((next.kind.variant == 'Symbol') and (next.kind.value == '{')):
        ...
        while True:
            ...
            if (token.kind.variant == 'EndOfFile'):
                break
            field_result = parse_struct_field(current)
            if field_result.success:
                current = field_result.parser
                if (field_result.field != None):
                    fields = append_field(fields, field_result.field)
                continue
            current = skip_struct_member(current)
    current = skip_trivia(current)
    terminator = parser_peek_raw(current)
    if ((terminator.kind.variant == 'Symbol') and (terminator.kind.value == ';')):
        current = parser_advance_raw(current)
...
def parse_model(parser, decorators):
    ...
    while True:
        ...
        if (maybe_close.kind.variant == 'EndOfFile'):
            break
    statement = Statement.ModelDeclaration(name=name, model_type=model_type, properties=properties, effects=effects, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_pipeline(parser, decorators):
    ...
    statement = Statement.PipelineDeclaration(signature=signature, body=body, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_tool(parser, decorators):
    ...
    body = block_result.block
    signature = FunctionSignature(name=name, is_async=False, parameters=parameters, return_type=return_type, effects=inferred_effects, type_parameters=[])
    statement = Statement.ToolDeclaration(signature=signature, body=body, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_test(parser, decorators):
    ...
    parser = block_result.parser
    body = block_result.block
    statement = Statement.TestDeclaration(name=name, body=body, effects=inferred_effects, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_function(parser, starts_with_async, decorators):
    ...
    body = block_result.block
    signature = FunctionSignature(name=name, is_async=is_async, parameters=parameters, return_type=return_type, effects=inferred_effects, type_parameters=type_parameters)
    statement = Statement.FunctionDeclaration(signature=signature, body=body, decorators=decorators)
    return StatementParseResult(parser=parser, statement=statement)
...
def parse_struct_field(parser):
    ...
    current = skip_trivia(current)
    terminator = parser_peek_raw(current)
    if (not ((terminator.kind.variant == 'Symbol') and (terminator.kind.value == ';'))):
        return StructFieldParseResult(parser=parser, field=None, success=False)
    current = parser_advance_raw(current)
...
def parse_decorators(parser):
    ...
    while True:
        current = skip_trivia(current)
        token = parser_peek_raw(current)
        if (not ((token.kind.variant == 'Symbol') and (token.kind.value == '@'))):
            break
...
def parse_for_statement(parser, decorators):
    ...
    if ((terminator.kind.variant == 'Symbol') and (terminator.kind.value == ';')):
        current = parser_advance_raw(current)
    statement = Statement.ForStatement(clause=clause, body=body, decorators=decorators)
...
def parse_loop_statement(parser, decorators):
    ...
    if ((terminator.kind.variant == 'Symbol') and (terminator.kind.value == ';')):
        current = parser_advance_raw(current)
    statement = Statement.LoopStatement(body=body_result.block, decorators=decorators)
    return BlockStatementParseResult(parser=current, statement=statement, success=True)
...
def parse_if_statement(parser, decorators):
    ...
    if identifier_matches(parser_peek_raw(current), 'else'):
        ...
        if ((terminator.kind.variant == 'Symbol') and (terminator.kind.value == ';')):
            current = parser_advance_raw(current)
    statement = Statement.IfStatement(condition=condition_expression, then_block=then_block, else_branch=else_branch, decorators=decorators)
    return BlockStatementParseResult(parser=current, statement=statement, success=True)
...
def parse_match_statement(parser, decorators):
    ...
    if (current.index < len(current.tokens)):
        ...
        if ((terminator.kind.variant == 'Symbol') and (terminator.kind.value == ';')):
            current = parser_advance_raw(current)
    statement = Statement.MatchStatement(expression=match_expression, cases=body_result.cases, decorators=decorators)
    return BlockStatementParseResult(parser=current, statement=statement, success=True)

def parse_match_cases(parser):
    current = skip_trivia(parser)
    current = advance_to_symbol(current, '{')
    if (not ((parser_peek_raw(current).kind.variant == 'Symbol') and (parser_peek_raw(current).kind.value == '{'))):
...
def parse_prompt_statement(parser, decorators):
    ...
    if ((parser_peek_raw(current).kind.variant == 'Symbol') and (parser_peek_raw(current).kind.value == ';')):
        current = parser_advance_raw(current)
    statement = Statement.PromptStatement(channel=channel, body=body, decorators=decorators)
    return BlockStatementParseResult(parser=current, statement=statement, success=True)
...
def parse_with_statement(parser, decorators):
    ...
    if ((parser_peek_raw(current).kind.variant == 'Symbol') and (parser_peek_raw(current).kind.value == ';')):
        current = parser_advance_raw(current)
    statement = Statement.WithStatement(clauses=clauses, body=body, decorators=decorators)
    return BlockStatementParseResult(parser=current, statement=statement, success=True)
...
def parse_return_statement(parser):
    ...
    if ((terminator.kind.variant == 'Symbol') and (terminator.kind.value == ';')):
        current = parser_advance_raw(current)
    statement = Statement.ReturnStatement(expression=expression)
    return BlockStatementParseResult(parser=current, statement=statement, success=True)
...
def parse_expression_statement(parser, decorators):
    ...
    current = parser_advance_raw(current)
    expression = expression_from_tokens(trimmed)
    statement = Statement.ExpressionStatement(expression=expression)
    return BlockStatementParseResult(parser=current, statement=statement, success=True)
...
def parse_unknown(parser):
    ...
    block = Statement.Unknown(tokens=tokens, text=text)
...
def collect_parenthesized(parser):
    current = skip_trivia(parser)
    start = parser_peek_raw(current)
    if (not ((start.kind.variant == 'Symbol') and (start.kind.value == '('))):
...
def collect_expression_block(state):
    ...
    if (not ((first.kind.variant == 'Symbol') and (first.kind.value == '{'))):
        return ExpressionBlockParseResult(state=current, tokens=[], success=False)
    tokens = []
    depth = 0
    while True:
...
def parse_lambda_expression(state):
    ...
    if (not ((open_paren.kind.variant == 'Symbol') and (open_paren.kind.value == '('))):
        return expression_parse_failure(state)
    current = expression_tokens_advance(current)
    ...
    return ExpressionParseResult(state=current, expression=Expression.Lambda(parameters=parameters, body=body, return_type=return_type), success=True)
...
def parse_expression_bp(state, min_precedence):
    ...
    while True:
        ...
        if (op == '..'):
            left_expression = Expression.Range(start=left_expression, end=rhs_result.expression)
        else:
            left_expression = Expression.Binary(operator=op, left=left_expression, right=rhs_result.expression)
...
def parse_prefix_expression(state):
    ...
    if (token.kind.variant == 'Symbol'):
        ...
        if ((sym == '-') or (sym == '!')):
            ...
            return ExpressionParseResult(state=operand_result.state, expression=Expression.Unary(operator=sym, operand=operand_result.expression), success=True)
...
def parse_primary_expression(state):
    ...
    token = expression_tokens_peek(state)
    if (token.kind.variant == 'NumberLiteral'):
        return ExpressionParseResult(state=expression_tokens_advance(state), expression=Expression.NumberLiteral(value=token.kind.value), success=True)
    if (token.kind.variant == 'BooleanLiteral'):
        return ExpressionParseResult(state=expression_tokens_advance(state), expression=Expression.BooleanLiteral(value=token.kind.value), success=True)
    if (token.kind.variant == 'StringLiteral'):
        return ExpressionParseResult(state=expression_tokens_advance(state), expression=Expression.StringLiteral(value=token.kind.value), success=True)
    if (token.kind.variant == 'Identifier'):
        name = identifier_text(token)
        if (name == 'true'):
            return ExpressionParseResult(state=expression_tokens_advance(state), expression=Expression.BooleanLiteral(value=True), success=True)
        if (name == 'false'):
            return ExpressionParseResult(state=expression_tokens_advance(state), expression=Expression.BooleanLiteral(value=False), success=True)
        if (name == 'null'):
            return ExpressionParseResult(state=expression_tokens_advance(state), expression=Expression.NullLiteral(), success=True)
        return ExpressionParseResult(state=expression_tokens_advance(state), expression=Expression.Identifier(name=name), success=True)
    if ((token.kind.variant == 'Symbol') and (token.kind.value == '(')):
        ...
        if (not ((closing.kind.variant == 'Symbol') and (closing.kind.value == ')'))):
            return expression_parse_failure(state)
    ...
    if ((token.kind.variant == 'Symbol') and (token.kind.value == '[')):
        ...
        if (not array_result.success):
            return expression_parse_failure(state)
        return ExpressionParseResult(state=array_result.state, expression=Expression.Array(elements=array_result.elements), success=True)
    if ((token.kind.variant == 'Symbol') and (token.kind.value == '{')):
        object_result = parse_object_literal(expression_tokens_advance(state))
        if (not object_result.success):
            return expression_parse_failure(state)
        return ExpressionParseResult(state=object_result.state, expression=Expression.Object(fields=object_result.fields), success=True)
    return expression_parse_failure(state)
...
def parse_postfix_chain(state, expression):
    ...
    while True:
        ...
        if (sym == '.'):
            ...
            current_expression = Expression.Member(object=current_expression, member=identifier_text(member_token))
            continue
        if (sym == '('):
            ...
            current_expression = Expression.Call(callee=current_expression, arguments=call_result.arguments)
            continue
        if (sym == '['):
            ...
            current_expression = Expression.Index(sequence=current_expression, index=index_result.expression)
            continue
        if (sym == '{'):
...
def parse_object_literal(state):
    ...
    while True:
        ...
        if (not ((colon.kind.variant == 'Symbol') and (colon.kind.value == ':'))):
            return ObjectLiteralParseResult(state=state, fields=[], success=False)
        current = expression_tokens_advance(current)
...
def parse_struct_literal(state, target):
    ...
    if (not fields_result.success):
        return expression_parse_failure(state)
    return ExpressionParseResult(state=fields_result.state, expression=Expression.Struct(type_name=type_result.parts, fields=fields_result.fields), success=True)

def expression_parse_failure(state):
    return ExpressionParseResult(state=state, expression=Expression.Raw(text=''), success=False)
...
def strip_surrounding_quotes(text):
    if (len(text) < 2):
        return text
    first = runtime.char_code(text[0])
    last = runtime.char_code(text[(len(text) - 1)])
    if ((first == 34) and (last == 34)):
        return runtime.substring(text, 1, (len(text) - 1))
    return text

def normalize_test_name(text):
    trimmed = trim_text(text)
    if (len(trimmed) >= 2):
        first = runtime.char_code(trimmed[0])
        last = runtime.char_code(trimmed[(len(trimmed) - 1)])
        if ((first == 34) and (last == 34)):
