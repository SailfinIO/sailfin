...
def lower_to_llvm(native_module):
    ...
    while True:
        ...
        diagnostics = list(diagnostics) + list(lowered.diagnostics)
        if (len(lowered.lines) > 0):
            lines = list(lines) + list(lowered.lines)
...
def emit_function(function):
    ...
    diagnostics = list(diagnostics) + list(preparation.diagnostics)
    lines = []
    ...
    lines = list(lines) + list(body.lines)
    diagnostics = list(diagnostics) + list(body.diagnostics)
    lines = append_string(lines, '}')
...
def append_parameter_binding(bindings, binding):
    return list(bindings) + list([binding])
