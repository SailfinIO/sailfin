...
| `E03xx` | Duplicate symbols / type conflicts; mixed `int`/`float` arithmetic operand reject (`E0306`, SFN-385); array/struct arithmetic rejects (`E0307`/`E0308`, SFN-562/SFN-584); proven primitive mismatch (`E0309`, SFN-675); array element type mismatch (`E0310`, SFN-910) | `typecheck*.sfn` |
| `E04xx` | Effect violations (`E0404` unrecognized effect root, SFEP-0017); workspace capability-envelope policy (`E0405` member declared-effect drift, `E0406` malformed `[workspace.capabilities]` envelope entry, SFEP-0051/SFN-416; `E0407` capsule manifest present but unreadable — `[capsule] name` missing, SFN-764); value resolution (`E0420` undefined free-function callee, SFN-544; `E0422` void call in value position, `E0423` call arity mismatch — a bare-identifier callee and, since SFN-1090, a pinned member-form builtin call (`fs.removeDirectory(...)`) whose receiver is an unshadowed namespace identifier, SFN-812/SFN-1090); import resolution, relative and capsule specs (`E0430` cannot find module, `E0431` symbol not defined in closure, #1953, SFN-686) | `diagnostics_render.sfn`, `effect_checker.sfn`, `typecheck_types.sfn`, `typecheck/call_signature.sfn`, `capsule_resolver.sfn` |
| `E05xx`–`E06xx` | Build / check tooling; parse diagnostics `E0500` (unrecognized top-level construct), `E0501` (malformed function parameter list), `E0502` (missing variable initializer), `E0503` (missing struct field type); capsule distribution (`E0612` capsule marked unpublishable, SFEP-0002 §3.7 / SFN-714; `E0613` private capsule resolved from a fetched origin, SFEP-0020 §3.6 / SFN-715); `E0614` (`sfn build --target=<triple>` value outside the closed set, SFEP-0068 §3.1 / SFN-774); `E0615` (archive member rejected or archive malformed during toolchain extraction — the CLI-consumer mapping of `sfn/archive`'s `ArchiveError`, SFEP-0071 §3.6 / SFN-753, emitted by `toolchain.sfn` at the `sfn toolchain install` extraction boundary); `E0616` (home directory unresolvable — `HOME`, `USERPROFILE`, and `HOMEDRIVE`+`HOMEPATH` all unset/empty, SFN-856); `E0617` (`sfn emit --target=<triple>` resolved a different triple than requested — a future-precedence guard, SFN-923); `E0618` (installed toolchain entry is corrupt — its recorded install-manifest payload failed verification, SFEP-0073 §3.8 / SFN-1066); `E0619` (installed toolchain entry is a partial install — the payload was never committed, SFEP-0073 §3.4 / SFN-1066); `E0620` (the selected toolchain is not installed — reported by `sfn toolchain active`, SFEP-0073 §3.4 / SFN-1066); LLVM tool-family resolution (SFEP-0066 §3.4 / SFN-948): `E0621` (a required family member is missing, or was found but failed to run), `E0622` (family version skew — a member reports a major other than the one the release supports), `E0623` (the requested target triple is outside SFEP-0066 §3.2's governed four, or the resolved `llc` registers no backend for it), `E0624` (a member ran but printed no recognizable `LLVM version`/`LLD` banner); `E0625` (`sfn build --static` resolved a triple outside its narrower closed set — Linux glibc x86_64/aarch64 only, SFN-1268); `E0626` (`--static` could not resolve its static link prerequisites on the host — a missing static glibc/libgcc artifact, SFN-1268); `E0627` (a dependency capsule declared `[build] link-libs`, which are ignored because the key is honored only for the capsule being built — `warning` severity, SFN-1269); `E0628` (the governed Linux x86-64/aarch64 direct `ld.lld` link — the default final-link path on the targets Sailfin owns Link for, SFEP-0066 §3.2 — could not resolve a prerequisite, e.g. no `ld.lld` on PATH or missing glibc CRT/gcc objects, or an unrecognized `SAILFIN_LINK_PROVIDER` value; fails closed rather than falling back to clang, SFN-953) | `tools/check.sfn`, `cli/commands/publish.sfn`, `capsule_resolver/provenance.sfn`, `capsule_resolver/discovery.sfn`, `cli/commands/build.sfn`, `cli/commands/emit.sfn`, `cli/commands/login.sfn`, `cli/commands/config.sfn`, `cli/commands/add.sfn`, `cli/commands/toolchain.sfn`, `toolchain/inspect.sfn`, `build/llvm_toolchain_resolver.sfn`, `build/target_diagnostics.sfn` |
...
| `E08xx` | `Result` / `?` operator; extern C-ABI (`E0801`–`E0803`, `E0805`; `E0804` retired by SFN-1294); bare function-type annotation reject (`E0826`); malformed array type spelling reject (`E0830`, SFN-385); interpolation operand not stringifiable (`E0832`, lowering-stage `[fatal]`, SFN-408); uninferable generic-struct static constructor reject (`E0833`, SFN-404); value-position `if` reject (`E0834`, SFN-442); character-literal reject (`E0835`, SFN-442); `Task<T>[]` heterogeneous handle push reject (`E0836`, SFN-441 / SFEP-0055); `Task<T>` double-await / use-after-move (`E0837`, SFN-446); Task or Channel handle nursery escape (`E0838`, SFN-446/SFN-694); named-function value type/effect mismatch (`E0839`, SFN-667); non-pointer-width named-function value ABI (`E0840`, SFN-667); `?`-desugar `value` shadowing (`E0841`, SFN-901); `spawn` target not a zero-argument named function or `fn` literal (`E0842`, SFN-1006); imported or prelude name in a function-value slot (`E0843`, SFN-1161); array higher-order builtin arity (`E0844`, SFN-691); array higher-order builtin argument shape (`E0845`, SFN-691); `@repr` misuse — unknown/malformed argument, or applied to an enum or generic struct (`E0846`, SFN-1289); `@repr(C)` struct field type with no C representation (`E0847`, SFN-1289); `@repr(C)` `size =`/`align =` assertion disagreeing with the computed layout (`E0848`, SFN-1289); typed extern callback signature mismatch, non-C-ABI function, or lambda (`E0850`, SFN-1293); variadic extern `...` placement (no fixed parameter before it, or a parameter after it) or a call-site argument in variadic position whose stated type C would have promoted (`E0851`, SFN-1291); store through a `*const T` raw pointer (`E0852`, SFN-1296) | `typecheck_types.sfn`, `ownership_checker.sfn`, `typecheck/array_hof_rules.sfn`, `llvm/expression_lowering/native/core_strings.sfn`, `typecheck_types/repr_c.sfn`, `typecheck_types/extern_abi.sfn` |
