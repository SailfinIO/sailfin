...
| **1.3** De-duplicate helpers in `sfn/test` | XS | `capsules/sfn/test/src/mod.sfn:13-50, 136-148` import from `sfn/strings` | `_number_to_string` and `_find_in_string` deleted (~40 LOC). The capsule depends on `sfn/strings` instead of inlining. Round-trip `make compile && make test` passes. | (none) |
