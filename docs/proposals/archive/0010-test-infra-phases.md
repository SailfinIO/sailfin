...
| **1.6** Test discovery filtering (`-k`, `--tag`) | S | `compiler/src/cli_commands.sfn:_collect_test_files_cmd`; new in-test filter pass over `TestDeclaration.name` and decorators | `sfn test -k auth` runs only tests whose name contains `auth`. `--tag slow` runs tests carrying `@tag("slow")` decorator (decorators already exist; see `compiler/capsules/analyzer/src/decorator_semantics.sfn`). | 1.1 |
