...
- [ ] Lower enum payload storage (including recursive variants) into LLVM so Stage2 can materialise compiler AST enums without Python fallbacks.
  - [x] Support inline payload emission for primitives/struct references and validate via struct-enum fixtures. Enum literals with payload fields now allocate the enum on the stack, store the tag, then use `getelementptr` and `bitcast` to store each field at its correct byte offset within the payload byte array, finally loading the complete enum value. Field offsets use the metadata from `EnumVariantInfo.fields[].index`. Validated by `compiler/tests/test_native_llvm_execution.py::test_native_llvm_execution_stores_enum_payload_fields`.
