...
1. **Stage2 Backend Delivery**
  ...
  - [ ] Lower borrow expressions (`&`, `&mut`, `borrow(...)`) and reference types into the native IR and LLVM backend, representing borrows as explicit pointer values and covering them with a regression in `compiler/tests/test_native_llvm_execution.py`.
  - [ ] Model move/borrow diagnostics in Stage2 by threading ownership metadata through `.sfn-asm`, rejecting conflicting borrows during lowering, and validating the checks with a focused suite once enforcement lands.
