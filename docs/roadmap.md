...
1. **Stage2 Backend Delivery**
  ...
  - [ ] Generalise `.for` lowering to preserve element types for `boolean[]`/`int[]` literals so non-number sequences iterate natively once array literal emission widens beyond `double`.
