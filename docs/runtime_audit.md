...
| Symbol | Status | Notes |
|---|---|---|
| `sailfin_runtime_string_length` | ✅ | Uses `_safe_strlen_asan` with ASAN-safe scanning and a tunable `SAILFIN_MAX_STRLEN_SCAN` cap. **#1315:** `string.sfn` no longer links through this façade extern — `sfn_str_sfn_len` now recovers length via the local word-load NUL walk. The C body stays exported (9 internal C callers) until #822. |
...
| `sailfin_runtime_grapheme_count` / `grapheme_at` | ✅ | Byte-indexed; returns immediate-codepoint pseudo-strings (tagged pointer `(codepoint << 32)`) for ASCII to avoid allocation on hot paths. **Accessor family flipped to Sailfin (#1315, C4/R1 of epic #1308):** the bare emission targets `sfn_str_grapheme_count` / `sfn_str_grapheme_at` are now real Sailfin bodies in `runtime/sfn/string.sfn` (word-load UTF-8 byte walks; `grapheme_at` materializes a real arena/libc 1-byte buffer and **no longer produces immediate pseudo-pointers**, retiring the Linux producer for this family). `grapheme_count` returns `f64` to match the descriptor's `double` C-ABI. The C `sfn_str_*` namesakes in `sailfin_runtime.c` are `static`; the legacy `sailfin_runtime_grapheme_*` bodies stay C (seed-built IR) until #822. |
| `sailfin_runtime_byte_at` / `find_byte_index` | ✅ | memchr-backed. **#1315:** the bare emission targets `sfn_str_byte_at` / `sfn_str_find_byte` are now real Sailfin bodies (word-load scan + immediate-codepoint guard) in `runtime/sfn/string.sfn`; the C `sfn_str_*` namesakes are `static`. No descriptor row (orphan targets given real bodies for surface completeness). |
| `sailfin_runtime_char_code` | ✅ | Handles immediate-codepoint and legacy C strings. **#1315:** the bare emission target `sfn_str_codepoint` is now a real Sailfin body in `runtime/sfn/string.sfn` (first-byte / immediate-codepoint decode, bootstrap-identical to the C body); returns `f64` to match the descriptor's `double` C-ABI. C namesake `static`; legacy `sailfin_runtime_char_code` stays C until #822. |
