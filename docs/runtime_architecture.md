...
>   issue #408).** The prelude facade's type-meta cluster now imports
>   its sfn-native definitions directly: `runtime_is_string_fn` /
>   `_is_number_fn` / `_is_boolean_fn` / `_is_array_fn` /
>   `_is_callable_fn` / `_resolve_runtime_type_fn` /
>   `_instance_of_fn` flip from `runtime.X` magic-namespace
>   bindings to imported `sfn_*` symbols from
>   `runtime/sfn/type_meta.sfn` (the M2.10 #402 surface). The
>   helper registry's `native_signature` rows (PR #749) already
>   routed compiled IR to `@sfn_is_*` / `@sfn_resolve_type` /
>   `@sfn_instance_of`, so emitted user code is unchanged byte-
>   for-byte; the M2.12b flip closes the audit at the Sailfin
>   source layer.
>
>   `runtime_is_void_fn` keeps its `runtime.is_void` delegate
>   because `runtime/sfn/type_meta.sfn` does not yet export
>   `sfn_is_void` (M3 follow-up). The remaining `runtime.X`
>   delegates in `runtime/prelude.sfn` are explicitly scoped to M3
>   and fall into four buckets:
>     1. **No `sfn_*` definition yet** — capability bridges
...
>        `grapheme_count`, `grapheme_at`. The IR-level call lands
>        on `@sfn_str_codepoint` / `@sfn_str_grapheme_count` /
>        `@sfn_str_grapheme_at`, but those symbols are still C
>        trampolines in `runtime/native/src/sailfin_runtime.c`
>        forwarding to the legacy `sailfin_runtime_*` bodies (the
>        Sailfin module exports `sfn_str_sfn_*` with the `_sfn_`
>        infix). The prelude-side `runtime.X` → import flip also
>        needs a `string` ↔ `* u8` / `int` ↔ `float` cast that's
>        out of scope per the issue's audit-only `In:` list.
>
>   The audit invariant the M2.12b flip targets is **"every
>   M2-replaced symbol's emitted call lands on the canonical
>   `sfn_*` symbol name"** — that holds end-to-end via either
>   direct sfn import (the type-meta cluster) or the registry's
>   `native_signature` routing (`sleep`, `process.run`, the
>   `print*` family, `sfn_str_codepoint`, `sfn_str_grapheme_*`).
>   Several of those `sfn_*` symbols are still C trampolines in
>   `runtime/native/src/sailfin_runtime.c` and stay live until M3
>   lifts each body into a Sailfin module; only `sfn_sleep`,
>   `sfn_process_run`, the `sfn_arena_sfn_*` / `sfn_rc_sfn_*`
>   families, and the M2.10 type-meta surface are defined in
>   Sailfin today. The truly dead-after-M2 portion is the legacy
>   `sailfin_runtime_*` bodies (e.g. `sailfin_runtime_print_raw`,
>   `sailfin_runtime_string_concat`) — still linked for seed
>   compat, but no fresh user emission references them. Pinned by
>   the standard `make compile` + `make test` self-host gate; the
>   determinism sweep on
...
>   integer side. Slice D (`as` casts) and E.1 (bare-literal defaulting +
>   `number` aliasing) shipped as follow-ups; E.2/E.3 (source migration
...
>   code currently mixes i64 and double everywhere via the alias.
...
replaced the C runtime (`runtime/native/`, deleted 2026-06-21 in #822). It
describes system structure, subsystem designs, compiler integration requirements,
a sequenced milestone plan, and open decisions. The runtime capsule root is now
`runtime/` (manifest at `runtime/capsule.toml`).
...
The M0.5 arena-in-C (`runtime/native/src/sailfin_arena.c`) was the sole
designed exception — an explicitly disposable temporary deleted at M3 alongside
the rest of `runtime/native/` (#822). No other C source file was created at any
phase.
...
**Status (2026-05-16): Slices A–D + E.1 + E.1.5 + E.2 + E.3a
shipped; E.3b deferred to v0.6.1.** Annotated locals,
...
fix-it suggesting `x != 0` (Slice D). **Slice E.1 (#488):** bare
unsuffixed integer literals default to `int` (i64); `: number`
becomes a deprecated alias for `: float` (both lower to `double`)
in every primitive type-mapping seam. `let x = 42; x + 1` lowers
to `alloca i64`, `add i64`. **Slice E.3a — integer-shaped
`: number` migration completed 2026-05-16 (#567):** every
...
- **Slice E — Bare-literal defaulting + `number` retirement.**
  ...
  `dominant_type` / etc. can be deleted. Migration order:
    ...
    3. **E.2 (#530, 2026-05-12):** The closed
       instruction-lowering numeric cluster spelled its
       ...
       by #530. The audit intentionally left the prelude/tests,
       future-seam files, and existing boundary casts for the
       follow-up E.3a sweep.
    4. **E.3a (2026-05-16; #555 split into #559–#567 plus #571,
       #572, #576, #578, #579, #586, #601–#609 lockstep PRs):**
       The remaining `: number` annotations in
       ...
       `compiler/src/llvm/utils.sfn` (`number_to_string` /
       `number_to_string` powers ladder); the C-ABI boundaries
       `sailfin_cli_main` and `native_cli_main` in
       `compiler/src/cli_main.sfn` (the C runtime declares them
       as `double` returns); the runtime `is_number` type-guard
       predicate at `runtime/prelude.sfn:471`; the literal-text
       predicates in
       `compiler/src/llvm/expression_lowering/native/core_text.sfn`;
       and the `Future` mapping seam in
       `compiler/src/llvm/type_mapping.sfn` and
       `compiler/src/llvm/expression_lowering/native/statement_suspension.sfn`.
       The slice picked up against E.1.5's warning regime (#550) —
       the original parallel-safe disjoint-subtree assumption broke
       on the `make_native_*_layout` family and the
       `string_utils` ↔ `llvm/utils` shadow chain, so the bulk PRs
       split into lockstep caller/callee micro-PRs (#579 →
       #601–#606). Migration triggers the v0.6.0 seed bump
       (alpha.16) and unblocks E.3b on that seed.
    5. **E.3b (#556):** Reapply #296 (`dominant_type` rejection
       of silent int/float coercion). Picks up on the v0.6.0
       (alpha.16) seed and targets v0.6.1. The rejection lands
       once every site agreed on int-vs-float through E.3a,
       which is now the case. Closes L2/L3.
    6. **E.4:** Cut a release; delete `number` everywhere.

**Self-hosting safety (Slice A through E.3a):** Slice A landed
additively — adding `float` → `double` to `map_primitive_type` and
admitting `int`/`float` in the extern accept-list did not change
any existing code path. Slices E.1 and E.1.5 layered the
bare-literal default and the warning-regime ABI mismatch
diagnostic without breaking the still-`: number` corpus. E.2 and
E.3a then migrated the compiler source and runtime prelude to
their semantic `: int` / `: float` shapes (or to `// alias-coverage`
opt-outs at documented seams); every PR self-hosted under
`make compile` and passed `make test-{unit,integration,e2e}` on
the seed it picked up against, with the bulk-PR lockstep
caller/callee splits (#579, #571 / #576) needed precisely to
preserve self-hosting across cross-subtree boundaries.
...
rules have been deleted. `runtime_globals.ll` (comments-only) and the
`runtime/native/` directory were deleted in #822 (2026-06-21).
...
4. Delete `runtime/native/ir/runtime_globals.ll`. ✅ (deleted with `runtime/native/` — comments-only file)
5. `runtime/native/` directory is empty or removed entirely. ✅ (deleted 2026-06-21, #822)
...
| `sailfin_runtime_throw/try_enter/try_leave/take_exception` | `sfn_throw`/`sfn_take_exception` (frame-based, M2.7b #404) | M2 — **legacy descriptor rows removed (#918)**: dead since M2.7b (no live call emit site), ABI-mismatched so dropped not flipped. Backing C symbols were in `runtime/native/` (now deleted, #822). |
...
- `runtime/native/ir/runtime_globals.ll` (deleted ✅ with `runtime/native/`, #822)
...
**Temporary exceptions — all deleted:**
...
| `runtime/native/ir/runtime_globals.ll` | LLVM stub (comments-only) | **Deleted 2026-06-21 (#822)** |

The Sailfin-emitted `@main` (M5) replaced `native_driver.c`. The
`runtime/native/` directory is now **fully deleted (#822, 2026-06-21)**.
The runtime capsule root is `runtime/` (manifest at `runtime/capsule.toml`).
Every line of Sailfin toolchain source is `.sfn`.
