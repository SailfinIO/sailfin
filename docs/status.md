...
| Closures with capture | **Shipped**. Lambdas capture enclosing variables end-to-end: free-variable / capture-set inference in `compiler/src/typecheck_captures.sfn` (#458, A1) → closure env-struct synthesis + alloc/load in `compiler/src/llvm/closures.sfn` (#459, A2 — now wired from `compiler/src/llvm/expression_lowering/native/core.sfn`) → lambda lifting + env-load prologue in `compiler/src/llvm/expression_lowering/native/lambda_lowering.sfn` and `compiler/src/llvm/lowering/instructions.sfn`, with hidden-env call-site dispatch (#689) so the `{fn_ptr, env*}` pair is callable. Capturing lambdas compile, link, and run end-to-end. Pinned by `compiler/tests/unit/typecheck_lambda_capture_test.sfn`, `compiler/tests/integration/closure_env_emission_test.sfn` + `compiler/tests/integration/closure_lifting_test.sfn`, and `compiler/tests/e2e/test_closure_capture.sh` (non-capturing, single-int-capture, and higher-order `fn(int) -> int` dispatch fixtures). | `map`/`filter`/`reduce`, spawn handlers, route handlers |
