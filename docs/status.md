...
Reconciled at release `0.10.6`. The seed pin and the compiler version are read
from `bootstrap.toml` (`[seed].version` — SFEP-0047) and
`compiler/capsule.toml` respectively; this file does not restate them, because
a hand-copied version is a version that goes stale.
...
**Feature PRs do not edit this file.** It is reconciled on the release cadence
by `/status-sweep`, which reads the issues and PRs that merged since the last
reconciliation and flips the affected rows in one commit.

That is a deliberate reversal. When every PR was obliged to update it, a third
of all commits touched this one file, they conflicted with each other
constantly, and — because a PR author writes narrative, not status — the file
grew by roughly twenty lines per change and almost never shrank. Both problems
have the same cause: a per-change writer has per-change information, and this
file wants current-state information. Batching the write to the cadence gives
the writer the vantage point the file actually needs.

The cost is bounded and deliberate: this file may lag `main` by up to one
release. When that matters — you need to know whether something merged
yesterday — the merged PR and the Linear issue are authoritative and this file
is not.
...
notes (auto-generated per tag). A row here is a **status plus a one-line
note**; when the note wants a paragraph, the paragraph belongs in the spec
chapter or the `docs/proposals/*` design doc that already owns that design
(e.g. `docs/proposals/0025-native-runtime-architecture.md`) and the note links
to it.
