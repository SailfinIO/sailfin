...
| Windows x86-64 | **Tier 3 — best effort** (`docs/conventions/target-tiers.md`). Cross-compiled from Linux (`ci-cross-windows`); release asset published. The only merge-blocking Windows validation is `smoke-windows`: the cross-built `.exe` boots (`--version`) and runs `sfn check` on one example — frontend only. Codegen, linking, the test suite, and self-hosting are never exercised on Windows in CI; native MSVC self-host in progress (SFEP-0021, tracking SFN-53–58) | Not a target — mediated vendor-library shim (SFEP-0016 §3.1) |
