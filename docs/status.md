...
| `sfn/test` | `"test"` | Partial | None (pure tier) / `io`, `clock` | Assertions: legacy `assert_*` (`![io]`), `pure_assert_*`, free-function `expect_*` tier, snapshot tier (#977). Fluent `expect(x).to_be(y)` deferred on generic monomorphization + cross-module method-dispatch ABI. Process-lifetime control (SFEP-0010 §3.2, `src/process_control.sfn`): `run_bounded`/`run_bounded_in`/`run_bounded_stdin` run a child under a wall-clock deadline (`![clock, io]`); `start_background`/`stop_background`/`with_background` scope a blocking server's lifetime, killed+reaped on scope exit including the throwing path; `wait_until` polls readiness in-process. `ChildOutcome.timed_out` disambiguates a deadline kill from exit code `137`. Capsule `required` capabilities widened `["io"]` → `["clock", "io"]` for this addition (ceiling on the capsule's own functions only, not inherited by consumers) |
