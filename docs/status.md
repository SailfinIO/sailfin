...
- **Toolchain version pinning — Phase 1** (SFEP-0046, SFN-167). `capsule.toml`
  ...
  **Phase 2 acquire — `sfn toolchain install <version>`** (SFEP-0046 §3.5,
  ...
  (SFEP-0071 §3.4/§3.6, SFN-898): `targz_extract` (`stdlib/archive/src/
...
- **Toolchain install-manifest recording + verification** (SFEP-0073 §3.8,
  SFN-1064). New `compiler/src/toolchain/manifest.sfn` defines the
  `.install-manifest` schema — a magic + integer schema-version line,
  `key = value` headers (`management_protocol` [recording SFEP-0073 §3.5's
  routing-compatibility field, which SFN-1067 routes on], `version`,
  `host`, `archive_sha256`), a blank line, then `<64-hex sha256>
  <store-relative path>` records, line-oriented like `SHA256SUMS` — plus a
  forward-compatible reader; new `compiler/src/toolchain/integrity.sfn` walks
  the staged payload, hashes it, and implements the verification primitives.
  `toolchain_lay_out_store_entry` (`compiler/src/toolchain/store.sfn`) writes
  `.install-manifest` recording the executable, the `sfn` alias, and every
  bundled `runtime/`, `capsules/`, and `workspace.toml` file, immediately
  before `.sha256` so the completeness marker used by `toolchain_store_ready`
  remains the last write even against a half-written staging directory.
  `sfn toolchain install` (`compiler/src/toolchain/install.sfn`) runs a
  forced-uncached full verification (`toolchain_verify_entry(staged_dir,
  true)`) of the staging directory before the atomic commit, so an entry
  commits only once every recorded payload is present and matches. Dispatch
  (`compiler/src/toolchain/dispatch.sfn`) hashes the selected executable
  against its manifest before `process.exec`, and verifies bundled
  runtime/capsule inputs before any build-like command (`build`/`check`/
  `run`/`test`) proceeds; a mismatch refuses execution and prints the exact
  `sfn toolchain install <version>` command, taking the target version from
  the manifest itself, and verification never repairs or deletes anything.
  Records are rejected for missing, modified, duplicate, path-escaping, or
  case-normalization-colliding entries, reusing `archive_check_member_path`
  (the SFEP-0071 §3.6 path-safety guard) rather than reimplementing it.
  Successful whole-entry verifications are memoized for the process lifetime
  only: SFEP-0073 §3.8 forbids a size-and-mtime cache key, and the manifest
  records neither field, so the memo cannot degrade into one. An entry with
  no `.install-manifest` — a legacy flat `versions/<version>` entry, a
...
- **Toolchain inspection — `list`/`active`/`verify`** (SFEP-0073 §3.9 slice 1,
  SFN-1066). Three read-only `sfn toolchain` leaves, previously exposed only
  `install <version>` (and `run`, intercepted earlier by the CLI's selector
  scan). `sfn toolchain list [--json]` enumerates every store entry across
  the host-qualified tree and the read-only legacy flat tree, reporting each
  entry's `state` (`complete`|`partial`|`corrupt`), `integrity`
  (`verified`|`unverified`|`failed`), and `roles` (`entry`|`selected`|
  `project`; `default` and `management` are locked into the vocabulary but
  never assigned yet). `sfn toolchain active [--json]` explains what the
  current directory selects and why — selected version and path, selection
  source, entry version and path, host triple, fetch policy
  (`auto`|`local`|`off`), and the project's `[toolchain]` requirement —
  performing no network request. `sfn toolchain verify [<version> | --all]
  [--json]` runs full, uncached payload verification against the SFN-1064
  install manifest, defaulting to the selected toolchain with no argument.
  All three share one locked JSON envelope, `sailfin-toolchains/1`
  (`docs/reference/toolchains-json-schema.md`), gated by the schema-lock test
  `compiler/tests/e2e/toolchain_inspect_json_test.sfn`. Exit matrix: `0` no
  actionable finding, `1` inspection completed with a finding (a partial or
  corrupt entry, or a selected-but-uninstalled toolchain), `2` setup/
  selection/usage failure (no envelope emitted). New diagnostics: `E0618`
  (corrupt entry), `E0619` (partial install), `E0620` (selected toolchain not
  installed). This is SFEP-0073 slice 1 of 5: the per-user default
...
- **Management-protocol routing + `sfn toolchain entry-version`** (SFEP-0073
  ...
  before the command tree is built, so an entry toolchain can hand a
  management leaf it was never built to parse to a newer installed payload —
  the mechanism that lets native lifecycle commands gain leaves without
  overwriting a package-manager-owned entry executable.
  `toolchain_management_survey` (`compiler/src/toolchain/management.sfn`)
  walks the host-qualified store only — the flat legacy tree carries no
  install manifest and is never surveyed — and admits a directory only when
  it is complete (`.sha256` present), its `.install-manifest` reads
  `TOOLCHAIN_MANIFEST_OK`, its recorded `version`/`host` match where it is
  stored, its version parses as valid semver, and its `management_protocol`
  falls in `[TOOLCHAIN_MANAGEMENT_PROTOCOL_MIN, _MAX]`
  (`compiler/src/toolchain/manifest.sfn`, both `1` today, distinct from
  `TOOLCHAIN_MANAGEMENT_PROTOCOL`, the value this build writes). An entry
  with no manifest is never a candidate — claiming it speaks protocol 1 is
  exactly what §3.5 forbids. Selection is newest-compatible across the store
  *and* the running entry; a payload that is not strictly newer loses and
  the command answers in-process, so routing only ever moves forward. Only
  the chosen candidate is hash-verified; a verification failure warns and
  the walk falls through to the next-newest, so one corrupt newest payload
  cannot strand management. A payload above the routable range can never
  become the management payload: with a compatible payload also present the
  entry warns and routes to it; with none, it refuses with
  bootstrap-installer/package-manager upgrade guidance and exit `2` (SFEP-0073
  §3.4 reserves `1` for a completed inspection that emitted an envelope),
  sparing only exact-version `sfn toolchain install <version>` as the one
  repair path §3.5 keeps in the entry. New leaf `sfn toolchain entry-version`
  reports this executable's version, host triple, path, and
  management-protocol range and is never itself routed — it is the
  diagnostic that still answers when routing is what's broken. Routing
  declines when `SAILFIN_TOOLCHAIN_DISPATCHED` is set (an explicit selector
  or project pin already won per §3.3 rank 1) or when a new
  `SAILFIN_TOOLCHAIN_MANAGEMENT=<version>` marker already names this process
...
- **Native pipeline verify command** (SFN-725). `sfn dev verify [--fast]
  ...
  wrapper. Phase identity is structural: the verb dispatches each
  ...
  leaves the last completed phase readable — a net gain over the retired
  bash wrapper, whose report write ran only from its `EXIT` trap. The
  verdict envelope is `sailfin-run/2` with `target` value `"verify"`,
  ...
  kind.
- **`--verdict` flag on `sfn test`/`sfn build`/`sfn dev bootstrap build`**
  (SFN-1120). Opt-in boolean flag, off by default, that emits the same
  `sailfin-run/2` trailer as `sfn dev verify` (`host` = the invoking
  command, `target` = `"test"`/`"build"`/`"bootstrap-build"`), rendered by
  the same `compiler/src/cli/verdict.sfn`; `status`/`failure` derive from
  the process exit code alone (`warn` is unreachable), and `phase`,
  `first_error`, and `report` are always `null` since none of these three
  runs a phase ledger or writes a report file. Armed in
  `compiler/src/cli/main.sfn` (`_v2_arm_verdict`). Spec:
  `docs/reference/run-result-schema.md#the---verdict-flag`.
...
- **`sfn package`.** Sailfin-native packaging: compiler mode, user-capsule
  ...
  **That is not the same as the nightly being green** — the same day it
  ...
  try to *fetch* the pinned seed — which the then-shipped pre-SFN-775 MinGW
  bridge binary could not do because that retired build carried no TLS.
  #3048 fixed the trigger
  ...
  0.10.2`) and `98ceafd5` (v0.10.4, `failed to re-exec the pinned seed`, at a
  time when `sfn_process_exec` was a documented `-1` stub on Windows).
  `sfn_process_exec` (`runtime/sfn/platform/process_windows.sfn:2220`) is no
  longer a stub: Win32 has no `execve`, so it spawns the child with the
  parent's stdio inherited, waits, and exits with the child's code. Three
  divergences from `execve` are stated rather than papered over — the parent
  process survives blocked in `WaitForSingleObject`, the process id changes
  rather than being reused, and a `TerminateProcess`-killed child reports an
  exit code, not a signal — and the `-1` return is preserved as the
  exec-failed sentinel, matching `execve`'s contract that any return at all
  means the exec did not happen. Whether this changes the nightly's outcome
  is not established. Because the step's name
  ...
  publishes as `sailfin_<version>_windows_x86_64-msvc.tar.gz` — and on
  v0.10.3 it succeeded in 14m50s, passing every step including `Verify
  compiler version matches tag` and `Verify installer payload reports
...
- **LLVM lowering retires the legacy diagnostics channel (SFN-916, SFEP-0061
  ...
  SFEP.
- **Import-resolution checking (#1953, SFN-686).** `sfn check` diagnoses an
  import spec that resolves to no module in the check resolver's pass
  (`E0430`) and a named specifier defined nowhere in the staged import
  closure (`E0431`, closure-wide "defined somewhere" — Sailfin resolves
  imports globally by name, not per-declaring-module export; prelude
  globals are exempt, since `runtime/prelude.sfn` names never appear in the
  staged closure symbol set even though the link defines them). Narrows the
  check≠build gap (#1389) for the wrong-import-depth class (e.g. #1952).
  Scope: relative (`./`, `../`) and capsule (`<scope>/<name>[/submodule]`,
  e.g. `sfn/...`) specs are checked; `runtime/...` specs and bare names are
...
- **Emit pipeline.** Parallel per-module emit fan-out (Stage E PR3, #278)
  ...
  (#378); native cross-target build and installer staging (SFN-775/SFN-776).
...
- **Native TLS 1.3 (SFEP-0036/SFEP-0048, SFN-341).** The `sfn/crypto` capsule
  ...
  the tracked follow-on that closes the gap. Windows binds the system `ROOT`
  certificate store (SFN-808): `runtime/sfn/platform/cert_roots_windows.sfn`
  enumerates it via `CertOpenSystemStoreA`/`CertEnumCertificatesInStore`,
  `-lcrypt32` is wired for the msvc ABI
  (`compiler/src/build/target.sfn:405`), and `SAILFIN_TLS_CAFILE` still
  overrides it. Confirmed with an A/B probe on a native MSVC build
  (2026-08-21): `sfn/http`'s `get()` against a real public endpoint
  (`https://raw.githubusercontent.com/SailfinIO/sailfin/main/README.md`),
  same probe source and toolchain, differing only by the SFN-1026 fix —
  without it, `RESOLVE_TLS_FAIL`; with it, `RESOLVE_TLS_OK`, completing a
  full TLS 1.3 handshake against a real public CA chain verified through the
  Windows system ROOT store, with no `SAILFIN_TLS_CAFILE` override set
  (`CRYPT32.dll` confirmed imported). SFN-1026 — `getaddrinfo` running
  before `WSAStartup`, which blocked every hostname-based client connect on
  native Windows — is fixed and shipped in v0.10.4 (merged as `d8c858b8`,
  #3061); v0.10.3 as published still has the bug. Application writes larger than one record's
...
- **Named functions as typed values** (SFN-667 / SFEP-0030 item 1): a concrete,
  ...
  SFN-108 — see the nested-function row below for that path and its limits.
  Fn-typed struct fields ship as of SFN-674: struct initializers store the
  `{fn_ptr, env}` pair inline (without aggregate boxing), and `r.handler(x)`
  GEP-loads that pair before routing it through the same closure-dispatch seam;
  this resolution runs ahead of ordinary struct-method fan-out. Dispatch through
  a fn-typed collection element remains unshipped. Imported/prelude names also
...
| Nested / local function declarations | **Shipped** (call-by-name and as-value) | Statement-position `fn name(...) -> T ![effects] { ... }` inside a function or lambda body (SFEP-0042, #1922, epic #1609). Non-capturing static items (Rust model): block-hoisted so siblings mutually recurse and a nested fn self-recurses; lifted to plain top-level functions (`sfn_nested_<name>_<N>`, no env/fat-pointer) and called by name via static dispatch. Referencing an enclosing local/parameter is rejected (`E0421`, fix-it → `let f = fn(...) => ...`); a same-named nested-fn parameter legally shadows. A declared-but-uncalled nested `fn foo() ![io]` imposes no effect on its parent; a *call* requires the effects transitively, and each nested body is effect-checked against its own row. Parser: `compiler/capsules/syntax/src/parser/statements/block.sfn`; scope/E0421: `compiler/capsules/analyzer/src/typecheck/function_scopes.sfn`; target-neutral lift: `compiler/capsules/codegen/src/lambda_lowering.sfn`; effects: `compiler/capsules/analyzer/src/effect_checker/mod.sfn`. Tests: `nested_function_declaration_test.sfn` (unit + e2e), `nested_function_effect_test.sfn`. **Sub-issue B (SFN-108) now ships**: a nested fn's bare name is legal in `fn(...)` value position (call argument, typed `let`/assignment, `return`, array/collection element, struct-field initializer) — the same `{fnptr, null-env}` closure pair SFN-667 built for top-level named functions, dispatched through the identical seam and mangled-symbol reference, with the shadow-aware lookup so a same-named `let`/parameter still wins at the reference site. The `E0808` guard is narrowed, not deleted: a nested fn with no expected `fn(...)` type, or a generic nested fn, is still `E0808`; nested fns now get the same `E0839`/`E0840` validation as top-level ones (free, since nested entries already carry full signature metadata). Calling through a fn-typed struct field now ships via SFN-674: the stored pair is loaded and sent through that identical seam. Calling through a fn-typed collection element remains unshipped. Codegen: `compiler/capsules/codegen/src/lambda_lowering.sfn` (`_rename_nested_reference`, `_lookup_nested_rename`), `compiler/capsules/codegen-llvm/src/expression_lowering/native/core_call_resolution/closure_callee.sfn` (field resolution); typecheck: `compiler/capsules/analyzer/src/typecheck_types/symbol_table_and_raw_exprs.sfn` (`SymbolEntry.is_nested_function`), `compiler/capsules/analyzer/src/typecheck/expression_walk.sfn`. Tests: `fn_reference_typecheck_test.sfn` (nested matrix), `nested_fn_value_test.sfn` + fixture `apply_nested.sfn`, `fn_field_dispatch_test.sfn` |
...
| `sfn check` | **Shipped** | Parse + typecheck + effect-check, no codegen; `--json` envelope; cross-module conformance; directory mode completes the full 156-file tree (~295 s — perf, not stability, is the open item); import resolution covering relative and capsule specs (`E0430`/`E0431`, #1953, SFN-686) |
...
- The binary's entry point is the Sailfin-emitted `@main` (M5, #451); no C
  ...
  sources live under `runtime/sfn/` and `runtime/prelude.sfn`. The former broad
  MinGW-only strong-stub IR is deleted: MinGW now links the same real Sailfin
  runtime providers as the native driver, with ABI differences declared in the
  runtime manifest. The sole target-specific IR is the narrow Windows
  `CreateSymbolicLinkA` `BOOLEAN` return bridge (SFN-1143), required because
  Sailfin's scalar extern surface cannot spell Win32's 8-bit return ABI.
...
| `io.poll_any` multi-fd readiness builtin | **Shipped** (SFN-155, epic #1540 Track A gap A3) | `io.poll_any(fds: int[], timeout_ms) -> int` — the multi-fd companion to `io.poll_readable`: waits on every fd in `fds` with a single `poll(2)` and returns which fd is ready (`>= 0`), or `-1` for timeout / empty list / error, so a stdio forwarder can wait on `{own stdin, child stdout, child stderr}` together without deadlock. Return shape is a sentinel `int` (`-1` == none), not an optional `fd?` — `int?` optional value-extraction isn't yet supported by the compiler (only `== null` round-trips). Sailfin-native body in `runtime/sfn/process.sfn` (`sfn_io_poll_any`). Windows implementation in `runtime/sfn/platform/process_windows.sfn` (`sfn_io_poll_readable:2135`, `sfn_io_poll_any:2163`, over `_get_osfhandle` + `PeekNamedPipe` with a bounded `Sleep(1)` loop) replaces the former `-1`/`false` stubs for both `io.poll_readable` and `io.poll_any`; a non-pipe handle (a console, or a socket, since `PeekNamedPipe` fails on one) is reported READY, so the caller's read may block — a degrade, strictly better than the prior unconditional "never readable" that hung a multiplexing caller outright, but not equivalent to `poll(2)`. `sfn_process_handle_stdout_fd`/`_stderr_fd` (`process_windows.sfn:1911`/`:1913`) still return `-1` on Windows, so the test runner's fd-based consumer takes its fd-free branch unchanged by this. Descriptor in `compiler/capsules/codegen-llvm/src/runtime_helpers/registry_process.sfn` (`io.poll_any`); declare-tracked in `lowering_helpers.sfn`. Pinned by `compiler/tests/integration/io_poll_any_test.sfn` (pipe-driven, `![io]`, 5 cases) |
...
- Release tarballs follow `sailfin_<version>_<os>_<arch>.tar.gz`. Release builds
  ...
  build with a real TLS 1.3 stack and, since v0.10.4 (SFN-1026), working
  networking, and the plain
  `sailfin_<version>_windows_x86_64.tar.gz`, the MinGW ABI build produced by
  the native target driver with the real Sailfin TLS/runtime graph. All three asset selectors —
  ...
  at any version). The MinGW asset remains the bootstrap seed for the native
  Windows lane pending SFN-58's full SFEP-0021 §4.5 release-cycle
  soak; see the TLS 1.3 row above for SFN-1026, now fixed and shipped in
  v0.10.4 (merged as `d8c858b8`, #3061). Release
...
| Windows x86-64 | **Tier 3 — best effort** (`docs/conventions/target-tiers.md`). Linux cross-builds are driver-native (`sfn build --target=x86_64-w64-mingw32`) and publish the release/bootstrap asset; `smoke-windows` (frontend-only: boots, `sfn check` on one example) blocks every source PR. SFN-55 (SFEP-0021 M9) adds native MSVC coverage: `build-compiler-windows` in `ci.yml` is a path-filtered `windows-2025` merge-blocking gate (fires only on PRs matching the Windows-relevant path filter, always on `merge_group`) that runs the SFN-53 diagnostic ladder plus boot/check/run/R1(try-throw)/R3(struct-channel) ABI gates. `windows-native-selfhost.yml` is the unconditional nightly backstop (push:main + 08:00 UTC cron): it builds release seed → working-tree compiler → fp1 → fp2, requires both byte identity and strict per-module IR identity, then runs the complete workspace test inventory once with fp1 and `--no-test-cache`. The automatic test-job budget is native-Windows-aware; the required suite reports discovered files/tests, pass/fail/skip counts, and wall time, and routes failures as `windows-native-tests`. This remains Tier 3 because the full suite is a post-merge nightly signal rather than a merge-blocking check; SFN-1014 owns required PR sharding. | Not a target — mediated vendor-library shim (SFEP-0016 §3.1) |
