...
- **Native TLS 1.3 (SFEP-0036/SFEP-0048, SFN-341).** The `sfn/crypto` capsule
  ...
  before `WSAStartup`, which blocks every hostname-based client connect on
  native Windows — is fixed on that branch but **not yet merged**; v0.10.3
  as published still has the bug. Application writes larger than one record's
