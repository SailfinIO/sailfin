...
Updated: 2026-07-23. Seed pinned to `0.8.0-alpha.5` (`bootstrap.toml`
...
| Non-allocating string-view slice (`substring`/`substring_unchecked`) | **Shipped** (SFN-42, SFEP-0025 §3.3, 2026-07-23) | `substring`/`substring_unchecked` now lower to `emit_substring_view` (`core_strings.sfn`) — an inline `{data+start, end-start}` aggregate view that aliases the source string's bytes, replacing the allocating `sfn_str_slice`/`sailfin_runtime_substring_unchecked` call. No copy, no arena allocation. Both indices are clamped to `[0, srclen]` (recovered from the source's carried length), so the historically "unchecked" entrypoint can no longer over-read the source tail — the trailing NUL that masked that over-read is gone along with the copy. This is the **first non-NUL-terminated string surface in the runtime**; it is sound only because every downstream consumer reads the carried length rather than NUL-scanning (#1704 length-aware query helpers, #1705 cstr-boundary copy-out, #1722 length-bounded `to_number`), and SFN-42 itself migrates the by-name `strings_equal`/`string.compare` helpers in `core_operands.sfn` to the length-aware `_lv` form. `sfn_str_slice` is kept as a dead-but-linked Sailfin body solely so the pinned seed's already-emitted `@sfn_str_slice` calls still resolve during `make compile`; it retires once the seed emits the new view directly. No user-facing lifetime/borrow enforcement over the returned view — that is Phase U. |
