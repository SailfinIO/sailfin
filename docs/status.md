...
Updated: 2026-08-07 (SFN-340). Seed pinned to `0.9.1` (`bootstrap.toml`
...
- **Native bootstrap install + fingerprint gate** (SFN-679). `sfn dev
  ...
  and `runtime`, content-addressed so restoring stale content under a fresh
  ...
  (2) The digest scope is `compiler/src` + `runtime` `.sfn` files only —
  inherited unchanged from the retired shell script, it does not cover
  `capsules/*/src/`, the C sources under `runtime/native/src/`, or
