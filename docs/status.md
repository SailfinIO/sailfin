...
| `int` / `float` numeric types | **Slice A shipped 2026-05-02** (annotated locals/params/returns lower to `i64` / `double`; extern accept-list admits both; integer/float arithmetic & comparison dispatch verified by `compiler/tests/e2e/test_numeric_int_float.sh`). Slices B–E (bitwise ops, wider widths, `as` casts, bare-literal defaulting + `number` retirement) deferred — see `docs/runtime_architecture.md` §3.7 for the limitations table (L1–L6). | Sizes/indices, bitwise ops, fixes the f64 precision hazard for explicitly-typed values; bare-literal precision still defers to Slice E |
