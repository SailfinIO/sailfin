...
| Generic type constraints | Planned | `fn sort<T: Comparable>`, real `Array<T>` / `HashMap<K, V>` / `Channel<T>`. Declaration-time bound validation shipped (#1868): a bound must name a real interface (E0821) of correct arity (E0822); instantiation-site satisfaction (E0820) and monomorphization remain |
