...
- The binary's entry point is the Sailfin-emitted `@main` (M5, #451); no C
  ...
  empty on every shipped target. The former MinGW-only strong-stub IR is
  deleted: MinGW now links the same real Sailfin runtime providers as the
  native driver, with ABI differences declared in the runtime manifest.
