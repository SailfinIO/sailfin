...
| Linux aarch64 | Tier-3 advisory CI cross-emits pass-1 with an arch-aware x86_64 compiler, then runs native pass-2/pass-3, the full suite, and the fixed-point verify on `ubuntu-24.04-arm` for source PRs and a nightly soak (SFN-474, SFEP-0056 §3.4–3.5); host-layout probe and direct `ld.lld` fast path included (SFN-473, §3.6); no published installer asset | Not a target |
