...
| `sfn test` lifecycle hooks (`before_each`/`after_each`/`before_all`/`after_all`) | **Shipped (#975, test-infra Phase 2.1) — ordering only** | Hook blocks parse into `TestDeclaration` with an additive `hook_kind` field and emit under a distinct `hook:<kind>` symbol. The synthesized test harness (`llvm/lowering/lowering_core.sfn`) wraps each `test:` call: `before_all` once before all tests, `before_each`/`after_each` around each test, `after_all` once after. Hooks are excluded from `-k`/`--tag` filtering (always run for surviving tests) and from the `all N tests passed` count. Covered by `compiler/tests/unit/lifecycle_hook_parse_test.sfn` + `compiler/tests/e2e/test_lifecycle_hook_ordering.sh`. **Limitations:** one hook per kind per file (a second is a duplicate-symbol error); a failing hook aborts the suite (with an attributable `HOOK before_all`/`HOOK after_all` stderr marker) — the `fail`-vs-`error` classification of dependent tests needs a recoverable per-test harness and is deferred (follow-up). The capsule-side registration API (`lifecycle.sfn`, #978) is seed-gated on this. |
| `sfn test --jobs N` (parallel multi-file runner) | **Shipped (#1236, test-infra Phase 4 Track B)** | Runs the multi-file runner's per-file `<self> test <file>` children concurrently through a bounded worker pool: a sliding window keeps ≤ N children live via `process.spawn_with_env`, and the parent reaps the oldest child in file order (`handle_read_stdout`/`handle_read_stderr` drain — mandatory before `handle_wait`, which closes the pipe fds — then `handle_wait`). In-order reaping preserves suite-banner attribution, the 137/139 signal-kill retry gate, per-child `sub-<i>` scratch isolation, the shared runtime-objdir pre-warm, and the file-based `--json` sub-frame aggregation unchanged; captured child output is re-emitted grouped per file, so human output and the single-frame jsonl document are never interleaved mid-file. Pooled children get an explicit environment (`spawn_with_env` never inherits): PATH/HOME/TMPDIR + every `SAILFIN_*`/`SFN_*` knob the toolchain reads, plus the per-child test vars. Default `--jobs 1` keeps the serial `process.run` loop byte-identical to the pre-#1236 runner; valid range is [1, 256] (anything else is a usage error, exit 2). **Memory:** each child is a full compiler invocation under its own 8 GB virtual cap — size N to the runner's physical RAM (macOS CI ~7 GB → small N); wiring CI/Makefile to pass `--jobs` is a post-pin follow-up since `make test` invokes the *pinned seed*-built runner. Covered by `compiler/tests/unit/test_jobs_flag_test.sfn` + `compiler/tests/e2e/test_runner_jobs_parallel.sh` (jobs-1 vs jobs-4 equivalence, failure propagation, usage errors). |
| `sfn test` snapshot matcher (`expect_snapshot` + `--update-snapshots`) | **Shipped (#977, test-infra Phase 2.4)** | `sfn/test::expect_snapshot(actual, key)` (`![io]`) writes `tests/__snapshots__/<key>.snap` on first run (golden absent), compares thereafter, and returns `MatchResult` with a line-by-line diff (length mismatch / first divergent index) on mismatch. The testable core `snapshot_match_in(actual, dir, key, update)` takes the directory + update flag explicitly. `sfn test --update-snapshots` re-baselines: the flag threads to each compiled test binary as `SAILFIN_UPDATE_SNAPSHOTS=1` (single-file leaf exec + multi-file child `env` prefix); non-snapshot runs keep a byte-identical child argv. The line diff is inlined rather than reusing #973's `![pure]` `matcher_eq_str_array` because an `![io]` function cannot also declare `![pure]` (E0403). Covered by `capsules/sfn/test/tests/snapshot_test.sfn` (5 tests: first-write, match, mismatch, length-divergence, update-overwrite). |
...
| Raw-byte string write primitive — `char_from_code` (issue #874) | **Shipped 2026-06-06**. `char_from_code(code: int) -> string` builds a one-byte string from the raw byte value `code` (1–255) — the write counterpart to the already-shipped byte read (`char_code(s[i])` returns the raw byte 0–255). Unlike `sfn_str_from_codepoint` (UTF-8 encode), it writes the byte verbatim with no expansion for 128–255, so `char_code(char_from_code(n)) == n` round-trips across 1–255. Removes the `char_from_code()`-pending workarounds flagged in `capsules/sfn/cli/src/style.sfn` and `capsules/sfn/crypto/src/mod.sfn` (the SHA-256 re-port and the hex-nibble lookup are follow-ups per the issue's `Out:`). Backed by a new C trampoline `sfn_str_from_byte(int64_t) -> char*` in `runtime/native/src/sailfin_runtime.c` (malloc'd NUL-terminated 1-byte buffer, arena-tracked, structure mirrors `sfn_str_from_codepoint`); the migration unit `sfn_str_sfn_from_byte` lands in `runtime/sfn/string.sfn` (proof-of-life trampoline, absorbs the body once the M1.A.2 aggregate flip lets it consume an arena directly). `runtime/prelude.sfn` exposes `char_from_code`, calling the C symbol through an inline `extern fn` (the #306 cross-module workaround `exec.sfn` uses — prelude emission runs before import contexts are staged, so an imported wrapper can't resolve its signature under the seed) and casting the `* u8` result to `string` on an alloca-backed local (the seed's literal-only cast bug is sidestepped). User call sites resolve via a new bare-name `char_from_code` descriptor in `compiler/src/llvm/runtime_helpers.sfn` (`symbol: "char_from_code"`, emitting `call i8* @char_from_code` linking to the prelude definition) — exactly the shape of the `char_code` row above; self-host is unaffected because the prelude's own body resolves through its function entry (#633), not the descriptor. **Limitation — byte 0:** a lone NUL is unrepresentable under the current NUL-terminated `string` model (`.length` is `strlen`-recovered), so `char_from_code(0)` returns `""`; faithful embedded-NUL storage is deferred to the `SfnString` aggregate flip (M1.A.2, `runtime_architecture.md` §2.2). Self-hosts (`make compile` / `make check`). Pinned by `compiler/tests/e2e/test_runtime_char_from_code.sh` (write-side `.length == 1` across the full 1–255, char_code round-trip for the high bytes 128–255, ASCII correctness via literal equality, raw high byte 200 stays one byte, multibyte read 195/169 intact, byte-0 empty-string case asserted so a future representation fix trips loudly). **macOS arm64 caveat (#1136):** `char_from_code` writes every byte correctly, but reading an ASCII byte (`1..0x7f`) back via `char_code` can mis-decode on macOS arm64 — `grapheme_at` encodes ASCII as the immediate-codepoint tag `(byte << 32)`, and on macOS the immediate-vs-real-pointer decision checks whether the address is mapped; for small byte values `byte << 32` may land on a mapped region (ASLR-dependent, varies run to run). A pre-existing `char_code`/immediate-encoding bug surfaced (not caused) by this work, so the test round-trips only the non-immediate `0x80..0xFF` path through char_code and covers ASCII via literal equality; tracked separately. | Completes the raw byte read/write pair pure-Sailfin code needs to build arbitrary binary/multibyte byte sequences; unblocks removing the crypto hex-nibble lookup and the CLI style workaround in follow-ups. |
...
| Closures with capture | **Shipped**. Lambdas capture enclosing variables end-to-end: free-variable / capture-set inference in `compiler/src/typecheck_captures.sfn` (#458, A1) → closure env-struct synthesis + alloc/load in `compiler/src/llvm/closures.sfn` (#459, A2 — now wired from `compiler/src/llvm/expression_lowering/native/core.sfn`) → lambda lifting + env-load prologue in `compiler/src/llvm/expression_lowering/native/lambda_lowering.sfn` and `compiler/src/llvm/lowering/instructions.sfn`, with hidden-env call-site dispatch (#689) so the `{fn_ptr, env*}` pair is callable. Capturing lambdas — including **multi-capture** (N≥2) — compile, link, and run end-to-end. #1106 fixed a two-or-more-capture miscompile: the env-alloc marker's capture list was comma-separated and, when a capturing lambda was passed inline as a call argument, the comma-splitting argument parser (`split_array_elements`) truncated the list to the first capture, so the env struct emitted one field too few (invalid GEP at link); the inter-capture separator is now `;` in the shared `_encode_captures`/`decode_captures` pair so the marker survives transport. Pinned by `compiler/tests/unit/typecheck_lambda_capture_test.sfn`, `compiler/tests/integration/closure_env_emission_test.sfn` + `compiler/tests/integration/closure_lifting_test.sfn`, and `compiler/tests/e2e/test_closure_capture.sh` (non-capturing, single-int-capture, higher-order `fn(int) -> int` dispatch, plus #1106's two-int, three-int, and mixed string+int capture fixtures). | `map`/`filter`/`reduce`, spawn handlers, route handlers |
...
**Status**: Largely closed. `Result<T, E>` + the postfix `?` operator now ship
end-to-end (#832/#833/#834, epic #371) — errors are visible in function
signatures and `?` propagates the `Err` arm with zero exception-runtime cost.
See the `Result<T, E>` + `?` row above and spec §12.

**Remaining**: `?` requires the operand's error type to match the enclosing
function's error type exactly — there is no `From<E>` coercion and no `E: Error`
bound yet, both gated on generic type constraints. `try`/`catch`/`throw` remain
for unrecoverable conditions and unmigrated code.
