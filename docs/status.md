...
`[seed].version`, mirrored in `.seed-version` during migration — SFEP-0047);
...
- **Compiler bootstrap manifest — `bootstrap.toml` + `sfn dev bootstrap`**
  ...
  Migration: `bootstrap.toml` is the source of truth; `.seed-version` is kept in
  lockstep (a mismatch is a hard error) until a bootstrap-aware seed is pinned
  and it is deleted in a follow-up (SFN-60). The Makefile bootstrap recipes stay
  as transitional compatibility shims.
