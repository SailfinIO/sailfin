...
| Sleep call-site routing (PR 1 of the sleep migration) | **Shipped 2026-05-04**. Runtime helper registry rewires compiled user `sleep(N)` and the prelude's `runtime_sleep_fn` to lower to `call void @sfn_sleep(double)` instead of `call void @sailfin_runtime_sleep(...)`. The symbol `@sfn_sleep` is currently a 3-line C trampoline in `runtime/native/src/sailfin_runtime.c` that calls `sailfin_runtime_sleep`. The Sailfin definition site `runtime/sfn/clock.sfn` ships in the tree as a typecheck/emit smoke target but is **not linked into any binary today**. Pinned by `compiler/tests/e2e/test_runtime_clock_skeleton.sh` and `compiler/tests/e2e/test_sleep_routes_to_sfn_clock.sh`. **Open follow-ups: #305 (standalone emit IR consistency), #308 (link-time subprocess-emit IPC race — PR 2's hard prerequisite). #306 (extern call return-type defaulting) shipped 2026-05-06 — see row below.** | Establishes the pattern that future C-to-Sailfin migrations will follow: rewire the call site, back the symbol with C, replace the C body with a Sailfin link in a follow-up PR once link infrastructure is reliable |
