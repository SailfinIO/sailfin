...
| Mechanical string ops (`string.sfn`) | **Shipped** (#1372, C5 of epic #1308, 2026-06-17) | All four — `sfn_str_len` (`string.length`), `sfn_str_eq` (string `==`), `sfn_str_to_number` (`string.to_number`), and `sfn_str_slice` (`substring`) — are now real Sailfin bodies (bare emission targets) in `runtime/sfn/string.sfn`; the C namesakes in `sailfin_runtime.c` are `static`. `len`/`eq`/`to_number` classify immediate-codepoint pseudo-pointers via the non-allocating `sfn_str_immediate_codepoint` bridge first (the #714/#716 crash class — `char_at` results arrive tagged), then `len` → bounded libc `strnlen` (16 MiB cap, no unbounded loop), `eq` → codepoint compare / length + `memcmp` (`_sfn_imm_eq_real` UTF-8 byte compare for the immediate-vs-real arm), `to_number` → ASCII-digit fast path / `strtod`. `slice` clamps the `f64` indices to i64 and calls `sailfin_runtime_substring_unchecked` (not the registry-hijacked `substring_unchecked` wrapper, which would self-recurse). **Two compiler bugs were fixed to enable this:** (1) `core_type_mapping.sfn::map_primitive_type` was missing `f64`/`f32`, so the call-site param resolver stringified `f64` arguments to directly-called Sailfin functions (`slice`'s two `double` params) — now mapped, matching the two canonical copies; this required a seed bump to **0.7.0-alpha.37**. (2) The UTF-8 masks use **decimal** literals because the lexer has no `0x` hex support (hex silently lowers to `0` — a latent bug noted for follow-up). ABI unchanged (`i64`/`i1`/`double`/`i8*`), no `runtime_helpers.sfn` edit. All four `sfn_str_*` symbols now resolve into `string.o`, none in the C runtime object (relink gate). `concat`/`append` (ABI-hard) remain in #1318. Pinned by `capsules/sfn/strings/tests/strings_test.sfn`. |
