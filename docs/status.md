...
| String interpolation (`{{ }}`) | Shipped | Primitive values and `int \| null` union payloads stringify in direct, narrowed, and match-bound positions (SFN-343); primitive-element arrays (`int[]`/`boolean[]`/`string[]`) and nested arrays render bracketed (`[1, 2, 3]`, `[[19, 22], [43, 50]]`) (SFN-408). Any interpolation operand with no stringify arm — e.g. `float[]` (deferred pending a seeded pointer-read-f64 intrinsic) or a struct — now fails the build loudly (`E0832 [fatal]`), never the old silent empty output. `${ }` migration planned pre-1.0 (see Known Design Issues) |
