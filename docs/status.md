...
| `int` / `float` numeric types | **Slices A + C shipped 2026-05-02**. Slice A: annotated locals/params/returns lower to `i64` / `double`; extern accept-list admits `int` / `float`; integer/float arithmetic & comparison dispatch verified by `compiler/tests/e2e/test_numeric_int_float.sh`. Slice C: extern accept-list expanded to `i16`/`u16`/`u32`/`u64`/`isize`/`f32` so the libc surface can name the practical widths. Slices B (bitwise ops), D (`as` casts + tighten `dominant_type` for the wider widths), and E (bare-literal defaulting + `number` retirement) deferred — see `docs/runtime_architecture.md` §3.7 for the limitations table (L1–L6). | Sizes/indices, bitwise ops (Slice B), fixes the f64 precision hazard for explicitly-typed values; bare-literal precision still defers to Slice E |
