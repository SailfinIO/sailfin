...
| Sized-integer (`u8`/`u16`/`u32`/`u64`/`usize`) `as`-cast sign correctness | **Shipped** (identifier/parameter operands; SFN-503) | Widening `as` casts from an unsigned source now emit `zext` (was always `sext`); int→float from an unsigned source emits `uitofp`; float→unsigned-int emits `fptoui`. Sign is recovered from the operand's source-level annotation via its local/parameter binding, so a compound operand (`s.field as u64`, `xs[i] as u64`, `f() as u64`) still has no annotation to recover and still selects the signed forms — scoped to the typecheck slice of SFEP-0058 §3.3 (SFN-501) |
