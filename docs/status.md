...
| `int` / `float` numeric types | **Slices A + B + C shipped 2026-05-02; Slice D parser shipped 2026-05-02 (numeric lowering deferred)**. Slice A: annotated locals/params/returns lower to `i64` / `double`; extern accept-list admits `int` / `float`; integer/float arithmetic & comparison dispatch verified by `compiler/tests/e2e/test_numeric_int_float.sh`. Slice B: `&`, `|`, `^`, `<<`, `>>` lex/parse/lower to LLVM `and`/`or`/`xor`/`shl`/`ashr i64`, verified by `compiler/tests/e2e/test_numeric_bitwise.sh`. Slice C: extern accept-list expanded to `i16`/`u16`/`u32`/`u64`/`isize`/`f32`. **Slice D parser-only:** `expr as Type` parses to `Expression.Cast { operand, target_type }`; the native `.sfn-asm` emitter renders `(operand) as <type>` (operand parens preserved so the existing LLVM-lowering cast recognizer can recover the operand via `strip_enclosing_parentheses`), the Sailfin-source emitter renders the human-friendly `expr as Type` form (no parens — no re-parse step consumes that output); `as` is a soft keyword reusing the import-alias precedent. Numeric pair LLVM lowering (sitofp / fptosi / sext / trunc / fpext / fptrunc) deferred as a follow-up; the previously-cited "alpha.8 seed self-host bug" was diagnosed and fixed 2026-05-02 — it was a parser silent-skip on comma-separated struct fields, see `docs/runtime_architecture.md` §3.7 status delta. Pinned by `compiler/tests/unit/numeric_cast_test.sfn`, `compiler/tests/unit/struct_field_separator_test.sfn` (incl. generic-type cases), and `compiler/tests/e2e/test_struct_field_separator.sh` (incl. `Result<T, E>` round-trip). Slice E (bare-literal defaulting + `number` retirement) still planned. | Sizes/indices, bitwise ops (shipped), fixes f64 precision hazard for explicitly-typed values; numeric `as` casts parse cleanly but currently hit "unsupported cast" diagnostic at lowering — pointer↔pointer / int↔pointer casts unaffected; bare-literal precision defers to Slice E |
