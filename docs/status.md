...
| Mechanical string ops (`string.sfn`) | **Shipped** (#1372, C5 of epic #1308, 2026-06-17) | `sfn_str_len` (`string.length`), `sfn_str_eq` (string `==`), and `sfn_str_to_number` (`string.to_number`) are now real Sailfin bodies (bare emission targets) in `runtime/sfn/string.sfn`; the C namesakes in `sailfin_runtime.c` are `static`. Each classifies immediate-codepoint pseudo-pointers via the non-allocating `sfn_str_immediate_codepoint` bridge first (the #714/#716 crash class — `char_at` results arrive tagged), then: `len` → bounded libc `strnlen` (16 MiB cap, no unbounded loop) on the decode-passthrough real pointer; `eq` → codepoint compare for immediates / length + `memcmp` for real strings (`_sfn_imm_eq_real` UTF-8 byte compare for the mixed arm); `to_number` → ASCII-digit fast path / `strtod`. UTF-8 masks use **decimal** literals (the seed lexer has no `0x` hex support — hex literals silently lower to `0`, a latent bug noted for follow-up). **`sfn_str_slice` (`substring`) is NOT yet ported** — its registry signature carries two `double` params and the pinned seed (≤0.7.0-alpha.36) miscompiled f64 function arguments; the codegen fix landed here (`core_type_mapping.sfn::map_primitive_type` now maps `f64`/`f32`, matching the two canonical copies), so `slice` flips to Sailfin once a seed built with this fix is pinned. The C `sfn_str_slice` stays the emission target meanwhile. ABI unchanged (`i64`/`i1`/`double`/`i8*`), no `runtime_helpers.sfn` edit. `concat`/`append` (ABI-hard) remain in #1318. Pinned by `capsules/sfn/strings/tests/strings_test.sfn`. |
