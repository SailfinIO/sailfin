...
| Linux aarch64 | Tier-3 advisory CI runs the native bootstrap, full suite, and triple-pass fixed-point verify on `ubuntu-24.04-arm` for source PRs and a nightly soak (SFN-474, SFEP-0056 §3.4–3.5); host-layout probe and direct `ld.lld` fast path included (SFN-473, §3.6); no published installer asset | Not a target |
