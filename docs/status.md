...
| `int` / `float` numeric types | **Slices A + B + C shipped 2026-05-02**. Slice A: annotated locals/params/returns lower to `i64` / `double`; extern accept-list admits `int` / `float`; integer/float arithmetic & comparison dispatch verified by `compiler/tests/e2e/test_numeric_int_float.sh`. Slice B: `&`, `|`, `^`, `<<`, `>>` lex/parse/lower to LLVM `and`/`or`/`xor`/`shl`/`ashr i64`, verified by `compiler/tests/e2e/test_numeric_bitwise.sh`. Slice C: extern accept-list expanded to `i16`/`u16`/`u32`/`u64`/`isize`/`f32`. Slices D (`as` casts + tighten `dominant_type` for the wider widths) and E (bare-literal defaulting + `number` retirement) deferred — see `docs/runtime_architecture.md` §3.7 for the limitations table (L1–L6). | Sizes/indices, bitwise ops (now shipped), fixes the f64 precision hazard for explicitly-typed values; bare-literal precision still defers to Slice E |
