...
- Replaced `recover_enums_from_file(_trace_enums)` call with `build_enum_infos_inline(fixed_enums_list, _trace_enums)`.
...
- Pruned unused imports: `substring` (from `string_utils`), `index_of` (from `utils`), `parse_number_local` + `split_lines_local` (from `lowering_text_utils`).
...
- **Effect narrowing**: `build_enum_infos_inline` is pure (no `![io]`). `serialize_enum_info`'s `![io]` obligation goes away with the function itself. `build_type_context_with_recovery` retains `![io]` because it still calls `serialize_struct_info` and writes `.phase_types_diagnostics`.
