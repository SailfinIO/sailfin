...
- **Linker dead-code strip for capsule links** — **In review (#1047, part of #1046).** Every `clang -c` in the driver (`compiler/src/cli_main.sfn`: runtime c-sources, ll-sources, sfn-source objects, and the `no_link` library compile) emits `-ffunction-sections -fdata-sections`, and the final link in `_clang_link_multi_with_opt` adds the platform-detected GC flag (`-Wl,--gc-sections` on Linux/lld, `-Wl,-dead_strip` on Darwin via the existing `uname -s` probe). The runtime provider surface is rooted via `-Wl,-u` so the GC keeps it; the strip is skipped (un-stripped link) if those roots can't be enumerated, so the runtime surface is never silently dropped. The capsule link step still enumerates a dep's whole `src/` tree, but the linker now drops every function/datum unreferenced from the entry point, so a consumer that imports one symbol from a multi-function capsule no longer carries the rest. Verified on a fixture: importing one of two exported functions strips the unused one (`nm` confirms absence). Sailfin-side per-symbol tree-shaking remains a separate, larger effort.
- **Faster-linker selection (mold/lld)** — **In review (#343, PR #1128).** The final clang link in `_clang_link_multi_with_opt` (`compiler/src/cli_main.sfn`) now opportunistically selects a faster linker via `-fuse-ld=`. The detection helper `_resolve_linker()` (`compiler/src/cli_commands_utils.sfn`, mirroring the retired `_resolve_llvm_objcopy_cmd` shape) probes PATH in order — `mold`, then `lld` (`ld.lld` or `lld`) — and passes `-fuse-ld=mold` / `-fuse-ld=lld` when found; otherwise it falls back to the platform default with no flag. `lld` is ~2–5× faster than `ld.bfd`/`ld.gold` on cold links of large object sets; `mold` is faster again. Both the end-user `sfn build` of binary capsules and the compiler self-build (`make compile`'s `sfn build -p compiler`) flow through this single call site, so both benefit. **Override contract:** `SAILFIN_LINKER` wins over auto-detection — `SAILFIN_LINKER=ld` (also `system`/`default`) forces the platform linker with no `-fuse-ld` flag (use this for reproducible builds pinned to a known linker); any other value selects that linker by name (e.g. `SAILFIN_LINKER=mold`). The chosen linker is surfaced on a single `[link] linker: …` info line (emitted on **stderr**, like `[cache]`, so it never contaminates a program's stdout under `sfn run` or a downstream pipe) so the selection is never invisible. **Platform gate:** auto-detection is Linux-only — `lld`'s Mach-O backend has historically lagged `ld64` on edge cases and the path is not smoke-tested on macOS, so Darwin keeps the system default unless the user opts in via `SAILFIN_LINKER`. The helper is intentionally self-contained so #1112's `Backend` interface can absorb the link call site (and this logic with it) without untangling driver-level state. Self-contained helper, no source-side tradeoffs.
