...
- **Linker dead-code strip for capsule links** — **Shipped (#1047, part of #1046).** Every `clang -c` in the driver (`compiler/src/cli_main.sfn`: runtime c-sources, ll-sources, sfn-source objects, and the `no_link` library compile) now emits `-ffunction-sections -fdata-sections`, and the final link in `_clang_link_multi_with_opt` adds the platform-detected GC flag (`-Wl,--gc-sections` on Linux/lld, `-Wl,-dead_strip` on Darwin via the existing `uname -s` probe). The capsule link step still enumerates a dep's whole `src/` tree, but the linker now drops every function/datum unreferenced from the entry point, so a consumer that imports one symbol from a multi-function capsule no longer carries the rest. Verified on a fixture: importing one of two exported functions strips the unused one (`nm` confirms absence; ~10 KB binary-size delta). Sailfin-side per-symbol tree-shaking remains a separate, larger effort.
