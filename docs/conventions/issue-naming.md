...
There are exactly **four** title shapes. Anything else is a bug.
...
### 5. Auto-generated workflow-failure issues

The gh-aw harness opens `[aw] <Workflow Name> failed` issues automatically
when an autonomous workflow crashes. Leave the `[aw]` prefix in place —
