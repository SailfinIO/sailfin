# Stage2 Debugging Status (Oct 2025)

## Latest Findings (Oct 2025)

- Duplicate-linkage diagnostics now land in `runtime/stage2_runner.py`, so the Stage2 debug log reports per-module duplicate counts plus a histogram of post-link renames. This ties each anonymous module ID back to its source (`module_4` → `emitter_sailfin.sfn`, `module_9` → `native_lowering.sfn`, etc.).
- The current Stage2 repro (`SAILFIN_STAGE2_DEBUG_LOG=scratch/stage2_debug.log python scratch/stage2_repro.py`) scopes 798 duplicate definitions across 16 modules and applies 683 post-link renames. Hotspots: `add` (15 definitions), `.str.len0.h177573` (11 copies), and helpers like `append_string`, `trim_text`, and enum variant constants. Most are already lowered with non-export linkage, so the rename pass is reworking largely harmless locals.
- The final abort persists (`Trace/BPT trap: 5`) after the duplicate warning. The new logs show the runtime still progressing deep into the lex/parse helpers (lots of `string_length` probes), confirming that duplicate spam is masking the remaining signal rather than causing the crash outright.

### Next Actions

1. Adjust the final dedupe loop in `Stage2Runner` to ignore functions/globals that already use local linkage so only real collisions trigger renames.
2. Update lowering to namespace or mark shared helpers (`add`, string utilities, enum constant generators) as `private` up front, reducing duplicate pressure.
3. Re-run the Stage2 repro once duplicate noise is under control and focus on the runtime trace around the trap to isolate the lingering failure.

## Current Outlook (Feb 2025 snapshot)
...
- **Runner instrumentation**
  ...
  - Additional IR instrumentation now drops debug markers throughout `compile_to_sailfin`, `parse_tokens`, `parse_block`, and `parse_unknown`, making it easy to correlate log output with parser progress (`2000+`, `6000+`, `8000+`, `10000+`/`11000+`/`13000+` ranges).
   ...
   - Stage2 debug logs now stop once they reach ~25 MB by default so WSL/CI disks do not overflow. Override with `SAILFIN_STAGE2_DEBUG_MAX_BYTES=0` for unlimited logging or set a custom byte budget.
