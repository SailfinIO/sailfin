...
ci-cross-windows:
	...
	: "sibling. This mirrors the native path's"; \
	: "target_condition_runtime_sfn_sources swap; the two lists are"; \
	: "separate mechanisms and BOTH must be updated when a runtime"; \
	...
	: "Also PLUS platform/mkdtemp_windows.sfn (SFN-993, no POSIX"; \
	: "counterpart to swap FROM): 'nm --defined-only libmingwex.a' shows"; \
	: "mkstemp present but mkdtemp ABSENT, so filesystem.sfn's"; \
	: "extern fn mkdtemp has no definition at the mingw link without"; \
	: "this entry — the bridge fails undefined, not merely degraded."; \
	: "Also MINUS platform/exclusive_create.sfn PLUS its sibling"; \
	...
	$(NATIVE_OUT) package --installer --target "$(MINGW_TARGET)" --out dist --compiler-bin "$$WIN_OUT"; \
