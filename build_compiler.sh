...
# Setup Python environment for bootstrap
setup_bootstrap() {
    log_info "Setting up bootstrap compiler..."
    ...
    # Check if bootstrap.py is executable
    if [ ! -f "bootstrap.py" ]; then
        log_error "bootstrap.py not found in bootstrap directory"
        exit 1
    fi
    
    log_success "Bootstrap compiler setup complete"
}

# Compile self-hosting compiler using bootstrap
compile_self_hosting() {
    log_info "Compiling self-hosting compiler..."
    
    # Create build directory
    mkdir -p "$BUILD_DIR"
    cd "$BUILD_DIR"
    
    # Use bootstrap compiler to compile the main self-hosting compiler
    log_info "Compiling main.sfn with bootstrap compiler..."
    python3 "$BOOTSTRAP_DIR/bootstrap.py" "$COMPILER_DIR/main.sfn" -o "main_compiler.py"
    
    if [ ! -f "main_compiler.py" ]; then
        log_error "Failed to compile main.sfn"
        exit 1
    fi
    
    # Create a wrapper script for the sfn compiler
...
EOF
    ...
    # Create the sfn binary wrapper
    cat > "sfn" << 'EOF'
#!/bin/bash
# Sailfin Compiler Binary Wrapper
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
...
EOF
    ...
    log_success "Self-hosting compiler compiled successfully"
}
...
main() {
    ...
    case "$command" in
        ...
        compiler)
            ...
            compile_self_hosting
            test_compiler
            log_success "🎉 Compiler build completed!"
            ;;
        test)
            if [ ! -d "$BUILD_DIR" ]; then
