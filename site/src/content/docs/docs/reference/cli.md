...
Selection is resolved once, from the current working directory, before the full command tree is parsed — so it applies to every ordinary command whose output can vary with the compiler (`version`, `init`, `fmt`, `check`, `build`, `run`, `test`, `emit`, `bench`, `package`, `add`, `lock`, `publish`, `capabilities`, `symbols`, `cache`), including a command a given toolchain doesn't recognize. Five prefixes are never project-dispatched: `dev`, the self-hosting driver, which builds a checkout with the binary it was invoked as; `toolchain`, which has to stay reachable to repair a broken or missing selected payload; and `config`, `login`, and `completion`, which are entry-level configuration surfaces — `completion` especially, since it is commonly run from a shell startup file and must never block on a download. Highest precedence first:

1. A one-shot CLI selector: `sfn +<version> <command> ...` (equivalently `sfn toolchain run <version> [--] <command> ...`).
2. An explicit `SAILFIN_TOOLCHAIN=<version>` (a mode word — `auto`/`local`/`off`/`0` — does not count as a selector here; see the table above). This rank is applied by `build`, `check`, `run`, `test`, and `version` only; on the other commands an exact `SAILFIN_TOOLCHAIN` is not yet a selection input, and setting one suppresses rank 3 without replacing it — prefer a `+<version>` one-shot there.
3. The active root's exact `[toolchain] version`.
4. The per-user default recorded by [`sfn toolchain default`](#default-toolchain-and-removal).
5. The entry toolchain on `PATH`.

Whichever candidate wins still has to satisfy this root's `sfn` floor and `channel` constraint — an explicitly selected compiler below the floor is a hard error unless one of the [escape hatches](#toolchain-pinning-flags) downgrades it to a warning. Those hatches (`--skip-toolchain-check`, `SAILFIN_SKIP_TOOLCHAIN_CHECK=1`, `SAILFIN_TOOLCHAIN=off`/`0`) also decline ranks 3 and 4 outright, for the same reason: an exact `version` and a recorded default both select unconditionally, so there is no mismatch for the hatch to override, and honoring it means running the entry toolchain and not dispatching at all. A manifest whose `version` is malformed or contradicts its own floor is a different case — that is a manifest error, not a version mismatch, and no hatch suppresses it. Dispatch works in both directions: a newer entry toolchain re-execs an older exact `version`, and an older entry hands the command to a newer one.
...
The default record lives at `<config-dir>/toolchain-default`, where `<config-dir>` is `SAILFIN_CONFIG_DIR` when set, otherwise `~/.sfn` — the same directory `sfn config`'s `config.toml` and `sfn login`'s `credentials` resolve against (see [`SAILFIN_CONFIG_DIR`](#environment-variables)). It's keyed by host triple, so one home directory — roaming or shared — can carry a different default per host, and it's written atomically (a temp file, then a rename), so an interrupted write can never publish a half-written default.
...
| `SAILFIN_BUILD_JOBS` | `sfn build -p compiler` | Override compiler module scheduling inside the build driver. Use `1` for serial bisects or a small value on memory-constrained hosts. Falls through to `[build] jobs` in `config.toml` (`~/.sfn/config.toml`, or wherever `SAILFIN_CONFIG_DIR` relocates it to) when unset, ahead of the CPU/RAM probe. The two rungs clamp differently: this variable is clamped only to `[1, 8]` with no RAM-budget check, while `[build] jobs` is clamped to `[1, 8]` *and* the host RAM budget — see below. |
...
| `SFN_REGISTRY` | `sfn add` / `sfn publish` | Override the package registry base URL for this shell. Takes precedence over `[registry] url` in `config.toml` (`~/.sfn/config.toml`, or wherever `SAILFIN_CONFIG_DIR` relocates it to). See [`sfn config`](#sfn-config-getsetunsetlist-key-value). |
| `SFN_TOKEN` | `sfn publish` | Bearer token used when uploading a capsule. Takes precedence over `credentials` (`~/.sfn/credentials`, or wherever `SAILFIN_CONFIG_DIR` relocates it to) written by `sfn login`. |
...
| `SAILFIN_CONFIG_DIR` | `sfn config` / `sfn login` / `sfn publish` / `sfn add` / `sfn build`/`run`/`test` / `sfn toolchain ...` | Override the directory holding all three per-user records, in place of `~/.sfn`: `config.toml` (`sfn config` get/set/list, the `toolchain.update-policy` gate, registry-URL resolution, and the SFEP-0076 `[build]` and `[target.<triple>]` rungs that steer `jobs`, `cache-dir`, `linker` and `cc` during a build), `credentials` (written by `sfn login`, read by `sfn publish`), and the toolchain-default record (`toolchain-default`). Because `config.toml` moves with it, this variable affects builds and `sfn add`, not only the commands that read the file directly. The override is total, not a search path: when set, the real `~/.sfn` is not consulted for any of the three, with no read-through fallback. **Changed:** earlier releases relocated only `toolchain-default` and left `config.toml` and `credentials` under the real home — a setup that already sets this variable will stop seeing an existing `~/.sfn/config.toml` or `~/.sfn/credentials`, and the next `sfn login` writes the token into the relocated directory. Use an **absolute** path: a relative value resolves against each process's working directory, so one setting would name a different directory per directory you build from. An empty `SAILFIN_CONFIG_DIR=` is indistinguishable from unset and falls back to `~/.sfn`. Does not relocate the capsule cache (`~/.sfn/cache/`), which still follows `HOME`. See [Default toolchain and removal](#default-toolchain-and-removal). |
