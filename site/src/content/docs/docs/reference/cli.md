...
| `SAILFIN_CONFIG_DIR` | `sfn config` / `sfn login` / `sfn publish` / `sfn toolchain default` / `remove` | Override the directory holding all three per-user records, in place of `~/.sfn`: `config.toml` (`sfn config` get/set/list, the `toolchain.update-policy` reader, registry-URL resolution, and the SFEP-0076 `[target.<triple>]` linker/cc rung), `credentials` (written by `sfn login`, read by `sfn publish`), and the toolchain-default record (`toolchain-default`). The override is total, not a search path: when set, the real `~/.sfn` is not consulted for any of the three, with no read-through fallback. An empty `SAILFIN_CONFIG_DIR=` is indistinguishable from unset and falls back to `~/.sfn`. Does not relocate the capsule cache (`~/.sfn/cache/`), which still follows `HOME`. See [Default toolchain and removal](#default-toolchain-and-removal). |
